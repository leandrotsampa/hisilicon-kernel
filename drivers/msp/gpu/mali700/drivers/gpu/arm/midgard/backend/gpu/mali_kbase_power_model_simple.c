...
static unsigned long model_static_power(unsigned long voltage)
{
	...
	const unsigned long coefficient = (400UL << 20) / (800000000UL >> 10);
	const unsigned long voltage_cubed = (voltage * voltage * voltage) >> 10;
	u32 s_svb_type;

	s_svb_type = kbase_svb_identify();
	...
	if(KBASE_CHIP_TYPE_TT == s_svb_type) {
		temp_scaling_factor =
			((20 * temp_cubed)
				- (900 * temp_squared)
				+ (41000 * temp)
				+ 50000)/100;
	} else if(KBASE_CHIP_TYPE_FF == s_svb_type) {
		temp_scaling_factor =
			((28 * temp_cubed)
			- (652 * temp_squared)
			+ (27802 * temp)
			+ 50000)/100;
	} else if(KBASE_CHIP_TYPE_SS == s_svb_type) {
		temp_scaling_factor =
			((14 * temp_cubed)
			- (857 * temp_squared)
			+ (40621 * temp)
		...
		unsigned long voltage)
{
	...
	const unsigned long coefficient = 2700; /* mW/(MHz*mV*mV) */
