...
{
    NUM_2_STRING(2D_FRAME),		//2D����
    NUM_2_STRING(2D_FRAME_VIRTUAL ),	//2D��������˿�), ת��ʹ��
    NUM_2_STRING(2D_DEI),		//2D����),  hd
    NUM_2_STRING(2D_DEI_VIRTUAL),	//2D����),  hd����˿�), ת��ʹ��
    NUM_2_STRING(2D_FIELD),		//2D����), ��������
    NUM_2_STRING(2D_FIELD_VIRTUAL),	//2D����), ������������˿�), ת��ʹ��
    NUM_2_STRING(2D_SD_FIRST),		//sd),	����),	tnr),  dei
    NUM_2_STRING(2D_SD_SECOND),		//sd),	����),	snr),  vhd0 funcs
    NUM_2_STRING(2D_SD_VIRTUAL),	//sd),	��������˿�), ת��ʹ��
    NUM_2_STRING(2D_CTS10),		//ֻ��2Dʱ��
    NUM_2_STRING(2D_CTS10_VIRTUAL ),	//CTS10), ����˿�
    NUM_2_STRING(2D_UHD),
    NUM_2_STRING(2D_UHD_VIRTUAL),
    NUM_2_STRING(3D_FRAME_L),		//3D����
    NUM_2_STRING(3D_FRAME_R),		//3D����
    NUM_2_STRING(ROTATION_Y),		//Y), C��ת
    NUM_2_STRING(ROTATION_Y_VIRTUA5),
    NUM_2_STRING(ROTATION_C),		//Y), C��ת
...
HI_S32 VPSS_DBG_SendDbgCmd_SaveYUV(VPSS_DBG_S *pstDbg, VPSS_DBG_CMD_S *pstCmd)
{
	...
	case DEF_DBG_PORT2_ID:
		...
		VPSS_FATAL("VPSS will save yuv from %d to %d\n", pstPortDbg->unInfo.bits.start_index,
...
HI_S32 VPSS_DBG_SendDbgCmd_Rotation(VPSS_DBG_S *pstDbg, VPSS_DBG_CMD_S *pstCmd)
{
	...
	case DEF_DBG_PORT2_ID:
		...
		VPSS_FATAL("set prot%d rotation debug disable!!\n", u32Count);
	    }
	    else
	    {
		...
		VPSS_FATAL("set prot%d rotation %d !!\n", u32Count, pstCmd->hDbgValue * 90);
	    }
...
HI_S32 VPSS_DBG_SendDbgCmd_SetOutBitWidth(VPSS_DBG_S *pstDbg, VPSS_DBG_CMD_S *pstCmd)
{
	...
	case DEF_DBG_PORT2_ID:
		...
		VPSS_FATAL("set prot%d outbitwidth debug disable!!\n", u32Count);
	    }
	    else
	    {
		...
		VPSS_FATAL("set prot%d outbitwidth %d bit!!\n", u32Count, pstCmd->hDbgValue * 2 + 8);
	    }
...
HI_S32 VPSS_DBG_SendDbgCmd_UVInvert(VPSS_DBG_S *pstDbg, VPSS_DBG_CMD_S *pstCmd)
{
	...
	case DEF_DBG_PORT2_ID:
	    ...
	    VPSS_FATAL("set prot%d uv Invert %s !!\n", u32Count,
...
HI_S32 VPSS_DBG_SendDbgCmd(VPSS_DBG_S *pstDbg, VPSS_DBG_CMD_S *pstCmd)
{
	...
	case DBG_INFO_FRM:
		...
		case DEF_DBG_SRC_ID:
		    ...
		    VPSS_FATAL("Print src image info from index %d to %d\n",
	...
	case DBG_SET_I2P:
		...
		case DEF_DBG_SRC_ID:
		    ...
		    VPSS_FATAL("set interlace to progress %s !!\n",
	...
	case DBG_REVISE_SRC_WH:
	    ...
	    VPSS_FATAL("Revise src image width*height %d*%d \n",
...
HI_S32 VPSS_DBG_ReplyDbgCmd_SaveYUV(VPSS_DBG_S *pstDbg, VPSS_DEBUG_E enCmd, HI_VOID *para1, HI_VOID *para2)
{
	...
	case DEF_DBG_PORT2_ID:
	    ...
	    break;
	case DEF_DBG_SRC_ID:
	    VPSS_FATAL("Do not support saving src frame!\n");
	    break;
	default:
...
HI_VOID VPSS_DBG_PrintFrameInfo(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_BOOL bSrcFrame, HI_U32 u32StartIndex, HI_U32 u32EndIndex)
{
    ...
    {
	HI_PRINT("Src Frame : Index %d Type %d Format %d W %d H %d Prog %d topfirst %d FieldMode %d enBufValidMode %d\n"
		 "	      PTS %d Rate %d LastFlag %#x Delta %d CodeType %d,SourceType %d,BitWidth %d,bSecure:%d\n"
    ...
    {
	HI_PRINT("Out Frame : Index %d Type %d Format %d W %d H %d LW %d LH %d PTS %d Rate %d Cnt %d Fidelity %d\n"
...
HI_VOID VPSS_CTRL_ProcPrintTimeStamp(struct seq_file *p, VPSS_INSTANCE_S *pstInstance)
{
	...
	PROC_PRINT(p,
		   ...
		   VPSS_PROC_LEVEL4 "TunnelOut		  : %lu(+%lu)"
		   ...
		   pstInstance->aulLogicEndTime[u32Count] - pstInstance->aulTunnelOutTime[u32Count],
...
HI_S32 VPSS_CTRL_ProcPrint_V2(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "------------------ DATE:%s --- SN:%s ------------------", VPSS_CODE_TIMESTAMP, VPSS_CODE_LASTGITNO);
