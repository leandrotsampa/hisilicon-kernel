 ...
 *-----------------------------------------------------------------*/
HI_S32 KEYLEDOpen(KeyLed_Status_E enKeyLedStatus)
{
    HI_S32 ret = 0;
    atomic_t Count;

    Count = (enKeyLedStatus & KeyON) ?
		     g_KeyOpenCount : g_LedOpenCount;
...
HI_S32 KEYLEDClose(KeyLed_Status_E enKeyLedStatus)
{
    HI_S32 ret = 0;
    atomic_t Count;
...
HI_S32 KEYLEDIoctl(struct inode *inode, struct file * file, HI_U32 cmd, HI_VOID * arg)
{
	...
	ret =  KEYLEDOpen(enKeyLedStatus);
	break;

    case HI_KEYLED_KEY_CLOSE_CMD:

	enKeyLedStatus = KeyON;
	ret =  KEYLEDClose(enKeyLedStatus);
	break;
	...
	CHECK_KEY_OPEN();
	ret = g_stKeyLedOpt.KEYLED_KEY_SetRepTime(*(HI_U32*)arg);
	break;
