...
#define DAX_SUPPORT(DataMark) \
		      if (   (M_JPEG_DAC == DataMark) || (M_JPEG_COM == DataMark) || (M_JPEG_TEM == DataMark) || (M_JPEG_DNL == DataMark))\
		      {\
			  return HI_FAILURE;\
		      }
...
static inline HI_BOOL DRV_JPEG_CheckIsSupportSofInfo(IMAG_INFO_S *pstImgInfo)
{
    ...
    if ((pstImgInfo->s32InWidth > 8095) || (pstImgInfo->s32InHeight > 8095))
    {
...
HI_S32 DRV_JPEG_Parse(HI_DRV_JPEG_INMSG_S *pstInMsg, IMAG_INFO_S *pstImgInfo)
{
    ...
    HI_U32 FirstBufOffset = 0;
    pstImgInfo->s32RestartInterval = -1;

    pVirBuf = (HI_CHAR*)HI_GFX_Map(pstInMsg->u32AllPhy,pstInMsg->u32AllLen,HI_TRUE);
    if (NULL == pVirBuf)
    {
       return HI_FAILURE;
    }

    FirstBufOffset = (pstInMsg->u32SavePhy > pstInMsg->u32AllPhy) ? (pstInMsg->u32SavePhy - pstInMsg->u32AllPhy) : (pstInMsg->u32AllPhy - pstInMsg->u32SavePhy);
    pstImgInfo->InputDataVirBuf[0] = (HI_CHAR*)(unsigned long)(pVirBuf + FirstBufOffset);
    pstImgInfo->InputDataVirBuf[1] = (HI_CHAR*)(unsigned long)pVirBuf;
	...
	(HI_VOID)HI_GFX_Unmap(pVirBuf,HI_TRUE);
