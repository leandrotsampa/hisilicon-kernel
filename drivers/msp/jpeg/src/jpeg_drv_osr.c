...
#define DRV_JPEG_IOCTLFUNC_ITEM_NUM_MAX	     22
...
static HI_S32 JPEG_DRV_SetDecBufUseState(HI_ULONG arg);
static HI_S32 JPEG_DRV_GetDecBufUseState(HI_ULONG arg);
#endif
...
static DRV_JPEG_IOCTLFUNC_ITEM gs_DrvJpegCtlFunc[DRV_JPEG_IOCTLFUNC_ITEM_NUM_MAX] = {
     {CMD_JPG_GETDEVICE, JPEG_OSR_GetDevice},	     {1,    NULL},
     {2,    NULL},				     {CMD_JPG_GETSUSPEND, JPEG_OSR_GetSuspend},
     ...
     {CMD_JPG_SET_LOWDELAY_LINE, JPEG_OSR_SetLowdelayLine},
#ifndef CONFIG_JPEG_PERFORMANCE_SUPPORT
     {CMD_JPG_SET_DECBUF_STATE,NULL},		     {CMD_JPG_GET_DECBUF_STATE,NULL},
#else
     {CMD_JPG_SET_DECBUF_STATE,JPEG_DRV_SetDecBufUseState}, {CMD_JPG_GET_DECBUF_STATE,JPEG_DRV_GetDecBufUseState},
#endif
...
HI_VOID DRV_JPEG_ResetReg(HI_VOID)
{
    ...
    /** only after 98mv200 support write register at first cancel and second off clock state **/
...
static HI_S32 DRV_JPEG_IsrFunction(HI_S32 irq, HI_VOID * devId, struct pt_regs * ptrReg)
{
...
#ifdef CONFIG_JPEG_PERFORMANCE_SUPPORT
    JPEG_DRV_SetDecBufUseState(HI_FALSE);
#endif
    ...
    {
    #ifdef CONFIG_JPEG_PERFORMANCE_SUPPORT
	JPEG_DRV_SetDecBufUseState(HI_TRUE);
    #endif
...
HI_S32 JPEG_DRV_GetIntStatusValue(JPG_GETINTTYPE_S *pstIntType)
{
    ...
    if ((0 == pstIntType->TimeOut) || (pstIntType->TimeOut > JPEG_INTTYPE_DELAY_TIME))
    {
	return HI_FAILURE;
    }
...
static HI_S32 JPEG_DRV_SetDecBufUseState(HI_ULONG arg)
{
    if (copy_from_user((HI_VOID *)&(s_pstruJpgOsrDev->bIsDecBufUsed), (HI_VOID *)arg, sizeof(HI_BOOL)))
    {
	s_pstruJpgOsrDev->bIsDecBufUsed = HI_FALSE;
	return -EFAULT;
    }
    return HI_SUCCESS;
}
...
static HI_S32 JPEG_DRV_GetDecBufUseState(HI_ULONG arg)
{
    if (copy_to_user((HI_VOID *)arg, (HI_VOID *)&(s_pstruJpgOsrDev->bIsDecBufUsed), sizeof(HI_BOOL)))
    {
	return -EFAULT;
    }
...
* description	: kernel 64bit and api 64bit call this function
		  CNcomment: �ں�̬���û�̬����64λ���øú��� CNend\n
* param[in]	: pFile	     CNcomment: �ļ�������     CNend\n
* param[in]	: cmd	     CNcomment: ����ĵ������� CNend\n
