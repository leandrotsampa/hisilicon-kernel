...
HI_S32 HpdEvent(HDMI_DEVICE_S* pstHdmiDev)
{
	...
	if( pstHdmiDev->stEdidInfo.stCapability.stHdcpSupport.bHdcp14Support
	    || pstHdmiDev->stEdidInfo.stCapability.stHdcpSupport.bHdcp22Support )
	{
	    break;
	}
	msleep(50);
...
HI_S32 DRV_HDMI_Open(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bUser)
{
	...
	*/
    ...
    if (!bHdmiOn)
    {
	 ...
	 /* report hpd event from hardware reset */
    }
    else
#endif
...
HI_S32 DRV_HDMI_AudioEnable(HDMI_DEVICE_S* pstHdmiDev,HDMI_AO_MODE_E enMode,HI_BOOL bEnable)
{
	...
	CHECK_POINTER(pstHdmiDev);
    CHECK_POINTER(pstHdmiDev->pstHdmiHal);
    CHECK_POINTER(pstHdmiDev->pstHdmiHal->HAL_HDMI_AudioEnable);
    CHECK_POINTER(pstHdmiDev->pstHdmiHal->HAL_HDMI_InfoframeEnableSet);
	...
	s32Ret |= pstHdmiDev->pstHdmiHal->HAL_HDMI_AudioEnable(pstHdmiDev->pstHdmiHal,pstAoAttr->enSoundIntf,bTmpEnable);
	bTmpEnable = pstAoEn->bAoAppEnable && pstAoEn->bAoDrvEnable && pstAoEn->bAoSelEanble && pstAoEn->bAoIfAppEanble;
	pstHdmiDev->pstHdmiHal->HAL_HDMI_InfoframeEnableSet(pstHdmiDev->pstHdmiHal,HDMI_INFOFRAME_TYPE_AUDIO,bTmpEnable);
...
static HI_S32 HdmiCmdProcess0(HI_U32 cmd, HI_VOID *arg, HI_BOOL bUser)
{
	...
	default:
	{
	    HDMI_INFO("cmd maybe in other process\n");
