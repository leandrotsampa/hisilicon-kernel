...
HI_VOID SYNC_PllTuneFreqCrgInit(HI_VOID)
{
    ...
    /* bpll tune crg init */
    CrgFbdiv = g_pstRegCrg->PERI_CRG110.bits.bpll_tune_int_cfg;
    CrgFrac  = g_pstRegCrg->PERI_CRG111.bits.bpll_tune_frac_cfg;

    if (CrgFbdiv == 0x0 && CrgFrac == 0x0)
    {
        CrgFbdiv = g_pstRegCrg->PERI_CRG_PLL3.u32;
        CrgFbdiv = CrgFbdiv & (0xfff);

        CrgFrac = g_pstRegCrg->PERI_CRG_PLL2.u32;
        CrgFrac = CrgFrac & (0xffffff);

        g_pstRegCrg->PERI_CRG110.bits.bpll_tune_int_cfg  = CrgFbdiv;
        g_pstRegCrg->PERI_CRG111.bits.bpll_tune_frac_cfg = CrgFrac;

        HI_INFO_SYNC("Bpll tune freq Crg init success !\n");
    }
...
HI_BOOL SYNC_CheckPllTuneFreqIsEnable(HI_VOID)
{
    ...
    BpllTuneFreqFlag = g_pstRegCrg->PERI_CRG_PLL3.u32;
    BpllTuneFreqFlag = (BpllTuneFreqFlag >> 25) & 0x01;
    ...
    if (VpllTuneFreqFlag || BpllTuneFreqFlag || EpllTuneFreqFlag || HpllTuneFreqFlag)
    {
...
HI_VOID SYNC_CalculateTargetFreq(HI_U32 *PllFbdiv, HI_U32 *PllFrac, HI_S32 PllDeltaFrac)
{
    ...
    else
    {
        ...
        {
            *PllFbdiv = *PllFbdiv - 1;
            *PllFrac  = *PllFrac + 0xffffff + PllDeltaFrac;
        }
...
HI_VOID SYNC_BpllTuneFreq(HI_U32 BpllFbdiv, HI_U32 BpllFrac)
{
    g_pstRegCrg->PERI_CRG110.bits.bpll_tune_int_cfg   = BpllFbdiv;
    g_pstRegCrg->PERI_CRG111.bits.bpll_tune_frac_cfg  = BpllFrac;
    g_pstRegCrg->PERI_CRG112.bits.bpll_tune_step_int  = 0;
    g_pstRegCrg->PERI_CRG113.bits.bpll_tune_step_frac = 0x10;

    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_en        = 0;        /* 3: 0->1 tune frq */
    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_cken      = 1;        /* 0: clock en */
    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_srst_req  = 0;        /* 1: sorft req */
    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_mode      = 1;        /* 2: tune mode */
    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_divval    = 1;        /* 4-7: tune clock div */
    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_soft_cfg  = 0;        /* 8: sorft config en */

    g_pstRegCrg->PERI_CRG114.bits.bpll_tune_en = 0x1;              /* 3: 0->1 tune frq */
}

HI_VOID SYNC_BpllFreqReset(HI_VOID)
{
    HI_U32 BpllFbdiv;
    HI_U32 BpllFrac;

    BpllFbdiv = g_pstRegCrg->PERI_CRG_PLL3.u32;
    BpllFbdiv = BpllFbdiv & (0xfff);

    BpllFrac = g_pstRegCrg->PERI_CRG_PLL2.u32;
    BpllFrac = BpllFrac & (0xffffff);

    SYNC_BpllTuneFreq(BpllFbdiv, BpllFrac);

    HI_INFO_SYNC("Bpll Freq Reset !\n");
}
...
HI_VOID SYNC_EpllFreqReset(HI_VOID)
{
    ...
    SYNC_BpllTuneFreq(EpllFbdiv, EpllFrac);
...
HI_VOID SYNC_HpllFreqReset(HI_VOID)
{
    ...
    SYNC_BpllTuneFreq(HpllFbdiv, HpllFrac);
...
HI_VOID SYNC_PllTuneFreq(SYNC_S *pSync, HI_S32 PPM)
{
    ...
    HI_U32 BpllTargetFbdiv;
    HI_U32 BpllTargetFrac;
    ...
    BpllTargetFbdiv = g_pstRegCrg->PERI_CRG110.bits.bpll_tune_int_cfg;
    BpllTargetFrac  = g_pstRegCrg->PERI_CRG111.bits.bpll_tune_frac_cfg;
    ...
    if (abs(BpllFbdiv - BpllTargetFbdiv) >= 2 )
    {
        g_pstRegCrg->PERI_CRG110.bits.bpll_tune_int_cfg  = BpllFbdiv;
        g_pstRegCrg->PERI_CRG111.bits.bpll_tune_frac_cfg = BpllFrac;

        BpllTargetFbdiv = BpllFbdiv;
        BpllTargetFrac  = BpllFrac;

        HI_WARN_SYNC("Bpll Fbdiv is changed !\n");
    }
    ...
    VpllAdjust = (VpllTargetFbdiv - VpllFbdiv) * 0xffffff + (VpllTargetFrac - VpllFrac);
    BpllAdjust = (BpllTargetFbdiv - BpllFbdiv) * 0xffffff + (BpllTargetFrac - BpllFrac);
    EpllAdjust = (EpllTargetFbdiv - EpllFbdiv) * 0xffffff + (EpllTargetFrac - EpllFrac);
    HpllAdjust = (HpllTargetFbdiv - HpllFbdiv) * 0xffffff + (HpllTargetFrac - HpllFrac);

    SYNC_CalculatePpm(VpllFbdiv, VpllFrac, VpllAdjust, &VpllPpm);
    SYNC_CalculatePpm(BpllFbdiv, BpllFrac, BpllAdjust, &BpllPpm);
    SYNC_CalculatePpm(EpllFbdiv, EpllFrac, EpllAdjust, &EpllPpm);
    SYNC_CalculatePpm(HpllFbdiv, HpllFrac, HpllAdjust, &HpllPpm);

    if (abs(VpllPpm - BpllPpm) > PPM_ADJUST_THRESHOLD)
    {
        BpllTargetFbdiv = BpllFbdiv;
        BpllTargetFrac  = BpllFrac;

        SYNC_CalculateDeltaFrac(BpllFbdiv, BpllFrac, VpllPpm, &BpllDeltaFrac);

        SYNC_CalculateTargetFreq(&BpllTargetFbdiv, &BpllTargetFrac, BpllDeltaFrac);

        BpllTurneBusy = g_pstRegCrg->PERI_CRG167.bits.bpll_tune_busy;
        if (BpllTurneBusy == 0)
        {
            SYNC_BpllTuneFreq(BpllTargetFbdiv, BpllTargetFrac);

            g_pstRegCrg->PERI_CRG110.bits.bpll_tune_int_cfg  = BpllTargetFbdiv;
            g_pstRegCrg->PERI_CRG111.bits.bpll_tune_frac_cfg = BpllTargetFrac;
        }

        HI_WARN_SYNC("VpllPpm and BpllPpm are not equal, VpllPpm:%d, BpllPpm:%d\n", VpllPpm, BpllPpm);
    }
    ...
    SYNC_CalculateDeltaFrac(BpllTargetFbdiv, BpllTargetFrac, PPM, &BpllDeltaFrac);
    ...
    if (!VpllTurneBusy && !BpllTurneBusy && !EpllTurneBusy && !HpllTurneBusy)
    {
