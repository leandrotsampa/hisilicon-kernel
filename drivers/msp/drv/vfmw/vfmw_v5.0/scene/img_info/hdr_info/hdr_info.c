...
static VOID HDR_ReadOutputHDRInfo(struct seq_file *p, HDR_OUTPUT_S *pstHDROutput)
{
    ...
    //PROC_PRINT(p, "%-35s : %d\n", "TransferCharacteristics", pstHDROutput->stColourInfo.TransferCharacteristics);
    ...
    switch (pstHDROutput->enSrcFrameType)
    {
        ...
        case HI_DRV_VIDEO_FRAME_TYPE_HLG:
            PROC_PRINT(p, "%-35s : %d\n", "u8BackWardsCompatible", pstHDROutput->unHDRInfo.stHLGInfo.IsBackwardsCompatible);
            break;
