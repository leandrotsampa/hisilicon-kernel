...
Gfd3kl4mhQ5PKaW8bqfJj0ceB5nhd8JH623aHkEj9jo8/OyxSH/AmiV/Use96f0njT6vrP3C
QpLOAuXAlUZO1A4wzCOPjDCW1XIKJ/1R0RP9RuiC1+PyMAjXcCkb5BBPbxe6pQ==#*/
...
SINT32 vfmw_read_proc_chn(struct seq_file *p, VOID *v)
{
    ...
    seq_printf(p, "%-40s :%d\n",     "Compress",             pChan->stSynExtraData.s32Compress_en);
    ...
    for (VdhId = 0; VdhId < MAX_VDH_NUM; VdhId++)
    {
        pVfmwGlobalStat = &g_VfmwGlobalStat[VdhId];
        ...
        {
            VdhCycle = pVfmwGlobalStat->u32VdhKiloCyclePerSecond * 1000 / VdhFps / (pFsp->stInstCfg.s32DecFsWidth / 16) / (pFsp->stInstCfg.s32DecFsHeight / 16);
        }
        else
        {
            VdhCycle = 0;
        }
        seq_printf(p, "%s%-21d :%d.%d%%\n", "VDH ",   VdhId, VdhLoad, pVfmwGlobalStat->u32VdhLoad % 10);
...
SINT32 vfmw_read_proc_buf(struct seq_file *p, VOID *v)
{
    ...
    {
        IsFree = FSP_IsPmvAvailable(&pFsp->pmvRec[i]);
        TotalFree += IsFree;

        seq_printf(p, "%-10d %-10d %-10d 0x%-10x %s\n", i,
                   pFsp->pmvRec[i].isValid, pFsp->pmvRec[i].isInUse,
                   pFsp->pmvRec[i].pmvAddr, ((IsFree)? "(Free)": ""));
    }
    ...
    for (i = 0; i < MAX_FRAME_NUM; i++)
    {
        ...
        seq_printf(p, "%d%d ", pFsp->stLogicFs[i].s32IsRef, pFsp->stLogicFs[i].s32DispState);
    }
    seq_printf(p, "\n\n");
