...
HI_BOOL SOS_VFMW_CheckUUID(HI_VOID)
{
    ...
    s32Ret = SRE_TaskSelfUUID(&stCurUuid);
    if (HI_SUCCESS == s32Ret)
    {
        if (0 == memcmp(&stCurUuid, &stValidUuid, sizeof(stValidUuid)))
        {
            isValid = HI_TRUE;
        }
    }
...
/* Secure IRQ                                                           */
/************************************************************************/
SINT32 SOS_Request_Irq(UINT32 IrqNum, HWI_PROC_FUNC pfnHandler, SINT32 flags, const char *name, VOID *dev)
{
    UINT32 ret;
...
SINT32 InitTrustedVfmwInterface(VOID)
{
    ...
    vfmw_Osal_Func_Ptr_S.pfun_Osal_MemCpy = asm_memcpy;
