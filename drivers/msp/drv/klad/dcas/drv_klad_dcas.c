...
static HI_S32 DRV_KLAD_DCAS_SetK30Key(HI_HANDLE handle, DRV_KLAD_SESSION* session)
{
    ...
    HAL_KLAD_DCAS_K30Start(level-1, (HI_U8)alg, addr, (HI_U8)session->content_key.odd, (HI_U8)engine, (HI_U8)target);
...
static HI_S32 DRV_KLAD_DCAS_SetK31Key(HI_HANDLE handle, DRV_KLAD_SESSION *session)
{
    HI_S32 ret = HI_FAILURE;
    HI_U8 addr = 0x0;
    HI_U8 level = 0;
    ...
    level = HAL_KLAD_DCAS_K31GetLevel();
    DRV_KLAD_CHECK_LEVEL(level);
    ...
    for (index = 0; index < level-1; index++)
    {
        ...
        HAL_KLAD_SetDataIn(session->session_key[index].key);
        HAL_KLAD_DCAS_K31Start((HI_U8)index, (HI_U8)alg, addr, 0, (HI_U8)engine, (HI_U8)target);
    ...
    HAL_KLAD_SetDataIn(session->content_key.key);
    HAL_KLAD_DCAS_K31Start(level-1, (HI_U8)alg, addr, (HI_U8)session->content_key.odd, (HI_U8)engine, (HI_U8)target);
...
static HI_S32 DRV_KLAD_DCAS_Crypto(HI_HANDLE handle, DRV_KLAD_SESSION* session)
{
    ...
    ca_type = HI_KLAD_GET_CATYPE(handle);
    klad_type = HI_KLAD_GET_KLADTYPE(handle);
    klad = HI_KLAD_GET_UNF_KLADTYPE(ca_type, klad_type);
    ...
    switch (klad)
    {
        ...
        case HI_UNF_KLAD_DCAS_TYPE_K31:
        {
            ret = DRV_KLAD_DCAS_SetK31Key(handle, session);
            break;
        }
...
static HI_S32 DRV_KLAD_DCAS_StartRKD(HI_VOID* arg)
{
    HI_S32 ret = HI_FAILURE;
    KLAD_CTL_RKD_S *ctl_rkd;

    DRV_KLAD_CHECK_NULL_PTR(arg);

    ctl_rkd = (KLAD_CTL_RKD_S*)arg;

    if (ctl_rkd->enMode >= HI_UNF_KLAD_DCAS_PROFILE_BUTT)
    {
        HI_ERR_KLAD("invalid profile mode %#x\n",  ctl_rkd->enMode);
        return HI_ERR_KLAD_INVALID_PARA;
    }
    if (ctl_rkd->enModuleID >= HI_UNF_KLAD_DCAS_MODULE_ID_BUTT)
    {
        HI_ERR_KLAD("invalid module id %#x\n",  ctl_rkd->enModuleID);
        return HI_ERR_KLAD_INVALID_PARA;
    }

    HAL_KLAD_DCAS_SetModuleId(ctl_rkd->au8Data);
    HAL_KLAD_DCAS_RKDStart((HI_U8)ctl_rkd->enMode, (HI_U8)ctl_rkd->enModuleID);
...
static HI_S32 DRV_KLAD_DCAS_K30GenDaNonce(HI_VOID* arg)
{
    ...
    if (HI_TRUE != session->sessiong_key_flag[level])
    {
        HI_ERR_KLAD("Not set level%d session key to handle %#x\n", level + 1, session->handle);
        return HI_ERR_KLAD_NOT_SET_ATTR;
    }
    HAL_KLAD_SetDataIn(session->session_key[level].key);
    HAL_KLAD_DCAS_K30Start((HI_U8)level, (HI_U8)alg, 0, 0, 0, 0);
    ret = DRV_KLAD_DCAS_CheckHalError();
    if (HI_SUCCESS != ret)
    {
        return ret;
...
static HI_S32 DRV_KLAD_DCAS_K31GenDaNonce(HI_VOID* arg)
{
    HI_S32 ret = HI_FAILURE;
    HI_UNF_KLAD_ALG_TYPE_E alg = HI_UNF_KLAD_ALG_TYPE_BUTT;
    DRV_KLAD_SESSION *session = HI_NULL;
    ...
    HAL_KLAD_SetDataIn(session->session_key[level].key);
    HAL_KLAD_DCAS_K31Start((HI_U8)level, (HI_U8)alg, 0, 0, 0, 0);
    ...
    state = HAL_KLAD_DCAS_K31GetDaNonceState();
    if (state != 0x1)
    {
        HI_ERR_KLAD("K3_0 DANonce not ready\n");
        return HI_ERR_KLAD_KEY_NOT_READY;
    ...
    HAL_KLAD_DCAS_K31GetDaNonce(da_nonce->da_nonce);
...
static DRV_KLAD_IOCTL_S g_klad_dcas_ioctl[] =
{
    ...
    {CMD_KLAD_STRAT_RKD,        DRV_KLAD_DCAS_StartRKD},
    {CMD_KLAD_K30_GEN_DANONCE,  DRV_KLAD_DCAS_K30GenDaNonce},
    {CMD_KLAD_K31_GEN_DANONCE,  DRV_KLAD_DCAS_K31GenDaNonce},
};
