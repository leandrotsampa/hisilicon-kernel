...
static HI_VOID DEV_KLAD_PROC_DumpReg(struct seq_file* p, HI_VOID* v)
{
    HI_U32 off = 0, i = 0, reg = 0;
    DRV_KLAD_PROC_PRINT("====dump memory 0X%08x====", CA_KLAD_BASE_ADDR);

    for (off = 0; off < 0x1000 - 4; off += 0x10) /*over follow*/
    {
        DRV_KLAD_PROC_PRINT("\n%04x: ", off);

        for (i = 0; i < 4; i ++)
        {
            reg = HAL_KLAD_ReadReg(CA_KLAD_BASE_ADDR + off + i);
            DRV_KLAD_PROC_PRINT(" %08x", reg);
        }
    }
