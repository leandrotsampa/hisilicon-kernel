...
static HI_S32 DRV_KLAD_SESSION_Create(HI_U32 klad, HI_HANDLE* create_handle)
{
    ...
    {
        create = HI_KLAD_MAKEHANDLE(HI_ID_CA, ca_type, klad_type, i);
...
static HI_S32 SetIVToCipher(HI_HANDLE hCipher, DRV_KLAD_CONTENT_IV_S *pstIV)
{
    ...
    static CIPHER_EXPORT_FUNC_S *g_pCipherFunc = HI_NULL;

    KLAD_FUNC_ENTER();

    ret = HI_DRV_MODULE_GetFunction(HI_ID_CIPHER, (HI_VOID**)&g_pCipherFunc);
    if (HI_SUCCESS != ret)
    {
    ...
    if (g_pCipherFunc && g_pCipherFunc->pfnCipherSetIV)
    {
        ret = (g_pCipherFunc->pfnCipherSetIV)(hCipher, pstIV->iv, pstIV->length);
    }
...
static HI_S32 SetIVToDemux(HI_HANDLE hDemux, DRV_KLAD_CONTENT_IV_S *pstIV)
{
    ...
    static DEMUX_EXPORT_FUNC_S *g_pDmxFunc = HI_NULL;

    KLAD_FUNC_ENTER();

    ret = HI_DRV_MODULE_GetFunction(HI_ID_DEMUX, (HI_VOID**)&g_pDmxFunc);
    if (HI_SUCCESS != ret)
    {
    ...
    if (g_pDmxFunc && g_pDmxFunc->pfnDmxSetDescramblerKey)
    {
        ret = (g_pDmxFunc->pfnDmxSetDescramblerIV)(hDemux, type, pstIV->iv, pstIV->length);
    }
...
static HI_U32 get_ca_type(unsigned int cmd, void* arg)
{
