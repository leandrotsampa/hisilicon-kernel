...
HI_S32 VI_PHY_Create(HI_HANDLE hVi, const HI_UNF_VI_ATTR_S* pstAttr)
{
    ...
    if (0 != osal_request_irq(VI_INT_NUM, VI_PHY_InterruptHandler, IRQF_SHARED, "vi", HI_NULL))
    {
...
{
    osal_free_irq(VI_INT_NUM, "vi",  HI_NULL);
    VI_PHY_BoardDeinit();
}
...
irqreturn_t VI_PHY_InterruptHandler(int irq, void* dev_id)
{
    ...
    if ((u32ChInt & VI_CC_INT) == VI_CC_INT)
    {
        ...
        {
            Ret = VI_DRV_BufPut(&g_ViDrv[enPort][u32Chn].stFrameBuf, HI_NULL, 0);
