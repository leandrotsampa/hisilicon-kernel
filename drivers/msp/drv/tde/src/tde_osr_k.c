...
HI_VOID HI_TDE_MODULE_GetFunction( TDE_EXPORT_FUNC_S **ppFunc)
{
...
HI_S32 tde_init_module_k(HI_VOID)
{
    ...
    if (0 != osal_request_irq(TDE_INTNUM, (irq_handler_t)tde_osr_isr,
                         IRQF_PROBE_SHARED, "tde", NULL))
    {
...
#else
    if (0 != osal_request_irq(TDE_INTNUM, (irq_handler_t)tde_osr_isr,
                         IRQF_SHARED, "tde", &g_TdeIrqHandle))
    {
        TDE_ERROR("osal_request_irq for TDE failure!\n");
        TdeHalRelease();
        return -1;
