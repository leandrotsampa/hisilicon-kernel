...
HI_BOOL gIsSliceInfo = 1;
HI_BOOL gIsAcqureOver = 1;
extern HI_BOOL gIsOnePicEnd;
#endif
...
HI_VOID VENC_DRV_BoardInit(HI_VOID)
{
    ...
    /* 98cv200_a/b: 0:500Mkz, 01:DPLL postdiv\CA\E4\B3\F6\A3\AC466/533MHz\A3\BB10:\B8\FAmde1 axi\D7\DC\CF\DFʱ\D6\D3ͬƵ\A3\BB11:\B1\A3\C1\F4*/
    ...
    unTmpValue.bits.venc_clk_sel  = 0;   //ѡ\D3\C3Ĭ\C8\CFƵ\C2\CA
#endif
...
#ifdef VENC_96MV200_CONFIG
    pstVeInfo->u32SliceBufPhyAddr = pstEncChnPara->stHal.SliceBufAddr;
    g_stVencChn[i].SliceBufAddr  = pstEncChnPara->stHal.SliceBufAddr;
    g_stVencChn[i].SliceBufSize  = pstEncChnPara->stHal.SliceBufSize;
#endif
...
#ifdef VENC_98MV200_CONFIG
static HI_UNF_HEVCE_NALU_TYPE_E TransNaluTypeNumForH265(HI_U32 NaluType)
{
                              ...
                              VENC_BUF_OFFSET_S* pstBufOffSet)
{
    ...
    if (gIsSliceInfo)
    {
        pstStream->VirAddr      = (HI_VIRT_ADDR_T)hVencChn->stHal.SliceBuf.pu8StartVirAddr;
        pstStream->u32SlcLen    = hVencChn->stHal.HeaderRealLength;

        if (!pstStream->u32SlcLen)
        {
            return HI_FAILURE;
        }

        pstStream->u32PtsMs  = 0;
        pstStream->bFrameEnd = HI_FALSE ;
        pstStream->enDataType.enHEVCEType = 0;
        pstBufOffSet->u32StrmBufOffset[0] = 0;
    }
    else
    {
        pstStream->VirAddr      = (HI_VIRT_ADDR_T)hVencChn->StreamMMZBuf.pu8StartVirAddr + VEDU_MMZ_ALIGN;
        pstStream->u32SlcLen    = hVencChn->stHal.VEDU_SLICE_LENGTH[0];
        pstStream->u32PtsMs     = 0;
...
HI_S32 VENC_DRV_ReleaseStream(VeduEfl_EncPara_S* hVencChn, VENC_INFO_ACQUIRE_STREAM_S* pstAcqStrm)
{
    ...
    if (gIsSliceInfo)
    {
        pstStream.VirAddr  = (HI_VIRT_ADDR_T)(pstEncChnPara->stCycBuf.pBase);

        gIsSliceInfo = 0;
    }
    else
    {
        pstStream.VirAddr  = (HI_VIRT_ADDR_T)(pstEncChnPara->stHal.SliceBuf.pu8StartVirAddr);
        gIsAcqureOver = 1;
    }
                              ...
                              VENC_BUF_OFFSET_S* pstBufOffSet)
{
...
#endif
    {
        ...
        if (pstEncChnPara->Protocol == VEDU_H265)
        {
            pstStream->enDataType.enHEVCEType = TransNaluTypeNumForH265(stVeduPacket.NaluType);
        }
        else
        {
                                       ...
                                       VeduEfl_EncPara_S* EncHandle, OPTM_VENC_CHN_S* pstVenc)
{
    ...
    if ((pstAttr->enVencType == HI_UNF_VCODEC_TYPE_H264) || (pstAttr->enVencType == HI_UNF_VCODEC_TYPE_HEVC)) //c00346155
    {
    ...
    else  /* jpeg*/
    {
        ...
        EncHandle->stRc.VoFrmRate = pstAttr->u32TargetFrmRate;
        EncHandle->stRc.ViFrmRate = pstAttr->u32InputFrmRate;
...
HI_S32 VENC_DRV_SetAttr(VeduEfl_EncPara_S* EncHandle, HI_UNF_VENC_CHN_ATTR_S* pstAttr, VENC_CHN_INFO_S* pstVeInfo)
{
    ...
    if ((pstAttr->enVencType == HI_UNF_VCODEC_TYPE_H264) && (pstVenc->stChnUserCfg.enVencProfile != pstAttr->enVencProfile))
    {
        ...
        pstEncChnPara->stHal.BaseCfg.vcpi_entropy_mode = !pstEncChnPara->H264CabacEn;
        pstEncChnPara->stHal.BaseCfg.vcpi_trans_mode   = (pstEncChnPara->H264HpEn == VEDU_H264_HIGH_PROFILE ? 0 : 1);
        pstEncChnPara->stHal.BaseCfg.vcpi_intra_cu_en  = (pstEncChnPara->H264HpEn == VEDU_H264_HIGH_PROFILE ? 7 : 5);
...
HI_S32 VENC_DRV_QueueFrame(VeduEfl_EncPara_S* hVencChn, HI_UNF_VIDEO_FRAME_INFO_S* pstFrameInfo )
{
    ...
    {
        if (HI_ID_BUTT == g_stVencChn[u32VeChn].enSrcModId)   //\C8\E7\B9\FBδ\B0\F3\B6\A8\D4\F2\C9\E8\D6\C3
        {
            pstEncChnPara->stSrcInfo.pfGetImage = VENC_DRV_EflGetImage;
            pstEncChnPara->stSrcInfo.pfPutImage = VENC_DRV_EflPutImage;
        }
