...
static void VENC_DRV_ProcWrite_PrintRCRatio(HI_CHAR* str, VeduEfl_EncPara_S* hVenc)
{
    VeduEfl_EncPara_S* pEncPara  = (VeduEfl_EncPara_S*)hVenc;
...
static HI_S32 VENC_DRV_ProcWrite(struct file* file, const char __user* buffer, size_t count, loff_t* ppos)
{
    ...
    if (count >= 1)
    {
        ...
        else if (!HI_OSAL_Strncmp(str1, "rcratioprint", 256))
        {
            VENC_DRV_ProcWrite_PrintRCRatio(str2, hVenc);
        }
        else
        {
            VENC_DRV_ProcHelp();
