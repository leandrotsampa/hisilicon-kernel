...
HI_S32 DRV_CIPHER_CalcRsa_SW(CIPHER_RSA_DATA_S *pCipherRsaData)
{
    ...
    if( 0 != ret )
    {
        HI_ERR_CIPHER("mbedtls_mpi_read_binary error!\n");
        ret = HI_FAILURE;
    }
    ...
    if ((pCipherRsaData->pu8K != HI_NULL) && (pCipherRsaData->u16KLen > 0))
    {
        ...
        if( 0 != ret )
        {
            HI_ERR_CIPHER("mbedtls_mpi_read_binary error, ret = %d!\n", ret);
            ret = HI_FAILURE;
        }
        ...
        if( 0 != ret )
        {
            HI_ERR_CIPHER("RSA public error, ret = %d!\n", ret);
            ret = HI_FAILURE;
        }
    }
    else
...
#if 0
    HI_PRINT_HEX ("C", pCipherRsaData->pu8Output, pCipherRsaData->u32DataLen);
#endif
