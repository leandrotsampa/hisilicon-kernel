...
HI_S32 DRV_CIPHER_ConfigChn(HI_U32 softChnId,  HI_UNF_CIPHER_CTRL_S *pConfig)
{
    ...
    bIVSet = pConfig->stChangeFlags.bit1IV ? HI_TRUE : HI_FALSE;
...
HI_S32 HI_DRV_CIPHER_SetIV(HI_HANDLE hCIHandle,  HI_U8 *pu8IV, HI_U32 u32IVLen)
{
    ...
    pSoftChan = &g_stCipherSoftChans[softChnId];
    memcpy(pSoftChan->stCtrl.u32IV, pu8IV, u32IVLen);
    pSoftChan->bIVChange = 1;
...
*/
     ...
     */
#ifdef HI_TEE_SUPPORT
    ret = osal_request_irq(CIPHER_IRQ_NUMBER, DRV_Cipher_ISR, IRQF_SHARED, "multicipher", &g_stCipherComm);
#else
...
HI_VOID DRV_CIPHER_DeInit(HI_VOID)
{
    ...
    /* free irq */
#ifdef HI_TEE_SUPPORT
    osal_free_irq(CIPHER_IRQ_NUMBER, "multicipher", &g_stCipherComm);
#else
