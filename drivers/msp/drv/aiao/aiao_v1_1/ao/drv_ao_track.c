...
{
    SND_TRACK_STATE_S* pTrack = HI_NULL;

    if (u32TrackID >= AO_MAX_TOTAL_TRACK_NUM)
    {
        HI_ERR_AO("Invalid track ID!\n");
        return HI_FAILURE;
    }
    ...
    {
#ifdef DRV_SND_MULTICH_AEF_SUPPORT
        HAL_AOE_AIP_SetDataSource(pTrack->enAIP[SND_AIP_TYPE_MC_PCM], HI_TRUE);
#endif
#ifdef DRV_SND_AD_OUTPUT_SUPPORT
        HAL_AOE_AIP_SetDataSource(pTrack->enAIP[SND_AIP_TYPE_ASSOC_PCM], HI_TRUE);
#endif
