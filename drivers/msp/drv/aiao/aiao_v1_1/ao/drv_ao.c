...
{
#ifndef HI_SND_DSP_SUPPORT  //sw aoe
...
static HI_S32 AO_IOCTL_SndOpen(struct file* file, HI_VOID* arg)
{
    ...
    if (HI_SUCCESS == s32Ret)
    {
        if (0 == atomic_read(&s_stAoDrv.astSndEntity[enSound].atmUseTotalCnt))
        {
            s32Ret = AO_SND_Open(enSound, &pstParam->stAttr, HI_NULL, HI_FALSE);
            if (HI_SUCCESS != s32Ret)
            {
                AO_Snd_FreeHandle(enSound, file);
                return s32Ret;
...
/* ao ioctl end */
typedef HI_S32 (*AO_IOCTL_FN)(struct file* file, HI_VOID* arg);

static const AO_IOCTL_FN g_fnAoIoctl[CMD_ID_AO_BUTT] =
{
    [CMD_ID_AO_SND_GETDEFOPENATTR]       = AO_IOCTL_GetSndDefOpenAttr,
    [CMD_ID_AO_SND_OPEN]                 = AO_IOCTL_SndOpen,
    [CMD_ID_AO_SND_CLOSE]                = AO_IOCTL_SndClose,
    [CMD_ID_AO_SND_SETMUTE]              = AO_IOCTL_SndSetMute,
    [CMD_ID_AO_SND_GETMUTE]              = AO_IOCTL_SndGetMute,
    [CMD_ID_AO_SND_SETVOLUME]            = AO_IOCTL_SndSetVolume,
    [CMD_ID_AO_SND_GETVOLUME]            = AO_IOCTL_SndGetVolume,

    [CMD_ID_AO_SND_SETHDMIMODE]          = AO_IOCTL_SndSetHdmiMode,
    [CMD_ID_AO_SND_GETHDMIMODE]          = AO_IOCTL_SndGetHdmiMode,

    [CMD_ID_AO_SND_SETSPDIFMODE]         = AO_IOCTL_SndSetSpdifMode,
    [CMD_ID_AO_SND_GETSPDIFMODE]         = AO_IOCTL_SndGetSpdifMode,

    [CMD_ID_AO_SND_SETSAMPLERATE]        = AO_IOCTL_SndSetSampleRate,
    [CMD_ID_AO_SND_GETSAMPLERATE]        = AO_IOCTL_SndGetSampleRate,

    [CMD_ID_AO_SND_SETTRACKMODE]         = AO_IOCTL_SndSetTrackMode,
    [CMD_ID_AO_SND_GETTRACKMODE]         = AO_IOCTL_SndGetTrackMode,

    [CMD_ID_AO_SND_SETSMARTVOLUME]       = AO_IOCTL_SndSetSmartVolume,
    [CMD_ID_AO_SND_GETSMARTVOLUME]       = AO_IOCTL_SndGetSmartVolume,

    [CMD_ID_AO_SND_SETSPDIFSCMSMODE]     = AO_IOCTL_SndSetSpdifSCMSMode,
    [CMD_ID_AO_SND_GETSPDIFSCMSMODE]     = AO_IOCTL_SndGetSpdifSCMSMode,
    [CMD_ID_AO_SND_SETSPDIFCATEGORYCODE] = AO_IOCTL_SndSetSpdifCategoryCode,
    [CMD_ID_AO_SND_GETSPDIFCATEGORYCODE] = AO_IOCTL_SndGetSpdifCategoryCode,

    [CMD_ID_AO_SND_SETALLTRACKMUTE]      = AO_IOCTL_SndSetAllTrackMute,
    [CMD_ID_AO_SND_GETALLTRACKMUTE]      = AO_IOCTL_SndGetAllTrackMute,

    [CMD_ID_AO_SND_SETADACENABLE]        = AO_IOCTL_SndSetAdacEnable,
    [CMD_ID_AO_SND_GETADACENABLE]        = AO_IOCTL_SndGetAdacEnable,

    [CMD_ID_AO_SND_ATTACHTRACK]          = HI_NULL,
    [CMD_ID_AO_SND_DETACHTRACK]          = HI_NULL,

#ifdef __DPT__
    [CMD_ID_AO_SND_SETPRECIVOL]          = AO_IOCTL_SndSetPreciVolume,
    [CMD_ID_AO_SND_GETPRECIVOL]          = AO_IOCTL_SndGetPreciVolume,
    [CMD_ID_AO_SND_SETALSAPRESCALE]      = AO_IOCTL_SndSetAlsaPrescale,

    [CMD_ID_AO_SND_SETBALANCE]           = AO_IOCTL_SndSetBalance,
    [CMD_ID_AO_SND_GETBALANCE]           = AO_IOCTL_SndGetBalance,

    [CMD_ID_AO_SND_SETARCENABLE]         = AO_IOCTL_SndSetArcEnable,
    [CMD_ID_AO_SND_GETARCENABLE]         = AO_IOCTL_SndGetArcEnable,
    [CMD_ID_AO_SND_SETARCMODE]           = AO_IOCTL_SndSetArcMode,
    [CMD_ID_AO_SND_GETARCMODE]           = AO_IOCTL_SndGetArcMode,
    [CMD_ID_AO_SND_SETARCCAP]            = AO_IOCTL_SndSetArcCap,
    [CMD_ID_AO_SND_GETARCCAP]            = AO_IOCTL_SndGetArcCap,

    [CMD_ID_AO_SND_SETAVCATTR]           = AO_IOCTL_SndSetAvcAttr,
    [CMD_ID_AO_SND_GETAVCATTR]           = AO_IOCTL_SndGetAvcAttr,
    [CMD_ID_AO_SND_SETAVCENABLE]         = AO_IOCTL_SndSetAvcEnable,
    [CMD_ID_AO_SND_GETAVCENABLE]         = AO_IOCTL_SndGetAvcEnable,

    [CMD_ID_AO_SND_SETGEQATTR]           = AO_IOCTL_SndSetGeqAttr,
    [CMD_ID_AO_SND_GETGEQATTR]           = AO_IOCTL_SndGetGeqAttr,
    [CMD_ID_AO_SND_SETGEQENABLE]         = AO_IOCTL_SndSetGeqEnable,
    [CMD_ID_AO_SND_GETGEQENABLE]         = AO_IOCTL_SndGetGeqEnable,
    [CMD_ID_AO_SND_SETGEQGAIN]           = AO_IOCTL_SndSetGeqGain,
    [CMD_ID_AO_SND_GETGEQGAIN]           = AO_IOCTL_SndGetGeqGain,

    [CMD_ID_AO_SND_SETDRCENABLE]         = AO_IOCTL_SndSetDrcEnable,
    [CMD_ID_AO_SND_GETDRCENABLE]         = AO_IOCTL_SndGetDrcEnable,
    [CMD_ID_AO_SND_SETDRCATTR]           = AO_IOCTL_SndSetDrcAttr,
    [CMD_ID_AO_SND_GETDRCATTR]           = AO_IOCTL_SndGetDrcAttr,

    [CMD_ID_AO_SND_SETPEQATTR]           = AO_IOCTL_SndSetPeqAttr,
    [CMD_ID_AO_SND_GETPEQATTR]           = AO_IOCTL_SndGetPeqAttr,
    [CMD_ID_AO_SND_SETPEQENABLE]         = AO_IOCTL_SndSetPeqEnable,
    [CMD_ID_AO_SND_GETPEQENABLE]         = AO_IOCTL_SndGetPeqEnable,

    [CMD_ID_AO_SND_ATTACHAEF]            = AO_IOCTL_SndAttachAef,
    [CMD_ID_AO_SND_DETACHAEF]            = AO_IOCTL_SndDetachAef,
    [CMD_ID_AO_SND_SETAEFBYPASS]         = AO_IOCTL_SndSetAefBypass,
    [CMD_ID_AO_SND_GETAEFBYPASS]         = AO_IOCTL_SndGetAefBypass,

    [CMD_ID_AO_SND_GETTRACKINFO]         = AO_IOCTL_SndGetTrackInfo,

    [CMD_ID_AO_SND_GETAEFBUFATTR]        = AO_IOCTL_SndGetAefBufAttr,
    [CMD_ID_AO_SND_GETDEBUGPARAM]        = AO_IOCTL_SndGetDebugAttr,

    [CMD_ID_AO_SND_SETADOUTPUTENABLE]    = AO_IOCTL_SndSetADOutputEnable,
    [CMD_ID_AO_SND_GETADOUTPUTENABLE]    = AO_IOCTL_SndGetADOutputEnable,
    [CMD_ID_AO_SND_DUPLICATETRACK]       = AO_IOCTL_SndTrackDuplicate,

    [CMD_ID_AO_SND_SETALLCASTMUTE]       = AO_IOCTL_SndSetAllCastMute,
    [CMD_ID_AO_SND_GETALLCASTMUTE]       = AO_IOCTL_SndGetAllCastMute,

    [CMD_ID_AO_SND_TRACKCONFIGINIT]      = AO_IOCTL_SndTrackConfigInit,
#endif

    [CMD_ID_AO_SND_GETXRUNCOUNT]         = AO_IOCTL_SndGetXRunCount,

    [CMD_ID_AO_SND_SETSOUNDDELAY]        = AO_IOCTL_SndSetDelayCompensation,
    [CMD_ID_AO_SND_GETSOUNDDELAY]        = AO_IOCTL_SndGetDelayCompensation,

    [CMD_ID_AO_SND_SETLOWLATENCY]        = AO_IOCTL_SndSetLowLatency,
    [CMD_ID_AO_SND_GETLOWLATENCY]        = AO_IOCTL_SndGetLowLatency,

    [CMD_ID_AO_SND_SETEXTDELAYMS]        = AO_IOCTL_SndSetExtDelayMs,

    [CMD_ID_AO_SND_DMACREATE]            = AO_IOCTL_SndDMACreate,
    [CMD_ID_AO_SND_DMADESTORY]           = AO_IOCTL_SndDMADestory,
    [CMD_ID_AO_SND_DMASETSAMPLERATE]     = AO_IOCTL_SndDMAModeSetSampleRate,

    /* AO Track ioctl */
    [CMD_ID_AO_TRACK_GETDEFATTR]         = AO_IOCTL_TrackGetDefAttr,
    [CMD_ID_AO_TRACK_CREATE]             = AO_IOCTL_TrackCreate,
    [CMD_ID_AO_TRACK_DESTROY]            = AO_IOCTL_TrackDestory,

    [CMD_ID_AO_TRACK_START]              = AO_IOCTL_TrackStart,
    [CMD_ID_AO_TRACK_STOP]               = AO_IOCTL_TrackStop,
    [CMD_ID_AO_TRACK_PAUSE]              = AO_IOCTL_TrackPause,
    [CMD_ID_AO_TRACK_FLUSH]              = AO_IOCTL_TrackFlush,

    [CMD_ID_AO_TRACK_SENDDATA]           = AO_IOCTL_TrackSendData,
    [CMD_ID_AO_TRACK_SETWEITHT]          = AO_IOCTL_TrackSetWeight,
    [CMD_ID_AO_TRACK_GETWEITHT]          = AO_IOCTL_TrackGetWeight,
    [CMD_ID_AO_TRACK_SETSPEEDADJUST]     = AO_IOCTL_TrackSetSpeedAdjust,
    [CMD_ID_AO_TRACK_GETDELAYMS]         = AO_IOCTL_TrackGetDelayMs,
    [CMD_ID_AO_TRACK_ISBUFEMPTY]         = AO_IOCTL_TrackIsBufEmpty,
    [CMD_ID_AO_TRACK_SETEOSFLAG]         = AO_IOCTL_TrackSetEosFlag,

    [CMD_ID_AO_TRACK_GETATTR]            = AO_IOCTL_TrackGetAttr,
    [CMD_ID_AO_TRACK_SETATTR]            = AO_IOCTL_TrackSetAttr,

    [CMD_ID_AO_TRACK_ATTACHAI]           = AO_IOCTL_TrackAttachAi,
    [CMD_ID_AO_TRACK_DETACHAI]           = AO_IOCTL_TrackDetachAi,
    [CMD_ID_AO_TRACK_SETABSGAIN]         = AO_IOCTL_TrackSetAbsGain,
    [CMD_ID_AO_TRACK_GETABSGAIN]         = AO_IOCTL_TrackGetAbsGain,

    [CMD_ID_AO_TRACK_SETMUTE]            = AO_IOCTL_TrackSetMute,
    [CMD_ID_AO_TRACK_GETMUTE]            = AO_IOCTL_TrackGetMute,
    [CMD_ID_AO_TRACK_SETCHANNELMODE]     = AO_IOCTL_TrackSetChannelMode,
    [CMD_ID_AO_TRACK_GETCHANNELMODE]     = AO_IOCTL_TrackGetChannelMode,

#ifdef __DPT__
    [CMD_ID_AO_TRACK_SETPRESCALE]        = AO_IOCTL_TrackSetPrescale,
    [CMD_ID_AO_TRACK_GETPRESCALE]        = AO_IOCTL_TrackGetPrescale,

    [CMD_ID_AO_TRACK_SETSOURCE]          = AO_IOCTL_TrackSetSource,
#endif

    [CMD_ID_AO_TRACK_SETFIFOBYPASS]      = AO_IOCTL_TrackSetFiFoBypass,
    [CMD_ID_AO_TRACK_MMAP]               = AO_IOCTL_TrackMmap,
    [CMD_ID_AO_TRACK_SETPRIORITY]        = AO_IOCTL_TrackSetPriority,
    [CMD_ID_AO_TRACK_GETPRIORITY]        = AO_IOCTL_TrackGetPriority,

    /* AO Cast ioctl */
    [CMD_ID_AO_CAST_GETDEFATTR]          = AO_IOCTL_CastGetDefAttr,
    [CMD_ID_AO_CAST_CREATE]              = AO_IOCTL_CastCreate,
    [CMD_ID_AO_CAST_DESTROY]             = AO_IOCTL_CastDestory,
    [CMD_ID_AO_CAST_SETENABLE]           = AO_IOCTL_CastSetEnable,
    [CMD_ID_AO_CAST_GETENABLE]           = AO_IOCTL_CastGetEnable,
    [CMD_ID_AO_CAST_GETINFO]             = AO_IOCTL_CastGetInfo,
    [CMD_ID_AO_CAST_SETINFO]             = AO_IOCTL_CastSetInfo,

    [CMD_ID_AO_CAST_ACQUIREFRAME]        = AO_IOCTL_CastAcquireFrame,
    [CMD_ID_AO_CAST_RELEASEFRAME]        = AO_IOCTL_CastReleaseFrame,

    [CMD_ID_AO_CAST_SETABSGAIN]          = AO_IOCTL_CastSetAbsGain,
    [CMD_ID_AO_CAST_GETABSGAIN]          = AO_IOCTL_CastGetAbsGain,

    [CMD_ID_AO_CAST_SETMUTE]             = AO_IOCTL_CastSetMute,
    [CMD_ID_AO_CAST_GETMUTE]             = AO_IOCTL_CastGetMute,
};
...
static HI_S32 AO_ProcessCmd(struct inode* inode, struct file* file, HI_U32 u32Cmd, HI_VOID* arg)
{
    ...
    if (HI_ID_AO == u32CmdType)
    {
        ...
        if (HI_NULL == g_fnAoIoctl[u32CmdId])
        {
            return HI_ERR_AO_NOTSUPPORT;
        }

        return (g_fnAoIoctl[u32CmdId])(file, arg);
    }
#if defined(HI_AIAO_VERIFICATION_SUPPORT)
