...
HI_S32 PLCIPHER_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "\n------------------------------------------CIPHER STATUS-------------------------\n");
    PROC_PRINT(p, "Chnid   Status   Encrypt   Alg      EvenOdd   PacketLen    Addr(in/out)    DataLen    \n");
    for(i = 0; i < PLCIPHER_CHAN_NUM; i++)
    {
      ...
      //PROC_PRINT(p, "Chnid   Status   Encrypt   Alg   EvenOdd   PacketLen    Addr(in/out)    DataLen    \n");
        PROC_PRINT( p, " %2d       %s      %d   %8s     %s         %08x/%08x   %d       \n",
                    i,
                    Status,
    ...
    memset(&stPLCipherChanInfo, 0, sizeof(stPLCipherChanInfo));
    stPLCipherChanInfo.ChanID = stDescStatus.LastChanNum;
    pstPLCipherChanInfo = &stPLCipherChanInfo;
    DRV_PLCIPHER_GetChanStatus(pstPLCipherChanInfo);
