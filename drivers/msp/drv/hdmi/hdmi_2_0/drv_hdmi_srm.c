...
HI_S32	DRV_HDMI_SrmDeinit(HI_VOID)
{
	...
	{
		HDMI_KFREE(pstSrmInfo->pu8RawData);
	}
...
HI_S32	DRV_HDMI_SrmSet(HI_U8 *pu8Data,HI_U32 u32Len,HI_BOOL bFromUser)
{
	...
	HI_U32			u32DevNum = 0;
	HI_U32			u32Offset = 0;
	HI_U8			au8HeadBuf[SRM_SIZE_HEADER];
	SRM_INFO_S		stSrmTmp;
	...
	/* check raw data */
	if((0==u32Len) || (u32Len < SRM_SIZE_HEADER + SRM_SIZE_LLC_SIGNATURE))
	{
	...
	HDMI_MEMSET(pstSrmInfo, 0 ,sizeof(SRM_INFO_S));
	pstSrmInfo->bInit 		= HI_TRUE;
	pstSrmInfo->pu8RawData 	= (HI_U8 *)HDMI_KMALLOC(u32Len);
