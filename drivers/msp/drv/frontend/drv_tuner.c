...
static HI_VOID select_demod_hi3130e(HI_U32 u32TunerPort, HI_U32 u32ResetGpioNo)
{
    HI_U8 u8DataTmp = 0;
...
#if defined(DEMOD_DEV_TYPE_X5HDV2QAM)
static HI_VOID select_demod_hi3130i(HI_U32 u32TunerPort, HI_U32 u32ResetGpioNo)
{
    HI_U8 u8DataTmp = 0;
...
#if defined (CHIP_TYPE_hi3716cv200) || defined (CHIP_TYPE_hi3716mv400)|| defined(CHIP_TYPE_hi3716mv410) \
    || defined(CHIP_TYPE_hi3716mv420) || defined(CHIP_TYPE_hi3796mv200)
...
|| defined(CHIP_TYPE_hi3716mv410) || defined(CHIP_TYPE_hi3716mv420) || defined(CHIP_TYPE_hi3796mv200)
...
static HI_VOID select_demod_hi3137(HI_U32 u32TunerPort)
{
    ...
    #endif
#else
    g_stTunerOps[u32TunerPort].demod_init = AVL68XX_Init;//hi3137_init;
    g_stTunerOps[u32TunerPort].tuner_connect = AVL68XX_Connect;//hi3137_connect;
    g_stTunerOps[u32TunerPort].tuner_get_status = AVL68XX_GetLockStatus;//hi3137_get_status;
    g_stTunerOps[u32TunerPort].tuner_get_ber = AVL68XX_GetBer;//hi3137_get_ber;
    g_stTunerOps[u32TunerPort].tuner_get_snr = AVL68XX_GetSnr;//hi3137_get_snr;
    g_stTunerOps[u32TunerPort].tuner_get_signal_strength  = AVL68XX_GetStrength;//hi3137_get_signal_strength;
    g_stTunerOps[u32TunerPort].tuner_get_freq_symb_offset = AVL68XX_GetFreqOffset;//hi3137_get_freq_symb_offset;
    g_stTunerOps[u32TunerPort].tuner_set_ts_type = AVL68XX_SetTsType;//hi3137_set_ts_type;
    g_stTunerOps[u32TunerPort].tuner_get_signal_info = AVL68XX_GetSignalInfo;//hi3137_get_signal_info;
    g_stTunerOps[u32TunerPort].tuner_setplpid = AVL68XX_SetPlpId;//hi3137_setplpid;
    g_stTunerOps[u32TunerPort].tuner_terscan_action = 0;//hi3137_tp_scan;
    g_stTunerOps[u32TunerPort].tuner_set_common_plp_id = AVL68XX_SetCommonPlpId;//hi3137_set_common_plp_id;
    g_stTunerOps[u32TunerPort].tuner_set_common_plp_combination = AVL68XX_SetCommonPlpCombination;//hi3137_set_common_plp_combination;
    g_stTunerOps[u32TunerPort].tuner_get_plp_num = AVL68XX_GetPlpNum;//hi3137_get_plp_num;
    g_stTunerOps[u32TunerPort].tuner_get_current_plp_type = AVL68XX_GetCurrentPlpType;//hi3137_get_current_plp_type;
    g_stTunerOps[u32TunerPort].tuner_get_plp_id = AVL68XX_GetPlpId;//hi3137_get_plp_id;
    g_stTunerOps[u32TunerPort].tuner_get_group_plp_id = AVL68XX_GetPlpGroupId;//hi3137_get_plp_group_id;
    g_stTunerOps[u32TunerPort].tuner_connect_timeout = AVL68XX_SetWaitTimes;//hi3137_connect_timeout;
    g_stTunerOps[u32TunerPort].tuner_set_ts_out = 0;//hi3137_set_ts_out;
    g_stTunerOps[u32TunerPort].tuner_set_ter_attr = AVL68XX_SetAttr;//hi3137_set_ter_attr;
    g_stTunerOps[u32TunerPort].tuner_set_antena_power =  AVL68XX_AntennaPower;//hi3137_set_antena_power;
    g_stTunerOps[u32TunerPort].tuner_standby = 0;//hi3137_standby;
    #if (1 == HI_PROC_SUPPORT)
    g_stTunerOps[u32TunerPort].tuner_get_registers = 0;//hi3137_read_tuner_registers;
    #endif
#endif
}
#endif
...
static HI_VOID select_demod_hi3138(HI_U32 u32TunerPort)
{
    g_stTunerOps[u32TunerPort].demod_init = hi3138_init;
...
static HI_S32 select_demod_type(HI_U32 u32TunerPort, HI_U32 u32ResetGpioNo)
{
    ...
    switch (g_stTunerOps[u32TunerPort].enDemodDevType)
    {
        ...
        {
            select_demod_hi3130e(u32TunerPort, u32ResetGpioNo);
            break;
        }
#endif
        ...
        {
            select_demod_hi3130i(u32TunerPort, u32ResetGpioNo);
            break;
        }
#endif
#if defined(DEMOD_DEV_TYPE_J83B)
        ...
        {
            bChipReset = HI_TRUE;
            select_demod_hi3138(u32TunerPort);
            break;
        }
#endif
...
static HI_VOID select_tuner_mxl608(HI_U32 u32TunerPort)
{
    ...
    if (g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_3137
     && g_stTunerOps[u32TunerPort].enDemodDevType != HI_UNF_DEMOD_DEV_TYPE_3138
...
HI_S32 HI_DRV_TUNER_Init(HI_VOID)
{
   ...
   // tuner_enable_crg();
    ...
    for (i = 0; i < TUNER_NUM; i++)
    {
        ...
        g_stTunerOps[i].demod_init = NULL;
...
//zhaobaoren
HI_S32 HI_DRV_TUNER_DISEQC_SendRecvMessage(HI_U32 u32TunerId,
                                           const HI_UNF_TUNER_DISEQC_SENDMSG_S * pstSendMsg,
                                           HI_UNF_TUNER_DISEQC_RECVMSG_S * pstRecvMsg)
{
    TUNER_DISEQC_SENDMSG_S stSend;
...
//zhaobaoren
HI_S32 HI_DRV_TUNER_SetLNBOutAnd22K(HI_U32 u32TunerId,
                                    HI_UNF_TUNER_FE_POLARIZATION_E enPolar, HI_UNF_TUNER_FE_LNB_22K_E enLNB22K)
{
    TUNER_LNB_OUT_S stLNBOut;
    HI_S32 s32Ret;
...
static HI_S32 HI_DRV_TUNER_Connect_S3(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    HI_S32 s32Ret = 0;
    TUNER_ACC_QAM_PARAMS_S stSignal = {0};
    HI_U32 u32TimeSpan = 0;
    ...
    printk("%s-->freq %d sys %d\n",__FUNCTION__,pstConnectPara->unConnectPara.stSat.u32Freq,pstConnectPara->unConnectPara.stSat.u32SymbolRate);
#ifdef zhaobaoren
    if (!((SAT_C_MIN_KHZ <= pstConnectPara->unConnectPara.stSat.u32Freq)
            && (SAT_C_MAX_KHZ >= pstConnectPara->unConnectPara.stSat.u32Freq))
        && !((SAT_KU_MIN_KHZ <= pstConnectPara->unConnectPara.stSat.u32Freq)
...
HI_S32 HI_DRV_TUNER_Connect(HI_U32  u32tunerId , const HI_UNF_TUNER_CONNECT_PARA_S  *pstConnectPara,HI_U32 u32TimeOut)
{
    ...
    {
        if(pstConnectPara->unConnectPara.stSat.enPolar == HI_UNF_TUNER_FE_POLARIZATION_BUTT)
        {
            s32Ret = HI_DRV_TUNER_Connect_S3(u32tunerId, pstConnectPara, u32TimeOut);
        }
        else
        {
...
EXPORT_SYMBOL(HI_DRV_TUNER_DISEQC_SendRecvMessage);
EXPORT_SYMBOL(HI_DRV_TUNER_SetLNBOutAnd22K);
