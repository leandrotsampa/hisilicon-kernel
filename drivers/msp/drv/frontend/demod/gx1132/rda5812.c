...
GX1132_STATE RDA5812Init(void);
/*
GX1132_STATE GX_RDA5812_Init_tuner(void)//20090731 gechq
{     
	s32 Read_tem;
	//sprintf(PrintfBuffer,"\n[RF]----rda5812\n");
	//GX1121_Printf(PrintfBuffer);

	Read_tem=GX1132_Read_one_Byte(0x06);
	if (Read_tem != FAILURE)
	{
		Read_tem |=0x80;     
		GX1132_Write_one_Byte(0x06,(u8)Read_tem);
	}
	RDA5812Init();
...
GX1132_STATE GX_Set_RDA5812(u32 RFfreq,u32 Symbol_Rate_Value)
{
	GX1132_STATE	state;
	u8	data[4];
	u8	crystal=27; // 27Mhz
...
********************************************************************************
*/
GX1132_STATE RDA5812Init(void)//20090818 gechq
{
	u8 i = 0;
