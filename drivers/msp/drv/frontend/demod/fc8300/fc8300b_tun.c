...
static enum BROADCAST_TYPE broadcast_type[4] = {ISDBT_13SEG, ISDBT_13SEG, ISDBT_13SEG, ISDBT_13SEG};

static u8 xtal_freq = 1; // 1 = 24Mhz     4;
static u8 catv_status[4] = {0, 0, 0, 0};
...
extern int getId(DEVICEID devid);
		...
		enum BROADCAST_TYPE broadcast)
{
	...
	fc8300b_write(handle, devid, 0x4a, broadcast_type[id]);
	...
	if ((broadcast_type[id] == ISDBT_13SEG) || (broadcast_type[id] == ISDBT_1SEG) ||
							(catv_status[id] == 2)) {
...
s32 fc8300b_set_freq(HANDLE handle, DEVICEID devid, u32 freq)
{
	...
	id = getId(devid);
 	...
 	if (freq < 273143)
 	{
 		if(broadcast_type[id] == ISDBT_13SEG)
 		{
 			catv_status[id] = 0;
 		}
		broadcast_type[id] = ISDBT_CATV_13SEG;
	}
	else
	{
		if(broadcast_type[id] == ISDBT_CATV_13SEG)
		{
			broadcast_type[id] = ISDBT_13SEG;
...
s32 fc8300b_get_rssi(HANDLE handle, DEVICEID devid, s32 *rssi)
{
	...
	*rssi = tmp;
	...
	if (broadcast_type[id] == ISDBT_1SEG || broadcast_type[id] == ISDBTMM_1SEG) {
