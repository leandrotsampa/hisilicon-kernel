...
#else
	bbm_byte_read(handle, devid, BBM_TS_SEL, &tsif_en);
	bbm_byte_write(handle, devid, BBM_TS_SEL, tsif_en & 0x7f);
#endif
#endif
...
#else
	bbm_byte_write(handle, devid, BBM_TS_SEL, tsif_en);
#endif
#endif
		...
		enum PRODUCT_TYPE product, enum BROADCAST_TYPE broadcast)
{
	...
	case FC8300_TUNER:
		tuner = &fc8300_tuner;
		tuner_addr = ((devid&0xff00)>>8); //FC8300_TUNER_ADDR;
		broadcast_type = broadcast;
		break;
	case FC8300B_TUNER:
		tuner = &fc8300b_tuner;
		tuner_addr = ((devid&0xff00)>>8); //FC8300B_TUNER_ADDR;
		broadcast_type = broadcast;
		break;
	default:
		return BBM_NOK;
