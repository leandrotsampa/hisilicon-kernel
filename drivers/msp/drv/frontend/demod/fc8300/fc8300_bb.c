...
s32 fc8300_reset(HANDLE handle, DEVICEID devid)
{
	...
	msWait(2);
#else 
	bbm_byte_write(handle, devid, BBM_SW_RESET, 0x7f);
	msWait(2);
	bbm_byte_write(handle, devid, BBM_SW_RESET, 0xcf);
	msWait(2);
	bbm_byte_write(handle, devid, BBM_SW_RESET, 0xff);
	msWait(2);
#endif
	return BBM_OK;
}
...
s32 fc8300_init(HANDLE handle, DEVICEID devid)
{
...
#ifdef BBM_I2C_TSIF
#ifdef BBM_TS_204
	bbm_byte_write(handle, devid, BBM_TS_SEL, 0xc0);
#else
	bbm_byte_write(handle, devid, BBM_TS_SEL, 0x80);
#endif /* #ifdef BBM_TS_204 */
#endif /* #ifdef BBM_I2C_TSIF */
	...
	bbm_byte_write(handle, devid, BBM_FUSELOAD, 0x03);
#else
	bbm_byte_write(handle, devid, BBM_FUSELOAD, 0x07);
#endif
	msWait(1);
	bbm_byte_write(handle, devid, BBM_FUSELOAD, 0x00);

	bbm_byte_write(handle, devid, BBM_PLL1_ENABLE, 0x01);
	bbm_byte_write(handle, devid, BBM_PLL1_PD, 0x00);

	fc8300_set_default_core_clk(handle, devid);
#ifdef BBM_I2C_TSIF
	fc8300_set_tsif_clk(handle, devid);
#endif
	...
	bbm_byte_write(handle, devid, BBM_ADC_PWRDN, 0x00);
	bbm_byte_write(handle, devid, BBM_ADC_RST, 0x00);
	bbm_byte_write(handle, devid, BBM_ADC_BIAS, 0x06);
	bbm_byte_write(handle, devid, BBM_BB2RF_RFEN, 0x01);
	bbm_byte_write(handle, devid, BBM_RF_RST, 0x00);
	bbm_byte_write(handle, devid, BBM_RF_POWER_SAVE, 0x00);
	bbm_long_write(handle, devid, BBM_MEMORY_RWM0, 0x07777777);
	bbm_byte_write(handle, devid, BBM_IQC_EN, 0x71);
	bbm_byte_write(handle, devid, BBM_CSF_GAIN_MAX, 0x0a);
	bbm_byte_write(handle, devid, BBM_ADC_CTRL, 0x27);
	bbm_byte_write(handle, devid, BBM_PSAT_ON_REF_1SEG_QPSK, 0x16);
	bbm_byte_write(handle, devid, BBM_PSAT_ON_REF_1SEG_16QAM, 0x16);
	bbm_byte_write(handle, devid, BBM_HOLD_RST_EN, 0x06);
	bbm_word_write(handle, devid, BBM_REF_DELAY_POST, 0x1900);
	bbm_word_write(handle, devid, BBM_REF_AMP, 0x03e0);
	bbm_word_write(handle, devid, BBM_AD_GAIN_PERIOD, 0x003f);
	bbm_word_write(handle, devid, BBM_FD_RD_LATENCY_1SEG, 0x0620);
	bbm_byte_write(handle, devid, BBM_DC_EST_EN, 0x00);

#if !defined(BBM_2_DIVERSITY) && !defined(BBM_4_DIVERSITY)
	bbm_byte_write(handle, devid, BBM_FD_OUT_MODE, 0x02);
	bbm_byte_write(handle, devid, BBM_DIV_START_MODE, 0x16);
#endif

	bbm_word_write(handle, devid, BBM_BUF_TS0_START, TS0_BUF_START);
	bbm_word_write(handle, devid, BBM_BUF_TS0_END, TS0_BUF_END);
	bbm_word_write(handle, devid, BBM_BUF_TS0_THR, TS0_BUF_THR);

	bbm_word_write(handle, devid, BBM_BUF_AC_A_START,
						AC_A_BUF_START);
	bbm_word_write(handle, devid, BBM_BUF_AC_A_END, AC_A_BUF_END);
	bbm_word_write(handle, devid, BBM_BUF_AC_A_THR, AC_A_BUF_THR);

	bbm_word_write(handle, devid, BBM_BUF_AC_B_START,
						AC_B_BUF_START);
	bbm_word_write(handle, devid, BBM_BUF_AC_B_END, AC_B_BUF_END);
	bbm_word_write(handle, devid, BBM_BUF_AC_B_THR, AC_B_BUF_THR);

	bbm_word_write(handle, devid, BBM_BUF_AC_C_START,
						AC_C_BUF_START);
	bbm_word_write(handle, devid, BBM_BUF_AC_C_END, AC_C_BUF_END);
	bbm_word_write(handle, devid, BBM_BUF_AC_C_THR, AC_C_BUF_THR);

	bbm_word_write(handle, devid, BBM_BUF_AC_D_START,
						AC_D_BUF_START);
	bbm_word_write(handle, devid, BBM_BUF_AC_D_END, AC_D_BUF_END);
	bbm_word_write(handle, devid, BBM_BUF_AC_D_THR, AC_D_BUF_THR);

#ifdef BBM_I2C_SPI
	bbm_byte_write(handle, devid, BBM_BUF_SPIOUT, 0x10);
#endif

#ifdef BBM_I2C_PARALLEL_TSIF
	bbm_byte_write(handle, devid, BBM_TS_CTRL, 0x8e);
#else
	bbm_byte_write(handle, devid, BBM_TS_CTRL, 0x0e);
#endif

#ifdef BBM_AUX_INT
	bbm_byte_write(handle, devid, BBM_SYS_MD_INT_EN, 0x7f);
	bbm_byte_write(handle, devid, BBM_AUX_INT_EN, 0xff);
	bbm_byte_write(handle, devid, BBM_FEC_INT_EN, 0x07);
	bbm_byte_write(handle, devid, BBM_BER_AUTO_UP, 0xff);
	bbm_byte_write(handle, devid, BBM_OSS_CFG_EN, 0x01);
#endif

	bbm_byte_write(handle, devid, BBM_FAIP_MTD_SR_SHIFT_VALUE, 0x10);
	bbm_byte_write(handle, devid, BBM_CIR_THR_23, 0x02);

#ifdef BBM_NULL_PID_FILTER
	bbm_byte_write(handle, devid, BBM_NULL_PID_FILTERING, 0x01);
#endif

#ifdef BBM_FAIL_FRAME
	bbm_byte_write(handle, devid, BBM_FAIL_FRAME_TX, 0x07);
#endif

#ifdef BBM_TS_204
	bbm_byte_write(handle, devid, BBM_FEC_CTRL_A, 0x03);
	bbm_byte_write(handle, devid, BBM_FEC_CTRL_B, 0x03);
	bbm_byte_write(handle, devid, BBM_FEC_CTRL_C, 0x03);
	bbm_byte_write(handle, devid, BBM_FEC_CTRL, 0x03);
#endif

#ifdef BBM_INT_LOW_ACTIVE
	bbm_byte_write(handle, devid, BBM_INT_POLAR_SEL, 0x00);
#endif

#ifdef BBM_SPI_30M
	bbm_byte_write(handle, devid, BBM_MD_MISO, 0x1f);
#endif

	bbm_byte_write(handle, devid, BBM_BCAS_ENABLE, 0x01);
	//bbm_byte_write(handle, devid, BBM_BCAS_ENABLE, 0x00);

	bbm_byte_write(handle, devid, BBM_INT_AUTO_CLEAR, 0x01);
	bbm_byte_write(handle, devid, BBM_BUF_ENABLE, 0x01);
	bbm_byte_write(handle, devid, BBM_BUF_INT_ENABLE, 0x01);

	bbm_byte_write(handle, devid, BBM_INT_MASK, 0x01);
	bbm_byte_write(handle, devid, BBM_INT_STS_EN, 0x01);
...
{
#ifdef BBM_I2C_TSIF
	bbm_byte_write(handle, devid, BBM_TS_SEL, 0x00);
	msWait(24);
#endif
...
u32 getTunerPort(DEVICEID devid)
{
	u32 tunerid = 0;
	tunerid = (devid - DIV_MASTER) >> 8;
	return tunerid;
}
...
s32 fc8300_scan_status(HANDLE handle, DEVICEID devid)
{
	...
	for (i = 0; i < ifagc_timeout; i++) {
		bbm_byte_read(handle, devid, 0x3025, &a);
	...
	for (; i < ffs_lock_timeout; i++) {
		bbm_byte_read(handle, devid, 0x3026, &a);
	...
	bbm_byte_read(handle, devid, 0x2023, &a);
	...
	switch (broadcast_type) {
	case ISDBT_1SEG:
		bbm_word_read(handle, devid, 0x4113, (u16 *) &data);
	...
	case ISDBTSB_3SEG:
		break;
	case ISDBT_13SEG:
		bbm_long_read(handle, devid, 0x4113, &data);
	...
	for (i = 0; i < ts_err_free_timeout; i++) {
		bbm_byte_read(handle, devid, 0x50c5, &a);
