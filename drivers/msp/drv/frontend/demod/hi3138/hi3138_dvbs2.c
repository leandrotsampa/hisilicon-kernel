...
extern HI_S32 hi3138_get_pair_idx(Hi3138_SIGNAL_INFO_ALLOCATION astHi3138Allocation[], HI_U32 *pu32Idx);
...
HI_S32  hi3138_get_total_isi_number(HI_U32 u32TunerPort,HI_U32 *pu32TotalStream)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    s32Ret = hi3136_get_total_isi_number(u32TunerPort, pu32TotalStream);
    if(HI_SUCCESS != s32Ret)
    {
...
HI_S32  hi3138_get_isi_id(HI_U32 u32TunerPort,HI_U8 u8Stream, HI_U8 *pu8IsiId)
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    s32Ret = hi3136_get_isi_id(u32TunerPort, u8Stream, pu8IsiId);
    if(HI_SUCCESS != s32Ret)
    {
...
static HI_S32 hi3138_get_lnb_src(HI_UNF_TUNER_ADC_PORT_E enADCId, Hi3138_LNB_POWER_SOURCE *penLNBSrc)
{
    switch(enADCId)
    {
        case HI_UNF_TUNER_ADC0:
            *penLNBSrc = HI3138_LNB_POWER_BY_ADDR0;
            break;
        case HI_UNF_TUNER_ADC1:
            *penLNBSrc = HI3138_LNB_POWER_BY_ADDR1;
            break;
        default:
            HI_ERR_TUNER("Invalid ADC port: %d.\n", enADCId);
            return HI_FAILURE;
    }
...
static HI_S32 hi3138_lnb_link_11(HI_U32 u32TunerPort)
{
    HI_U32 u32Idx;
    ...
    switch (u32Idx)
    {
        case 4: /*T+S1*/
        {
            ...
            {
                s32Ret = hi3138_get_lnb_src(stHi3138Allocation[0].enADCId, &stHi3138Allocation[0].enLNBSrc);
            }
            else if(stHi3138Allocation[1].enHi3138SigType == HI_UNF_TUNER_SIG_TYPE_SAT)
            {
                s32Ret = hi3138_get_lnb_src(stHi3138Allocation[1].enADCId, &stHi3138Allocation[1].enLNBSrc);
            }
            break;
        }
        case 5: /*ISDBT+S1*/
        ...
        default:
        {
            HI_ERR_TUNER("Invalid SigType Pair idx: %d.\n", u32Idx);
            return HI_FAILURE;
        }
    }
...
HI_S32 hi3138_lnb_link(HI_U32 u32TunerPort)
{
        ...
        (stHi3138Allocation[1].enHi3138SigType == HI_UNF_TUNER_SIG_TYPE_BUTT))
    {
        ...
        (stHi3138Allocation[1].enHi3138SigType != HI_UNF_TUNER_SIG_TYPE_BUTT))
    {
        s32Ret = hi3138_lnb_link_11(u32TunerPort);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_TUNER("hi3138_lnb_link_11 failed.\n");
            return HI_FAILURE;
        }
    }
