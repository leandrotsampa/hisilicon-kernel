...
HI_S32 hiISDBT_get_signal_info(HI_U32 u32TunerPort, HI_UNF_TUNER_SIGNALINFO_S *pstInfo)
{
    HI_U8 u8Temp = 0;
    ...
    qam_read_byte(u32TunerPort, HI3138_T_CHN_FFT_GI, &u8Temp);
    u8Grd = (u8Temp & 0x3);
    u8FFT = (u8Temp & 0x38) >> 3;
    ...
    HI_INFO_TUNER("-->IN--hiISDBT_get_signal_info,ExistanceFlag:%d,u8LayerAExist:%d,u8LayerBExist:%d,u8LayerCExist:%d.\n",
    ...
    /*获取层参数信息*/
    s32Ret = hiISDBT_get_TMCC_info(u32TunerPort, &(pstInfo->unSignalInfo.stIsdbT.stISDBTTMCCInfo));
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER("hiISDBT_get_TMCC_info failed.\n");
        return s32Ret;
    }
