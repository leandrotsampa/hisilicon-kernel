...
HI3136_LOCK_STATUS_E hi3136_Read_Lock_Status(HI_U32 u32TunerPort)  //?????????????
{
    HI_U8 u8Temp = 0;
...
static HI_VOID hi3136_vcm_ts_clk(HI_U32 u32TunerPort,HI_U32 fs_cur)
{
    ...
    {
        qam_read_byte(u32TunerPort, HI3138_S_ISI_NUM, &temp1);
        u32Total = temp1 & 0x3f;
        qam_read_byte(u32TunerPort,HI3138_S_TS_CTRL3,&temp1);
    }
    else
    {
        qam_read_byte(u32TunerPort, ISI_NUM, &temp1);
        u32Total = temp1 & 0x3f;
        qam_read_byte(u32TunerPort,TS_CTRL3,&temp1);
    }

    qam_read_byte(u32TunerPort, CR_STATE, &temp3);
    temp1 &= 0x80;
    temp3 &= 0x01;
    if((!temp1 && temp3) || (u32Total > 1))
    {
...
HI_S32 hi3136_get_ber(HI_U32 u32TunerPort, HI_U32 *pu32ber)
{
    ...
    if (hi3136_Read_Lock_Status(u32TunerPort) >= SYNC_LOCKED)
    {
        if(HI_UNF_DEMOD_DEV_TYPE_3138 == g_stTunerOps[u32TunerPort].enDemodDevType)
        {
            ...
            temp = (u8Temp >> 4) & 0x01;     //DVBS OR DIRECT TV
        }
        else
        {
...
HI_S32 hi3136_send_continuous_22K(HI_U32 u32TunerPort, HI_BOOL b22k_on) // ????22k,
{
    HI_U8   u8Temp = 0;
    HI_U32  diseqc_Ratio;
...
static HI_VOID hi3136_get_signal_info_s2(HI_U32 u32TunerPort, HI_UNF_TUNER_SIGNALINFO_S *pstInfo)
{
    ...
    u32ArrayNum = sizeof(astInfo) / sizeof(astInfo[0]) - 1; //???????????????????
...
HI_S32 hi3136_lock_TP_BS(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *tp)
{
    ...
    if(temp_TPnum > 0)
    {
        ...
        for(j = 0; j < temp_TPnum; j++)     //???Fs Fc???????????????????????????????????
        {
            ...
            reliablity = abs(cbs_fc) / 64 + read_reliable;      //??????????? ? ????
        ...
        qam_write_byte(u32TunerPort, CBS_CTRL_RDADDR, 0x00);    //???????Ram????0
        hi3136_setlpf(u32TunerPort, cbs_fs_sel * 1000 / 10 * 7 + abs(fc_true) * 1000 + 3500000);    //???????????????§³§»????3.5M

        tuner_mdelay(10); //?????????TUNER????????????????????????????????tuner???????????¨²???????
        hi3136_HotReset_CHIP(u32TunerPort);
...
HI_U32 hi3136_BS_one_Window(HI_U32 u32TunerPort, HI_U32 Fcenter/*MHz*/, HI_U32 Lpf_BW_window/*kHz*/, HI_UNF_TUNER_SAT_TPINFO_S *pstChannel)
{
    ...
    do
    {
        ...
        time_cnt += 5;
        qam_read_bit(u32TunerPort, CBS_FINISH, 0, &u8Temp);
    } while(!u8Temp && (time_cnt < 300));       //??????CBS_FINISH ????,??????10000ms??????????
    ...
    if(TP_Num != 0)
    {
        ...
        for(i = 0; i < TP_Num; i++)
        {
            ...
            qam_read_byte(u32TunerPort, CBS_FS_H, &tempH);
            Fs_Bs = tempL + (tempH << 8);
            pstChannel->u32SymbolRate = Fs_Bs * 1000;//??????

            qam_read_byte(u32TunerPort, CBS_RELIABLITY, &reliablity);
            pstChannel->cbs_reliablity = reliablity;    //cbs ?????????????????
            pstChannel++;
        }
    }
    else
...
HI_S32 hi3136_blindscan_action(HI_U32 u32TunerPort, TUNER_BLINDSCAN_PARA_S *pstPara)
{
    ...
    qam_read_byte(u32TunerPort, CBS_CTRL_RDADDR, &u8Temp);
    u8Temp &= 0xe0;
    qam_write_byte(u32TunerPort, CBS_CTRL_RDADDR, u8Temp);//??cbs_rd_addr ????
...
HI_S32 hi3136_set_sat_attr(HI_U32 u32TunerPort, HI_UNF_TUNER_SAT_ATTR_S *pstSatTunerAttr)
{
    ...
    HI_U32 clk_xo_khz;
    HI_U32 clk_demo_khz;
    HI_U32 demo_div, demo_div_int, demo_div_fh, demo_div_fl;
    ...
    fec_div = clk_fec_khz  * 6 * 2048 / clk_xo_khz * 2;  // ??U32????
    ...
    qam_write_byte(u32TunerPort, PLL_CTRL3, (HI_U8)demo_div_int);   // ??????????????
    qam_write_byte(u32TunerPort, PLL_CTRL4, (HI_U8)demo_div_fl);    // ??????§³????????8¦Ë
    qam_write_byte(u32TunerPort, PLL_CTRL5, (HI_U8)demo_div_fh);    // ??????§³????????4¦Ë
...
HI_S32 hi3136_connect(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *pstChannel)
{
    ...
    s32FuncRet = hi3136_lock_TP_BS(u32TunerPort, pstChannel); //??????BS
    if (HI_SUCCESS != s32FuncRet)
    {
        TIME_COST_END;
...
HI_S32 hi3136_DiSEqC_send_msg(HI_U32 u32TunerPort, HI_UNF_TUNER_DISEQC_SENDMSG_S *pstSendMsg)
{
    ...
    diseqc_Ratio = (hi3136_cfg[u32TunerPort].u32CLK_DEMO_kHz / 22) & 0xffff;
    qam_write_byte(u32TunerPort, DSEC_RATIO_L, (HI_U8)(diseqc_Ratio & 0xff));
    qam_write_byte(u32TunerPort, DSEC_RATIO_H, (HI_U8)((diseqc_Ratio & 0xff00) >> 8)); //????22K ???

    qam_read_byte(u32TunerPort, TX_CTRL, &diseqc_mode);
    u8Temp = diseqc_mode & 0x01;   /*diseqc ??????????????diseqc module ??¦Ë???????????*/
    qam_write_byte(u32TunerPort, TX_CTRL, u8Temp);
    ...
    qam_write_byte(u32TunerPort,TX_CTRL,u8Temp); /*????num+1?????,??????????diseqc???*/

    u8Int = pstSendMsg->u8Length / 2;
    u32DelayMs = (HI_U32)(u8Int * 27 + (pstSendMsg->u8Length % 2) * 14) + 16 + 16;
    msleep_interruptible(u32DelayMs);   //???????30ms
...
HI_S32 hi3136_tp_verify(HI_U32 u32TunerPort, TUNER_TP_VERIFY_PARAMS_S * pstChannel)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if(fec_ok_bit || ((i==0) && !hist_is_ok && !s1_high_cn && (low_cn  || try_enough ||may_sfu  || !chance_2)) || ((i==1) && (!s1_high_cn ||try_enough))) //??????????????
            break;
     }
...
HI_S32 hi3136_send_tone(HI_U32 u32TunerPort, HI_U32 u32Tone)
{
    ...
    qam_write_byte(u32TunerPort, DSEC_RATIO_H, (HI_U8)((u32DiseRatio & 0xff00) >> 8)); //????22K ???


    u8Temp = u8DiseMode & 0x01;   /*diseqc ??????????????diseqc module ??¦Ë???????????*/
    qam_write_byte(u32TunerPort, TX_CTRL, u8Temp);
    switch(u32Tone)
    {
        case 0:
