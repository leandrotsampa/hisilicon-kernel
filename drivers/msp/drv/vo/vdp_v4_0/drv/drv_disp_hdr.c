...
HI_VOID DISP_WorkModeChangeProcess(DISP_S *pstDisp,
                                             HI_DRV_DISP_WORKING_MODE_E enCurWorkMode,
                                             HI_DRV_DISP_WORKING_MODE_E enPreWorkMode)
{
    HI_BOOL bWorkModeChange = HI_FALSE;
    HI_BOOL bCurSdrWorkMode = HI_TRUE;

    if (((HI_DRV_DISP_WORKING_MODE_SDR == enCurWorkMode)
        && (HI_DRV_DISP_WORKING_MODE_SDR != enPreWorkMode))
        || ((HI_DRV_DISP_WORKING_MODE_SDR != enCurWorkMode)
        && (HI_DRV_DISP_WORKING_MODE_SDR == enPreWorkMode)))
    {
        bWorkModeChange = HI_TRUE;
    }

    bCurSdrWorkMode = (HI_DRV_DISP_WORKING_MODE_SDR == enCurWorkMode)? HI_TRUE : HI_FALSE;
...
HI_VOID ReviseHDROutRect(HI_RECT_S* pstWinOutRect, HI_RECT_S* pstFmtResolution)
{
	if (pstWinOutRect->s32X < 0)
	{
	    if (pstWinOutRect->s32X + pstWinOutRect->s32Width > 64 )
	    {
	        pstWinOutRect->s32Width = pstWinOutRect->s32X + pstWinOutRect->s32Width;
	        pstWinOutRect->s32X = 0;
	    }
	    else
	    {
	        pstWinOutRect->s32Width = 64;
	        pstWinOutRect->s32X = 0;
	    }
	}

	if (pstWinOutRect->s32Y < 0)
	{
	    if (pstWinOutRect->s32Y + pstWinOutRect->s32Height > 64 )
	    {
	        pstWinOutRect->s32Height = pstWinOutRect->s32Y + pstWinOutRect->s32Height;
	        pstWinOutRect->s32Y = 0;
	    }
	    else
	    {
	        pstWinOutRect->s32Height = 64;
	        pstWinOutRect->s32Y = 0;
	    }
	}

	if (pstWinOutRect->s32X + pstWinOutRect->s32Width > pstFmtResolution->s32Width)
	{
	    pstWinOutRect->s32Width = pstFmtResolution->s32Width - pstWinOutRect->s32X ;

	  
	}

	if (pstWinOutRect->s32Y + pstWinOutRect->s32Height > pstFmtResolution->s32Height)
	{
	    pstWinOutRect->s32Height = pstFmtResolution->s32Height - pstWinOutRect->s32Y ;
	}
	
	if (pstWinOutRect->s32Height < 64 )
	{
	   pstWinOutRect->s32Height = 64;
	}
	
	if (pstWinOutRect->s32Width < 64 )
	{
	  pstWinOutRect->s32Width = 64;
	}
...
HI_VOID DISP_ISR_SourceProcess(HI_VOID *hHandle, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
	...
	ReviseHDROutRect(&stWinDolbyRefInfo.stWinOutRect,&stDispInfo.stFmtResolution);

    enWorkMode = DISP_WorkingModePolicy(&stWinDolbyRefInfo, &pstDisp->stHdrPlayInfo);

    DISP_WorkModeChangeProcess(pstDisp, enWorkMode, pstDisp->stHdrPlayInfo.enLastWorkMode);
    ...
    {
        DISP_VideoHDRPathProcess(pstDisp, enWorkMode, &stWinDolbyRefInfo, &stHdmiHdrAttr);
