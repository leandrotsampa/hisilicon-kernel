...
static HI_VOID UpdateLayerMuteInfor(VDP_LAYER_VID_INFO_S *pstInfo)
{
    ...
    pstInfo->stSrcInfo.enDataWidth   = VDP_DATA_WTH_8;
    pstInfo->stSrcInfo.enSrcClrSpace = VDP_CLR_SPACE_YUV_709;
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_DATA].bSecure = HI_FALSE;
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_DATA].bSmmu = HI_FALSE;

    pstInfo->stSrcInfo.stAddr[VDP_ADDR_DATA].u32LumAddr = gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_MUTE_V1_L];
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_DATA].u32ChmAddr = gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_MUTE_V1_C];

    pstInfo->stSrcInfo.stAddr[VDP_ADDR_DATA].u32LumStr  = 0;
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_DATA].u32ChmStr  = 0;

    pstInfo->stSrcInfo.stAddr[VDP_ADDR_3D].bSecure = HI_FALSE;
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_3D].bSmmu = HI_FALSE;

    pstInfo->stSrcInfo.stAddr[VDP_ADDR_3D].u32LumAddr = gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_MUTE_V1_L];
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_3D].u32ChmAddr = gstVdpCoefBufAddr.u32CoefPhyAddr[VDP_COEF_BUF_MUTE_V1_C];

    pstInfo->stSrcInfo.stAddr[VDP_ADDR_3D].u32LumStr  = 0;
    pstInfo->stSrcInfo.stAddr[VDP_ADDR_3D].u32ChmStr  = 0;
}
...
VDP_LAYER_VID_INFO_S stInfo_tmp;
HI_S32 VDP_DRV_SetVidLayer(HI_U32 u32LayerId, VDP_LAYER_VID_INFO_S *pstLayerInfor)
{
    ...
    VDP_LAYER_VID_INFO_S *pstInfo = HI_NULL;
    HI_S32 s32IfirCoef[8] = {0,0,0,0,0,0,-10,42};
    ...
    memset((HI_VOID *)&stInfo_tmp, 0, sizeof(VDP_LAYER_VID_INFO_S));
    memcpy((HI_VOID *)&stInfo_tmp, pstLayerInfor, sizeof(VDP_LAYER_VID_INFO_S));
    ...
    {
        VDP_VID_SetReadMode                (u32LayerId, (VDP_DATA_RMODE_E)0,(VDP_DATA_RMODE_E)0);
    }
    else
    {
    ...
    VDP_VID_SetIfirCoef            (VDP_LAYER_VID0, s32IfirCoef);
    VDP_VID_SetIfirCoef            (VDP_LAYER_VID1, s32IfirCoef);
    ...
    stCropInfo.enDrawPixelMode   = pstInfo->enDrawPixelMode;
    ...
    stAddrInfo.enDataWidth = pstInfo->stSrcInfo.enDataWidth;
    ...
    stZmeInfo.enDrawPixelMode  = pstInfo->enDrawPixelMode;
    stZmeInfo.enHzmeMode  = pstInfo->enHzmeMode;
    stZmeInfo.bHzmeEn  = pstInfo->bHzmeEn;
...

    {
        ...
        stColorSpaceInfo.enInClrSpace  = pstLayerInfor->enInClrSpace;
        stColorSpaceInfo.enOutClrSpace = pstLayerInfor->enOutClrSpace;
    ...
    {
        VDP_VID_SetRegionMuteEnable    (u32LayerId, pstInfo->u32RegionNO, HI_FALSE);
...
HI_S32  VDP_DRV_SetVidCrop (HI_U32 u32LayerId, const VDP_DRV_CROP_INFO_S *pstCropInfo, VDP_DRV_ADDR_INFO_S *pstAddrInfo)
{
    ...
    #if 1
    if(pstCropInfo->enDrawPixelMode == VDP_DRAW_PIXEL_MODE_2)
    {
        stCropOutRect.u32Wth /=2;
    }
    else if(pstCropInfo->enDrawPixelMode== VDP_DRAW_PIXEL_MODE_4)
    {
        stCropOutRect.u32Wth /=4;
    }
    else if(pstCropInfo->enDrawPixelMode== VDP_DRAW_PIXEL_MODE_8)
    {
    ...
    VDP_VID_SetDrawPixelMode(u32LayerId,pstCropInfo->enDrawPixelMode);
...
HI_VOID VDP_DRV_SetVidZme2(HI_U32 u32LayerId, HI_BOOL bEnable,VDP_DRV_ZME_INFO_S *pstZmeInfo)
{
    ...
    if (VDP_LAYER_VID0 == u32LayerId)
    {
        if(HI_TRUE == bEnable)
        {
            ...
            VDP_VID_SetZme2FirEnable           (u32LayerId, VDP_ZME_MODE_HORL, pstZmeInfo->pstZmeDrvPara.bZmeFirHL);
            VDP_VID_SetZme2FirEnable           (u32LayerId, VDP_ZME_MODE_HORC, pstZmeInfo->pstZmeDrvPara.bZmeFirHC);
            VDP_VID_SetZme2MidEnable           (u32LayerId, VDP_ZME_MODE_HOR, pstZmeInfo->pstZmeDrvPara.bZmeMedH);



            VDP_VID_SetZme2Enable              (u32LayerId, VDP_ZME_MODE_HOR, HI_TRUE);
            VDP_VID_SetZme2Phase               (u32LayerId, VDP_ZME_MODE_HOR, 0*ZME_HPREC);
            VDP_VID_SetZme2HorRatio            (u32LayerId, pstZmeInfo->stZme2InRect.u32Wth*ZME_HPREC/pstZmeInfo->stOutRect.u32Wth);


            VDP_VID_SetZme2Enable              (u32LayerId, VDP_ZME_MODE_VER, HI_TRUE);

            VDP_VID_SetZme2FirEnable           (u32LayerId,  VDP_ZME_MODE_VERL, pstZmeInfo->pstZmeDrvPara.bZmeFirVL);
            VDP_VID_SetZme2FirEnable           (u32LayerId,  VDP_ZME_MODE_VERC, pstZmeInfo->pstZmeDrvPara.bZmeFirVC);
            VDP_VID_SetZme2MidEnable           (u32LayerId, VDP_ZME_MODE_VER, pstZmeInfo->pstZmeDrvPara.bZmeMedV);


#ifdef VDP_PQ_STRATEGY
            pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWIn  = pstZmeInfo->stZme2InRect.u32Wth;
            pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWOut = pstZmeInfo->stOutRect.u32Wth;
            pstZmeInfo->pstZmeDrvPara.u32ZmeFrmHIn  = pstZmeInfo->stZme2InRect.u32Hgt;
            ...
            pstZmeInfo->pstZmeDrvPara.u8ZmeYCFmtIn  = 0;
            ...
            VDP_VID_SetZme2Phase               (u32LayerId, VDP_ZME_MODE_VERL, pstZmeRtlPara.s32ZmeOffsetVL);
            VDP_VID_SetZme2Phase               (u32LayerId, VDP_ZME_MODE_VERC, pstZmeRtlPara.s32ZmeOffsetVC);
            VDP_VID_SetZme2Phase               (u32LayerId, VDP_ZME_MODE_VERBL,pstZmeRtlPara.s32ZmeOffsetVLBtm);
            VDP_VID_SetZme2Phase               (u32LayerId, VDP_ZME_MODE_VERBC,pstZmeRtlPara.s32ZmeOffsetVCBtm);

            VDP_VID_SetZme2VerRatio            (u32LayerId, pstZmeInfo->stZme2InRect.u32Hgt*ZME_VPREC/pstZmeInfo->stOutRect.u32Hgt);

#if 1 //zme2
            GetVdpZmeCoef(HI_PQ_ZME_COEF_1,HI_PQ_ZME_COEF_TYPE_LH,HI_PQ_ZME_TAP_8T32P, (HI_S16*)gstVdpCoefBufAddr.pu8CoefVirAddr[VDP_COEF_BUF_ZME_TMP]);
...
{
    //VDP_PRINT("VDP_DRV_SetVid1Cvfir start \n");

    if(u32LayerId == VDP_LAYER_VID1)
    {
            if(pstZmeInfo->enInFmt == VDP_VID_IFMT_SP_420 )
            {
                //VDP_PRINT("VDP_DRV_SetVid1Cvfir VDP_VID_IFMT_SP_420 \n");
                ...
                //VDP_VID_SetCvfirVCoef         (u32LayerId, pstZmeInfo->pstCvfirVCoef);
            }
            ...
            {
                //VDP_PRINT("VDP_DRV_SetVid1Cvfir VDP_PROC_FMT_SP_422 \n");
                VDP_VID_SetZmeInFmt            (u32LayerId, VDP_PROC_FMT_SP_422);
                VDP_VID_SetZmeOutFmt           (u32LayerId, VDP_PROC_FMT_SP_422);
                VDP_VID_SetZmeEnable           (u32LayerId, VDP_ZME_MODE_VERC, HI_FALSE);
            }
            ...
            //VDP_PRINT("VDP_DRV_SetVid1Cvfir end \n");
    }
                                                ...
                                                VDP_VID_IFMT_E   enInZmeFmt)
{
    ...
    if (pstZmeInfo->enInFmt == VDP_VID_IFMT_SP_420 || pstZmeInfo->enInFmt == VDP_VID_IFMT_SP_TILE ||  pstZmeInfo->enInFmt == VDP_VID_IFMT_SP_TILE_64)
    {
            s32RatioHL  =(pstZmeInfo->stZme2InRect.u32Wth*100)/pstZmeInfo->stInRect.u32Wth;
            s32RatioHC  =(pstZmeInfo->stZme2InRect.u32Wth*100)/pstZmeInfo->stInRect.u32Wth;
            s32RatioVL  = pstZmeInfo->stZme2InRect.u32Hgt*100/pstZmeInfo->stInRect.u32Hgt;
...
{
    VDP_RECT_S stZme1Out = pstZmeInfo->stZme2InRect;
    ...
    if((pstZmeInfo->stZme2InRect.u32Wth != pstZmeInfo->stOutRect.u32Wth)
        || (pstZmeInfo->stZme2InRect.u32Hgt !=  pstZmeInfo->stOutRect.u32Hgt))
    {
        VDP_DRV_SetVidZme2(u32LayerId, HI_TRUE,pstZmeInfo);
    }
    else
    {
        VDP_DRV_SetVidZme2(u32LayerId, HI_FALSE,pstZmeInfo);
    }
    ...
    {
        pstZmeInfo->stInRect.u32Hgt /=2;
    }
    else if(pstZmeInfo->enDrawMode== VDP_DRAW_MODE_4)
    {
        pstZmeInfo->stInRect.u32Hgt /=4;
    }
    else if(pstZmeInfo->enDrawMode== VDP_DRAW_MODE_8)
    {
        pstZmeInfo->stInRect.u32Hgt /=8;
    }
    else //no draw
    {
        pstZmeInfo->stInRect.u32Hgt /=1;
    }

    if(pstZmeInfo->bHzmeEn == HI_TRUE)
    {
        pstZmeInfo->stInRect.u32Wth /= 2;
    }

    if(pstZmeInfo->enDrawPixelMode == VDP_DRAW_PIXEL_MODE_2)
    {
        pstZmeInfo->stInRect.u32Wth /= 2;
    }
    else if(pstZmeInfo->enDrawPixelMode== VDP_DRAW_PIXEL_MODE_4)
    {
        pstZmeInfo->stInRect.u32Wth /= 4;
    }
    else if(pstZmeInfo->enDrawPixelMode== VDP_DRAW_PIXEL_MODE_8)
    {
        pstZmeInfo->stInRect.u32Wth /= 8;
    }
    else //no pixel draw
    {
        pstZmeInfo->stInRect.u32Wth /= 1;
    }

    u32ZmeInResoWidth  = pstZmeInfo->stInRect.u32Wth;
    ...
    if (VDP_LAYER_VID0 == u32LayerId)
    {
        VDP_VID_SetHpzmeHpzmeEn(VDP_LAYER_VID0,pstZmeInfo->bHzmeEn);
        VDP_VID_SetHpzmeHpzmeMode(VDP_LAYER_VID0,pstZmeInfo->enHzmeMode);

        VDP_VID_SetHpzmeCoef0(VDP_LAYER_VID0,6);
        VDP_VID_SetHpzmeCoef1(VDP_LAYER_VID0,52);
        VDP_VID_SetHpzmeCoef2(VDP_LAYER_VID0,6);

        VDP_VID_SetSrOutReso (u32LayerId, pstZmeInfo->stOutRect);
    }
    ...
    /*u32ZmeInResoWidth /2 first, and *2 sencondly, to avoid overflow.*/
    VDP_VID_SetZmeHorRatio             (u32LayerId, (u32ZmeInResoWidth/2*ZME_HPREC)/stZme1Out.u32Wth*2*u32HorRation);
...
#ifdef VDP_PQ_STRATEGY
    pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWIn  = u32ZmeInResoWidth;
    ...
    if (u32LayerId == 1)
    {
        if ((pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWIn == 0)
           || (pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWOut == 0)
           || (pstZmeInfo->pstZmeDrvPara.u32ZmeFrmHIn == 0)
           || (pstZmeInfo->pstZmeDrvPara.u32ZmeFrmHOut == 0))
        {
            VDP_PRINT("---zme in/out:%d,%d,%d,%d!\n", pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWIn,
                                               pstZmeInfo->pstZmeDrvPara.u32ZmeFrmWOut,
                                               pstZmeInfo->pstZmeDrvPara.u32ZmeFrmHIn,
                                               pstZmeInfo->pstZmeDrvPara.u32ZmeFrmHOut);
        }
    }
    ...
    if(u32LayerId == VDP_LAYER_VID1)
    {
        ...
        VDP_VID_SetZmeFirEnable        (u32LayerId, VDP_ZME_MODE_VER, pstZmeInfo->pstZmeDrvPara.bZmeFirVC);
    }
...
#if 1
HI_S32 VDP_DRV_SetV0V1CscCfg(HI_U32 u32LayerId,HI_S16  S16Yuv2Rgb[3][3],
...
HI_S32 VDP_DRV_GetCscMode(VDP_CLR_SPACE_E enInClr, VDP_CLR_SPACE_E enOutClr,VDP_CSC_MODE_E *enCscMode,HI_PQ_CSC_MODE_E *enPqCscMode)
{
    #ifndef  __DISP_PLATFORM_BOOT__
    static VDP_HISI_HDR_CFG_S pstHdrCfg;
         ...
         || (enInClr == VDP_CLR_SPACE_RGB_2020 && enOutClr == VDP_CLR_SPACE_RGB_2020))
    {
        ...
        *enPqCscMode = HI_PQ_CSC_RGB2RGB             ; /* RGB          -> RGB */
         ...
         || (enInClr == VDP_CLR_SPACE_YUV_2020 && enOutClr == VDP_CLR_SPACE_YUV_2020_L)
         || (enInClr == VDP_CLR_SPACE_YUV_2020 && enOutClr == VDP_CLR_SPACE_YUV_2020)
         ...
         )
    {
       ...
       // VDP_VID_SetVdmVdmRgb2yuvEn(VDP_LAYER_VID0, HI_FALSE);
        ...
        || (enInClr == VDP_CLR_SPACE_YUV_2020 && enOutClr == VDP_CLR_SPACE_YUV_709_L)
        || (enInClr == VDP_CLR_SPACE_YUV_2020 && enOutClr == VDP_CLR_SPACE_YUV_709)
    ...
    )
    {
        ...
        *enPqCscMode = HI_PQ_CSC_YUV2RGB_2020_709_L             ; /* YCbCr L      -> YCbCr L */
        ...
        pstHdrCfg.eHiHdrMode = HIHDR_BT2020_IN_709_OUT;
        //get
        VDP_DRV_GetHiHdrCfg(&pstHdrCfg,&pstDmCfg);
        //set
        VDP_DRV_SetHiHdrCfg(&pstDmCfg);
        VDP_DRV_SetBt2020GammaCoef(pstHdrCfg.eHiHdrMode,&pstPqHdrCfg);
        VDP_DRV_SetBt2020DegmmaCoef(pstHdrCfg.eHiHdrMode,&pstPqHdrCfg);
        ...
        *enPqCscMode =HI_PQ_CSC_YUV2YUV_2020_709_L_L;
        ...
        || (enInClr == VDP_CLR_SPACE_YUV_2020 && enOutClr == VDP_CLR_SPACE_RGB_709_L)
        || (enInClr == VDP_CLR_SPACE_YUV_2020 && enOutClr == VDP_CLR_SPACE_RGB_709))
    {
...
HI_S32 VDP_DRV_SetVidCsc(HI_U32 u32LayerId, VDP_CLR_SPACE_E enInClr, VDP_CLR_SPACE_E enOutClr)
{
    ...
    VDP_DRV_GetCscMode(enInClr,enOutClr,&enCscMode,&enPqCscMode);
    ...
    else
    {
            ...
            && (enPqCscMode != HI_PQ_CSC_YUV2RGB_2020_709_L))
        {
           ...
           // VDP_DRV_SetV0V1CscCfg(u32LayerId,S16Yuv2Rgb,S32Yuv2RgbOutDc,S32Yuv2RgbInDc);
#endif
        }
    }
