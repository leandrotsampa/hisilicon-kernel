...
HI_S32 VDP_DRV_SetVidBaseCfg(VDP_HISI_HDR_CFG_S *pstHdrCfg)
{
    ...
    VDP_VID_SetLayerEnable (VDP_LAYER_VID0, !pstHdrCfg->bMuteEn);
    ...
    if (pstHdrCfg->stSrcInfo.enSrc3dMode == VDP_DISP_MODE_2D)
    {
		...
		VDP_VID_Set16RegionSmmuLumBypass (VDP_LAYER_VID0, 1, !pstHdrCfg->stSrcInfo.bSmmuEn);
    	VDP_VID_Set16RegionSmmuChromBypass (VDP_LAYER_VID0, 1, !pstHdrCfg->stSrcInfo.bSmmuEn);
    }
    else
    {
	...
	if(pstHdrCfg->stCropRect.u32Hgt*10 <= pstHdrCfg->stVideoRect.u32Hgt*24 )
    {
        pstHdrCfg->stHdrVdpCfgExt.enDrawMode = VDP_DRAW_MODE_1;
    }
    else if(pstHdrCfg->stCropRect.u32Hgt*10 <= pstHdrCfg->stVideoRect.u32Hgt*40 )
    {
        pstHdrCfg->stHdrVdpCfgExt.enDrawMode = VDP_DRAW_MODE_2;
    }
    else if(pstHdrCfg->stCropRect.u32Hgt*10 <= pstHdrCfg->stVideoRect.u32Hgt*60 )
    {
        pstHdrCfg->stHdrVdpCfgExt.enDrawMode = VDP_DRAW_MODE_4;
    }
	else 
    {
        pstHdrCfg->stHdrVdpCfgExt.enDrawMode = VDP_DRAW_MODE_8;
    }
...
HI_S32 VDP_DRV_SetHisiHdrMode(VDP_HISI_HDR_CFG_S *pstHdrCfg,HI_PQ_HDR_CFG  *pstPqHdrCfg)
{
    ...
    stColorSpaceInfo.bSourceIsSdr = pstHdrCfg->stSrcInfo.bSdrFrm;
    stColorSpaceInfo.enInClrSpace = pstHdrCfg->stSrcInfo.enSrcClrSpace;
    stColorSpaceInfo.enOutClrSpace = pstHdrCfg->stVdpDispInfo.enOutClrSpace;
    VDP_DRV_SetVdpBkg(VDP_LAYER_VID0, &stColorSpaceInfo);
