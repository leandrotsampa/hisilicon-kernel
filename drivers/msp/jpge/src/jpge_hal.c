...
static HI_S32 Jpge_StartOneFrame_toVenc(Jpge_EncPara_S	*pstEncPara, Venc2Jpge_EncIn_S *pEncIn)
{
    ...
    Ret = copy_from_user(pstEncPara->pJpgeYqt,pEncIn->pJpgeYqt,sizeof(HI_U8)*64);
    if (Ret < 0)
    {
	return HI_FAILURE;
    }

    Ret = copy_from_user(pstEncPara->pJpgeCqt,pEncIn->pJpgeCqt,sizeof(HI_U8)*64);
    if (Ret < 0)
    {
	return HI_FAILURE;
    }
...
static HI_S32 Jpge_EndOneFrame(Jpge_EncPara_S  *pstEncPara, Jpge_EncIn_S *pstEncIn, Jpge_EncOut_S *pEncOut)
{
    ...
    s32Ret = copy_from_user(&pNaluHdr,(HI_VOID*)((unsigned long)pstEncIn->pOutBuf + pstEncPara->Vir2BusOffset),sizeof(Jpge_NaluHdr_S));
    if (s32Ret < 0)
    {
	return -EFAULT;
    }
