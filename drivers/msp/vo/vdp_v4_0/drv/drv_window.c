...
static HI_BOOL WinJudgeDobyELVPSSPreZME(HI_DRV_DISP_FMT_E eFmt, HI_RECT_S stWinOutRect, HI_U32 u32SrcWidth,HI_U32 u32SrcHeight)
{
    HI_BOOL bNeedVpssPreZME = HI_FALSE;

    if ((0 == (stWinOutRect.s32Width % (HI_WIN_OUT_RECT_WIDTH_ALIGN * 2))) && (0 == (stWinOutRect.s32Height % (HI_WIN_OUT_RECT_HEIGHT_ALIGN * 2))))
    {
	if (((u32SrcWidth * 2 ) == stWinOutRect.s32Width ) && ((u32SrcHeight * 2) == stWinOutRect.s32Height))
	{
	    bNeedVpssPreZME = HI_TRUE;
	}

	/*format 4K60/50 ,el if > 1920,maybe  lowband*/
	if ((eFmt == HI_DRV_DISP_FMT_3840X2160_50)
	    || (eFmt == HI_DRV_DISP_FMT_3840X2160_60)
	    || (eFmt == HI_DRV_DISP_FMT_4096X2160_50)
	    || (eFmt == HI_DRV_DISP_FMT_4096X2160_60)
	   )
	{
	    if (((stWinOutRect.s32Height > 1080) || (stWinOutRect.s32Width > 1920)))
	    {
		bNeedVpssPreZME = HI_TRUE;
	    }
	}
...
static HI_S32 WinCalDobySourceInfo(WINDOW_S *pstWin, HI_U32 u32VideoLayer, HI_DISP_DISPLAY_INFO_S *pstDispInfo, HI_DRV_WIN_PRIV_INFO_S *pstInfo)
{
    ...
    HI_U32 u32SrcOrgWidth = 0;
    HI_U32 u32SrcOrgHeight = 0;
	...
	if (pstWin->stBuffer.stWinBP.pstDisplay)
	{
	    ...
	    if ((HI_TRUE == pstWin->stBuffer.stWinBP.pstDisplay->bValidData2)
				&& (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_EL == pstElFrm->enSrcFrameType))
	    {
		pstPrivInfo = (HI_DRV_VIDEO_PRIVATE_S*)(pstElFrm->u32Priv);
		u32SrcOrgWidth = pstPrivInfo->stVideoOriginalInfo.u32Width;
		u32SrcOrgHeight	 = pstPrivInfo->stVideoOriginalInfo.u32Height;
	    }
	}
	else
	{
	...
	if (WinJudgeDobyELVPSSPreZME(pstDispInfo->eFmt, pstInfo->stOutRect, u32SrcOrgWidth, u32SrcOrgHeight))
	{
...
HI_S32 WinUpdateWinAttr(WINDOW_S *pstWin,const HI_DISP_DISPLAY_INFO_S *pstDispInfo)
{
    ...
    WinInRectAlign(&pstWin->stUsingAttr.stInRect);
...
HI_S32 WinCaptureFrame(HI_HANDLE hWin, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_U32 *stMMZPhyAddr, HI_U32 *stMMZlen)
{
    ...
    HI_DRV_VIDEO_FRAME_S *pstFrmTmp = NULL;
    HI_VDEC_PRIV_FRAMEINFO_S *pstPrivInfo = HI_NULL;
    HI_U32 datalen = 0, y_stride = 0, c_stride = 0, height = 0;
