...
static HI_VOID PTSREC_HandleValidSrcPTS(HI_HANDLE hHandle, IMAGE *pstImage)
{
    HI_U32 u32NumPass = 0;
    HI_U32 u32PtsPass = 0;
    ...
    /* CNcomment: ����������ЧPTS���PTS��ֵ��֡������StatPtsDelta */
	...
	if (u32NumPass)
	{
		...
		/**CNcomment: ֻ������2�ε�FrameTime������ͬ�Ż����CalcFrameTime*/
		...
		HI_INFO_PTSREC("pts %d exception\n", u32SrcPts);
	...
	{
	    HI_INFO_PTSREC("Frame index %d repeat\n", pstImage->frame_num);
	    s_stPtsRecChan[hHandle].s32InterPtsDelta = 20;
	}
    }
    else
...
static HI_U32 PTSREC_GetFrameTime_User(PTSREC_CHAN_INFO_S *pstPtsInfo, IMAGE *pstImage, HI_U8 bDolbyHDR)
{
...
static HI_U32 PTSREC_GetFrameTime_Other(PTSREC_CHAN_INFO_S *pstPtsInfo, IMAGE *pstImage, HI_U8 bDolbyHDR)
{
    HI_U32 u32FrameTime = HI_INVALID_TIME;
...
static HI_VOID PTSREC_HandleInValidSrcPTS(HI_HANDLE hHandle, IMAGE *pstImage, HI_U8 bDolbyHDR)
{
    HI_U32 u32NumPass = 0;
    HI_U32 u32FrameTime = HI_INVALID_TIME;
    HI_U32 u32Pts;
	...
	{
	    u32FrameTime = PTSREC_GetFrameTime_User(pstPtsInfo, pstImage, bDolbyHDR);
	}
	else /* HI_UNF_AVPLAY_FRMRATE_TYPE_USER_PTS ... */
	{
	    u32FrameTime = PTSREC_GetFrameTime_Other(pstPtsInfo, pstImage, bDolbyHDR);
	}
...
HI_U32 PTSREC_CalcStamp(HI_HANDLE hHandle, VDEC_CHANNEL_S *pstChan, IMAGE *pstImage)
{
    ...
    /* u32SrcPts is valid */
    if (HI_INVALID_PTS != (HI_U32)pstImage->SrcPts)
    {
	PTSREC_HandleValidSrcPTS(hHandle, pstImage);
    }
    /* u32SrcPts is invalid */
    else
    {
	PTSREC_HandleInValidSrcPTS(hHandle, pstImage, pstChan->stVdecHdrAttr.AvplayHDRAttr.bEnable);
    }
