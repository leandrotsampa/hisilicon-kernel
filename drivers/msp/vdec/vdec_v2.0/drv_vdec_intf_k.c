...
#define HI_VDEC_MAX_INSTANCE	16
...
/* VDECȫ�����ο����� bit0: DynamicFs Mask Ctrl; other: reserve*/
...
static HI_S32 BUFMNG_VPSS_Init(BUFMNG_VPSS_INST_S* pstBufVpssInst)
{
	...
	for (i = 0; i < pstBufVpssInst->u32BufNum; i++)
	{
	    ...
	    {
		//HI_VFREE_BUFMNG(pstBufNode);//����ʧ�ܲ����ͷţ���Ҫ����ǰ�Ķ��ͷ�
		...
		list_for_each_safe(pos, n, &(pstBufVpssInst->stVpssBufAvailableList))
		{
		    pstTarget = list_entry(pos, BUFMNG_VPSS_NODE_S, node);
		    HI_DRV_VDEC_UnmapAndRelease(&(pstTarget->VDECBuf_frm));
		    HI_DRV_VDEC_UnmapAndRelease(&(pstTarget->VDECBuf_meta));//DRM: meta�ڴ�Ϊ�ǰ��ڴ� c00277632
		    list_del_init(pos);
		    HI_VFREE_BUFMNG(pstTarget);
		}
...
static HI_S32 BUFMNG_VPSS_RecBuffer(HI_HANDLE hVpss, HI_HANDLE hPort, BUF_VPSS_S* pstMMZ_Buffer, HI_U32* pu32Stride)
{
    ...
    /*ɾ��stVpssBufAvailableList�ϵ�pos�ڵ�*/
    list_del_init(pos);
    pstTarget->bBufUsed = HI_TRUE;
    /*��pos�ڵ����ӵ�stVpssBufUnAvailableList��ȥ*/
    ...
    //�� ��ַΪ-1ʱ����ʾ�ϲ�δ����metabuf,�����ǰѽṹ�崫��vpss����vpss���ж�
...
static HI_S32 VDEC_Event_RatioNotZero(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    ...
    pstChan->stStatInfo.u32VdecDecErrFrame++;//ͳ�ƴ���֡��
...
static HI_S32 VDEC_Event_ReleaseFrameBufOnly(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
	...
	{
	    pstChan->stFrameBufferRecord[u32Index].bWaitRelease = HI_TRUE;    //�ڵ�֡��ȴ��ͷ�
	    break;
	}
    }
...
static HI_S32 VDEC_Chan_CreatePort(HI_HANDLE hVpss, HI_HANDLE* phPort, VDEC_PORT_ABILITY_E ePortAbility )
{
    ...
    /*CNcomment:����Ӧ���ҵ�hVpss��Ӧ���ǵڼ���astChanEntity*/
    for (i = 0; i < HI_VDEC_MAX_INSTANCE_NEW; i++)
    {
	if (hVpss == s_stVdecDrv.astChanEntity[i].stVpssChan.hVpss)
	{
    ...
    if (HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE == stVpssPortCfg.stBufListCfg.eBufType)
    {
	stVpssPortCfg.s32OutputWidth = 0;    // ������Ϊ0��ʾ������������Ӧ����
...
static HI_S32 VDEC_Chan_GetPortParam(HI_HANDLE hVpss, HI_HANDLE hPort, VDEC_PORT_PARAM_S* pstPortParam)
{
    ...
    /*��Ҫ�Ǹ�vo�ṩ��ȡ/�ͷ�֡��ĺ�����wininfo�ı�Ĵ�������,��ȡ֡��ĺ���ֻ��Ϊ�˺��ͷ����*/
				       ...
				       HI_DRV_VIDEO_FRAME_S* pstFrame)
{
	...
	if (0 == EnVcmp)
	{
	    switch (pstImage->format.vdh_2d_en)	 /*1d  or 2d*/
	    {
		case 0:
	...
	{
	    switch (pstImage->format.vdh_2d_en)
	    {
		case 0:
		    if (pstImage->format.isCompressed)
		    {
				      ...
				      HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    {
	if ((0 == EnVcmp) || (pstImage->format.isCompressed == 0))
	{
...
static HI_S32 VDEC_RlsSpecialFrame(VDEC_CHANNEL_S* pstChan)
{
	...
	if (DRV_VDEC_BUFFER_STATUS_PREALLOC == pstChan->stFrameBufferRecord[i].enFrameBufferNodeStatus) //Ԥ����Ľڵ㣬ͨ��Ԥ���亯���ͷ�
	{
	...
	if (DRV_VDEC_BUFFER_STATUS_PREALLOC == pstChan->stFrameBufferRecord[i].enHdrBufferNodeStatus) //Ԥ����Ľڵ㣬ͨ��Ԥ���亯���ͷ�
	{
...
//�˺����Ѿ����ã������о���,ֱ�Ӹɵ�,��������ģ����ܻ������е��ô˺����ĵط�Ϊ������뱨�����Ƚ������ڴ�ʵ�ָɵ�
...
static HI_S32 VDEC_Chan_InitParam(HI_HANDLE hHandle, HI_UNF_AVPLAY_OPEN_OPT_S* pstCapParam)
{
    ...
    pstChan->stOption.u32DynamicFrameStoreAllocEn = ((MaskCtrlWord & 0x1) == 1) ? 0 : s_stVdecDrv.astChanEntity[hHandle].u32DynamicFsEn; //l00273086 ʹ�ܶ�̬���书��
    pstChan->stOption.s32ExtraFrameStoreNum = PLUS_FS_NUM + 1;//l00273086 PLUS_FS_NUM + 1:�������ŷ�ֹ����
    pstChan->stOption.s32DelayTime = 0;//l00273086 ��̬����֡��ʱ�������䲻���ڴ�vfmw�ȴ���ʱ��
    pstChan->stOption.u32CfgFrameNum = 0;//l00273086 ����֡��
    pstChan->stOption.u32MaxMemUse = -1;//l00273086������֡��
    ...
    if (stMemConfig.u32TotalSize > 1536 && stMemConfig.u32TotalSize <= 2048)//2G�ڴ��½����������
    {
    ...
    else if (stMemConfig.u32TotalSize > 768 && stMemConfig.u32TotalSize <= 1024)//1G�ڴ��½����������
    {
    ...
    else if (stMemConfig.u32TotalSize > 512 && stMemConfig.u32TotalSize <= 768)//768M�ڴ��½����������
    {
    ...
    else if (stMemConfig.u32TotalSize > 256 && stMemConfig.u32TotalSize <= 512)//512M�ڴ��½����������
    {
    ...
    else if (stMemConfig.u32TotalSize <= 256)//256M�ڴ�����������
    {
    ...
    //��û��DFS��ʱ�򣬶���MVC����ֱ��ʹ��menuconfig�е�֡������ѡ��ᵼ����ЩMVC������֡�治�������ܲ��š������menuconfig�и��ģ��ֻᵼ��ÿ��ͨ�����������֡�棬������֡�治������������ղ�Ʒ����δ��DFSʱ�ڵײ�д����
...
static HI_S32 VDEC_Chan_GetMemSize(HI_HANDLE hHandle)
{
    ...
    /* Hevc�����ڴ湫ʽ��һ������GetMemSize����ǰ��ͨ������һ��HEVC��������������ʶ�� */
...
HI_S32 VDEC_Chan_AllocVFMWMem(HI_HANDLE hHandle)
{
	...
	if (pstChan->stMemSize.ScdDetailMem > 0)
	{
	    ...
	    /*pstChan->stSCDMMZBuf.u32SizeD�Ĵ�С���Ǵ�vfmw��ȡ�Ĵ�С:pstChan->stMemSize.ScdDetailMem*/
	...
	/* CNcomment: �ⲿ����vfmw�Լ����з��䣬scd��vdh���ڴ���vdec���з���*/
	pstChan->stOption.MemDetail.ChanMemCtx.Length  = 0;
	pstChan->stOption.MemDetail.ChanMemCtx.PhyAddr = 0;
	pstChan->stOption.MemDetail.ChanMemCtx.VirAddr = HI_NULL;
	...
	if (u32VDHSize > 0)
	{
	    ...
	    {
		if (1 == pstChan->stOption.u32DynamicFrameStoreAllocEn) //������ö�̬֡��ķ���,�������ﲻ��Ҫ������������ l00273086//
		{
		    pstChan->stVDHMMZBuf.u32Size = 0;
		    pstChan->stVDHMMZBuf.u32StartPhyAddr = HI_NULL;
		    pstChan->stVDHMMZBuf.pu8StartVirAddr = HI_NULL;
...
HI_S32 VDEC_Chan_FreeVFMWMem(HI_HANDLE hHandle)
{
    ...
    /*CN:�ͷŶ�̬������ڴ�,410��̬�ڴ�����vdec��������ģ�������vdec�����ͷ�*/
...
HI_S32 HI_DRV_VDEC_SetCtrlInfo(HI_HANDLE hHandle, HI_UNF_AVPLAY_CONTROL_INFO_S* pstCtrlInfo)
{
    ...
    //�øĶ�Ϊ��ʱ�汾����zhouzhong(z00111416)��ͨ�󣬺���PVR�����ϲ�����u32DispOptimizeFlag
    //��ʱ��Ҫɾ���˴�����
...
HI_S32 HI_DRV_VDEC_DecodeIFrame(HI_HANDLE hHandle, HI_UNF_AVPLAY_I_FRAME_S* pstStreamInfo,HI_BOOL bCapture, HI_BOOL bUserSpace)
{
    ...
    /* Here we invoke USE_UP function to release the atomic number, so that the VFMW can decode
      ...
      EVNT_NEW_IMAGE */
    /*CNcomment: �˴�����USE_UP�ͷ��� ��ʹ VFMW ���Ե��û�ȡ�����ӿ� ���н���
      VDEC_Event_Handle ���Դ��� EVNT_NEW_IMAGE �¼� */
    VDEC_CHAN_USE_UP(&s_stVdecDrv.astChanEntity[hHandle]);
...
static HI_S32 VDEC_Ioctl_CreateVpss(struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    if (s32Ret == HI_SUCCESS)
    {
	*((HI_HANDLE*)arg) = hOuthandle;  //��vpss�ľ�����أ����û�̬������ʹ��
    }
			     ...
			     FRAME_BUF_S   *pstFrameBuf)
{
	...
	if (HI_SUCCESS != s32Ret)
	{
	    ...
	    return; //��ֹFrame����ʧ�ܣ���������metadata��pmv�������ڴ��ֲ���������ȥ������metadata��pmv�޷��ͷš�
	}
	else
	{
	    //״̬ΪSYSALLOC��ʾ�˽ڵ��¼��֡����Ϣ��Ч�����ڴ����Զ�̬����
				...
				FRAME_BUF_S   *pstFrameBuf)
{
	...
	//hdr metadata��Ϣֱ�Ӵ�ϵͳ���涯̬���䣬��ΪvoҪ�ô�Ƭbufferӳ���û�̬�����ַ��������ݵ�������ַ����ֱ�Ӵ�ϵͳ�������Ļ�����ӳ��ʧ��
	...
	if (HI_SUCCESS != s32Ret)
	{
	    ...
	    return;//��ֹmetadata����ʧ�ܣ���������pmv�������ڴ��ֲ���������ȥ������frame��pmv�޷��ͷš�
	}
	else
	{
	...
	//��¼metadata��Ϣ���ֲ�����stFrameBuf�У����ø�vfmw
				...
				FRAME_BUF_S   *pstFrameBuf)
{
    ...
    //���pmv��buffer�Ѿ���������򲻼������룬pmv��buffer����Ϊ�ο�֡+1����֡��buffer�ĸ�����
			       ...
			       FRAME_BUF_S   *pstFrameBuf)
{
    ...
    //ͬʱ���䵽frame,pmv,hdr�ڵ�ž����ø�vfmw,�������pmv�ڵ��Ѿ����ù��ˣ��˴���������Ҫ����pmv�ڵ�
...
static SINT32 VdecTaskFunc(VOID* param)	  //l00273086
{
    ...
    u32TaskRunFlag = 1;//��ʶTask��ʼ��ת����ֹtask��û�п�ʼrun�ͱ�ǿ��close���¹�ס
