...
HI_S32 DRV_HIFB_FenceInit(HIFB_PAR_S *par)
{
    ...
    gs_SyncInfo.pstTimeline = hi_sync_timeline_create("hifb");
...
HI_VOID DRV_HIFB_FenceDInit(HIFB_PAR_S *par)
{
    ...
    hi_sync_timeline_destroy((struct hi_sync_timeline*)gs_SyncInfo.pstTimeline);
...
HI_VOID DRV_HIFB_FENCE_FrameEndCallBack(HI_VOID)
{
	...
	hi_sync_timeline_signal(gs_SyncInfo.pstTimeline, 1);
...
HI_S32 DRV_HIFB_FENCE_Create(HI_VOID)
{
    ...
    pt = hi_sync_pt_create(gs_SyncInfo.pstTimeline, sizeof(*pt), u32FenceValue);
    if (NULL == pt)
    {
	return -ENOMEM;
    }
