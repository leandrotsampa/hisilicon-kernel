...
* others:	: NA
***************************************************************************************/
static HIFB_COLOR_FMT_E hifb_getfmtbyargb(struct fb_bitfield *red,
...
* description	: CNcomment: ���·����ڴ棬���ڴ��ͷŴ���,�������ԭ����Ϊ��
			     ��֤����ڴ治�ٱ��߼�ʹ�õ�ʱ����ͷţ������Ͳ��ᷢ������ҳ������
			     CNend\n
* param[in]	: HI_VOID
* retval	: NA
* others:	: NA
...
* description	: CNcomment: ����canvas buffer ��Ϣ CNend\n
* param[in]	: HI_VOID
* retval	: NA
* others:	: NA
***************************************************************************************/
...
static HI_S32 hifb_vo_callback(HI_VOID *pParaml, HI_VOID *pParamr)
{
    ...
    do_gettimeofday(&tv);
    NowTimeMs = tv.tv_sec * 1000 + tv.tv_usec / 1000;
    if ((NowTimeMs - pstPar->stFrameInfo.u32StartTimeMs) >= 1000)
    {
	pstPar->stFrameInfo.u32StartTimeMs = NowTimeMs;
    ...
    g_stDrvAdpCallBackFunction.HIFB_DRV_SetOutVoCallBackScanLine(*pu32LayerId);
...
static HI_S32 hifb_refresh_0buf_3D(HI_U32 u32LayerId, HIFB_BUFFER_S *pstCanvasBuf)
{
    ...
    /**<--alloc stereo buffer >**/
    Ret = HIFB_OSR_ReAllocStereoBuf(u32LayerId, BufferSize);
    if (HI_SUCCESS != Ret)
    {
	HIFB_ERROR("===func : %s line : %d error\n",__FUNCTION__,__LINE__);
	return Ret;
...
static HI_S32 hifb_refresh_1buf_3D(HI_U32 u32LayerId, HIFB_BUFFER_S *pstCanvasBuf)
{
    ...
    /**2
     ** if display address is not the same as inital address,
     ** please config it use old address,�����?��ĵ�ַ���ǳ�ʼ���ĵ�ַ���л���ʾ�???
     **/
...
static HI_S32 DRV_HIFB_RefreshUserBuffer(HI_U32 u32LayerId)
{
    ...
    {/**<-- has user buffer >**/
	HI_GFX_Memset(&stCanvas, 0x0, sizeof(HIFB_BUFFER_S));
...
static HI_S32 HIFB_OSR_SetColorKey(struct fb_info *info, HI_ULONG arg)
{
	...
	/**
		**ARGB8888: RLen = 8   GLen = 8	  BLen = 8
		**RGB888:     RLen = 8	 GLen = 8   BLen = 8
		**RGB565:     RLen = 5	 GLen = 6   BLen = 5
		**ARGB1555: RLen = 5   GLen = 5	  BLen = 5
		**/
...
static HI_S32 DRV_OSR_StereoCallBack(HI_VOID * pParaml,HI_VOID * pParamr)
{
	...
	if (HIFB_REFRESH_MODE_WITH_PANDISPLAY == pstPar->stExtendInfo.enBufMode)
	{
	    ...
	    Ret = HIFB_OSR_ReAllocStereoBuf(pstPar->stBaseInfo.u32LayerID, BufferSize);
...
* description	:
		  CNcomment: ���ں�̬�����smmu��ַӳ���û�̬�������ַ��
			     Ҫ��MMZʹ���ں�̬��׼ʵ�֣�Ҫ��SMMU������Ҫ�Լ���ӳ��ҳ�� CNend\n
* retval	:
* others:	: NA
****************************************************************************/
static HI_S32 hifb_map_mem_kernel_to_user(struct fb_info *info, struct vm_area_struct *vma)
...
#if defined(CFG_HIFB_FENCE_SUPPORT) && defined(CFG_HIFB_STEREO3D_HW_SUPPORT)
    HIFB_CHECK_EQUAL_RETURN(HI_FALSE, pstPar->st3DInfo.IsStereo, HI_SUCCESS);
