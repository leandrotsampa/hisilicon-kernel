...
{
    VFMW_UC_EXT_OpenModule();	 //VDEC_OpenModule();
...
VOID VFMW_ExitModule(VOID)
{
    ...
    VFMW_UC_EXT_CloseModule(); //VDEC_ExitModule();
...
SINT32 VFMW_Control(SINT32 ChanID, VDEC_CID_E eCmdID, VOID *pArgs, UINT32 ParamLength)
{
	...
	{
	    Ret = VFMW_UC_EXT_Control(ChanID, eCmdID, pArgs, ParamLength);
	}
...
#endif
    {
	ret = VFMW_UC_EXT_GetImage(ChanID, pImage);
...
#endif
    {
	ret = VFMW_UC_EXT_ReleaseImage(ChanID, pImage);
    }
