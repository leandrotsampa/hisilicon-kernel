...
SINT32 OSAL_CreateTask( VOID *pParam, CHAR TaskName[], VOID *pTaskFunction )
{
...
SINT32 OSAL_WaitEventCondition(OSAL_EVENT* pEvent, SINT32 condition, SINT32 msWaitTime)
{
    SINT32 l_ret;
...
SINT32 OSAL_WaitEvent(OSAL_EVENT* pEvent, SINT32 msWaitTime)
{
    ...
    if (l_ret == -ERESTARTSYS)
    {
	dprint(PRN_ALWS, "%s: wake up by interrupt!\n",__func__);
    }

    return (l_ret <= 0) ? OSAL_ERR: OSAL_OK;
}
...
SINT32 KernelMemMalloc(CHAR *MemName, UINT32 len, UINT32 align, UINT32 IsCached, MEM_DESC_S *pMemDesc)
{
    ...
    pMemDesc->PhyAddr = (SINT32)stSMMUBuffer.u32StartSmmuAddr;
    ...
    pMemDesc->MmuAddr = (SINT32)stMmzBuffer.u32StartPhyAddr;
...
SINT32 KernelMemFree(MEM_DESC_S *pMemDesc)
{
    ...
    stSMMUBuf.u32StartSmmuAddr = (UINT32)pMemDesc->MmuAddr;
...
SINT32 Osal_SpinLockInit(SpinLockType spinLockType)
{
    OSAL_IRQ_SPIN_LOCK *pOsalSpinLock = NULL;

    pOsalSpinLock = GetSpinLockByEnum(spinLockType);
    if (pOsalSpinLock == NULL)
    {
	dprint(PRN_ERROR,"Get spin lock fail!\n");
	return OSAL_ERR;
    }
...
SINT32 Osal_WaitWaitQue(MutexType mutexType, SINT32 condition, SINT32 waitTimeInMs)
{
	...
	case G_INTWAITQUEUE:
	    retVal = OSAL_WaitEventCondition(&g_WaitScdStopWorking, condition, waitTimeInMs);
	    break;
