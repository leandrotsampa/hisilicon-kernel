...
/*=============== EXTERN VALUE =================*/
extern VDH_NORMAL_CTX_S	 NormalCtx;
...
static VOID GetWaitTime(TASK_ARRAY_S *pTaskArray, TASK_MEM_S *pTaskMem, UINT32 *pWaitTime)
{
	...
	{
	    *pWaitTime = OSAL_GetTimeInMs() - pTaskMem->Task.StartTime;
	}
    }
}

static SINT32 GetUserTask(TASK_PARAM_S *pTaskParam, TASK_ARRAY_S *pUserTaskArray, TASK_MEM_S *pUserTaskMem, UINT32 SecureWaitTime)
{
    UINT32 UserWaitTime = 0;
    SINT32 ret = VDH_ERR;
    pUserTaskMem = &pUserTaskArray->Task[pUserTaskArray->Tail];
	if (1 == pUserTaskMem->InUsed)
	{
	    if (0 == pUserTaskMem->Task.StartTime)
	    {
		pUserTaskMem->Task.StartTime = OSAL_GetTimeInMs();
		UserWaitTime = 1;
	    }
	    else
	    {
		UserWaitTime = OSAL_GetTimeInMs() - pUserTaskMem->Task.StartTime;
	    }

	    if (UserWaitTime > SecureWaitTime)
	    {
		memcpy(pTaskParam, &pUserTaskMem->Task, sizeof(TASK_PARAM_S));
		pUserTaskMem->InUsed	= 0;
		pUserTaskArray->TaskNum = (pUserTaskArray->TaskNum-1 > 0)? (pUserTaskArray->TaskNum-1): 0;
		pUserTaskArray->Tail	= (pUserTaskArray->Tail+1) % MAX_TASK_NUM;
		ret = VDH_OK;
	    }
	}
...
static SINT32 GetKernelTask(TASK_PARAM_S *pTaskParam, TASK_LIST_S *pTaskList, TASK_NODE_S *pTaskNode, UINT32 SecureWaitTime, UINT32 *pNormalWaitTime)
{
	...
	if (1 == pTaskNode->InUsed)
	{
		...
		*pNormalWaitTime = 1;
	    ...
	    {
		*pNormalWaitTime = OSAL_GetTimeInMs() - pTaskNode->Task.StartTime;
	    }
	    if (*pNormalWaitTime > SecureWaitTime)
	    {
...
static SINT32 GetSecureTask(TASK_PARAM_S *pTaskParam, TASK_ARRAY_S *pSecureTaskArray, TASK_MEM_S *pSecureTaskMem)
{
    SINT32 ret = VDH_ERR;
    pSecureTaskMem = &pSecureTaskArray->Task[pSecureTaskArray->Tail];
    if (1 == pSecureTaskMem->InUsed)
    {
	memcpy(pTaskParam, &pSecureTaskMem->Task, sizeof(TASK_PARAM_S));
	pSecureTaskMem->InUsed	  = 0;
	pSecureTaskArray->TaskNum = (pSecureTaskArray->TaskNum-1 > 0)? (pSecureTaskArray->TaskNum-1): 0;
	pSecureTaskArray->Tail	  = (pSecureTaskArray->Tail+1) % MAX_TASK_NUM;
	ret = VDH_OK;
    }
...
SINT32 VDH_GetTask_To_Process(MODULE_TYPE_E Type, SINT32 ModuleID, TASK_PARAM_S *pTaskParam)
{
    ...
    TASK_ARRAY_S   *pSecureTaskArray = NULL;
    TASK_MEM_S	   *pSecureTaskMem   = NULL;
    TASK_ARRAY_S   *pUserTaskArray   = NULL;
    TASK_MEM_S	   *pUserTaskMem   = NULL;

    switch(Type)
    {
	case MODULE_SCD:
	    pTaskList  = &NormalCtx.NormalScdTask[ModuleID];
	    pSecureTaskArray = &NormalCtx.pCommonContext->SecureScdArray[ModuleID];
	    pUserTaskArray  = &NormalCtx.pCommonContext->UserScdArray[ModuleID];
	    break;

	case MODULE_MFDE:
	    pTaskList  = &NormalCtx.NormalMfdeTask[ModuleID];
	    pSecureTaskArray = &NormalCtx.pCommonContext->SecureMfdeArray[ModuleID];
	    pUserTaskArray  = &NormalCtx.pCommonContext->UserMfdeArray[ModuleID];
	    break;
    ...
    if (pSecureTaskArray->Tail != pSecureTaskArray->Head)
    {
	GetWaitTime(pSecureTaskArray, pSecureTaskMem, &SecureWaitTime);
    }

    if (pUserTaskArray->Tail != pUserTaskArray->Head)
    {

	ret = GetUserTask(pTaskParam, pUserTaskArray, pUserTaskMem, SecureWaitTime);
    }

    OSAL_WriteLockIRQ(&pTaskList->TaskLock);
    if (!OSAL_LIST_EMPTY(&pTaskList->TaskList))
    {
       ret = GetKernelTask(pTaskParam, pTaskList, pTaskNode, SecureWaitTime, &NormalWaitTime);
    }
	...
	if (pSecureTaskArray->Tail != pSecureTaskArray->Head)
	{
	    ret = GetSecureTask(pTaskParam, pSecureTaskArray, pSecureTaskMem);
	}
    }
...
VOID VDH_Process_ScdTask(SINT32 ModuleID)
{
    ...
    else if (USER_SCD_TASK == TaskParam.Type)
    {
	pModule->UserStartCount++;
	pModule->ModuleState = MODULE_USER_WORKING;
    }
...
VOID VDH_Process_MfdeTask(SINT32 ModuleID)
{
    ...
    else if (USER_MFDE_TASK == TaskParam.Type)
    {
	pModule->UserStartCount++;
	pModule->ModuleState = MODULE_USER_WORKING;
    }
...
VOID VDH_DSPCode_Management(SINT32 ModuleID)
{
	...
	{
	    pRequire = &pCtx->Dsp_Code[ModuleID].UserRequire;
	...
	{
	    /* dsp already in used */
	    dprint(PRN_ERROR, "%s %s failed: dsp already in used, require %d, code type %d!\n", __func__, (0==i)?"normal or user":"secure", *pRequire, pCtx->Dsp_Code[ModuleID].Code);
	    return;
	}
	else
	{
...
VOID VDH_Module_Management(VOID)
{
	...
	if (1 == pCtx->Dsp_Module[i].NormalNeed || 1 == pCtx->Dsp_Module[i].SecureNeed || 1 == pCtx->Dsp_Module[i].UserNeed)
	{
    ...
    for (i=0; i<SCD_MODULE_NUM; i++)
    {
	if (1 == pCtx->Scd_Module[i].NormalNeed || 1 == pCtx->Scd_Module[i].SecureNeed || 1 == pCtx->Scd_Module[i].UserNeed)
	{
    ...
    for (i=0; i<MFDE_MODULE_NUM; i++)
    {
	if (1 == pCtx->Mfde_Module[i].NormalNeed || 1 == pCtx->Mfde_Module[i].SecureNeed || 1 == pCtx->Mfde_Module[i].UserNeed)
	{
    ...
    for (i=0; i<BPD_MODULE_NUM; i++)
    {
	if (1 == pCtx->Bpd_Module[i].NormalNeed || 1 == pCtx->Bpd_Module[i].SecureNeed || 1 == pCtx->Bpd_Module[i].UserNeed)
	{
...
VOID VDH_Check_WakeUpReason(SINT32 type)
{
	...
	if (IS_WAKE_UP_BY(type, WAKE_UP_USER_SCD_TASK))
	{
	    dprint(PRN_ERROR, "Distributor wake up by user scd task, Type = %d\n", type);
	}
	if (IS_WAKE_UP_BY(type, WAKE_UP_USER_MFDE_TASK))
	{
	    dprint(PRN_ERROR, "Distributor wake up by user mfde task, Type = %d\n", type);
	}
...
SINT32 VDH_Read_Proc(struct seq_file *p, VOID *v)
{
	...
	seq_printf(p, "Normal Need	:%-7d  | Secure Need	     :%d  |  User Need	      :%-7d\n", pCtx->Scd_Module[i].NormalNeed,
									      pCtx->Scd_Module[i].SecureNeed, pCtx->Scd_Module[i].UserNeed);
	seq_printf(p, "N_Start_Count	:%-7d  | S_Start_Count	     :%d  |  U_Start_Count    :%-7d\n", pCtx->Scd_Module[i].NormalStartCount,
									      pCtx->Scd_Module[i].SecureStartCount, pCtx->Scd_Module[i].UserStartCount);
	seq_printf(p, "N_Task_Num	:%-7d  | S_Task_Num	     :%d  |  U_Task_Num	      :%-7d\n", NormalCtx.NormalScdTask[i].TaskNum,
									      pCtx->SecureScdArray[i].TaskNum, pCtx->UserScdArray[i].TaskNum);
	seq_printf(p, "\n");
	}
	...
	seq_printf(p, "Normal Need	:%-7d  | Secure Need	     :%d  |  User Need	      :%-7d\n", pCtx->Mfde_Module[i].NormalNeed,
									      pCtx->Mfde_Module[i].SecureNeed, pCtx->Mfde_Module[i].UserNeed);
	seq_printf(p, "N_Start_Count	:%-7d  | S_Start_Count	     :%d  |  U_Start_Count    :%-7d\n", pCtx->Mfde_Module[i].NormalStartCount,
									      pCtx->Mfde_Module[i].SecureStartCount, pCtx->Mfde_Module[i].UserStartCount);
	seq_printf(p, "N_Task_Num	:%-7d  | S_Task_Num	     :%d  |  U_Task_Num	      :%-7d\n", NormalCtx.NormalMfdeTask[i].TaskNum,
									      pCtx->SecureMfdeArray[i].TaskNum, pCtx->UserMfdeArray[i].TaskNum);
	seq_printf(p, "\n");
	}
	...
	seq_printf(p, "Normal Need	:%-7d  | Secure Need	     :%d  |  User Need	    :%-7d\n", pCtx->Bpd_Module[i].NormalNeed,
									     pCtx->Bpd_Module[i].SecureNeed, pCtx->Bpd_Module[i].UserNeed);
	seq_printf(p, "N_Start_Count	:%-7d  | S_Start_Count	     :%d  |  U_Start_Count    :%-7d\n", pCtx->Bpd_Module[i].NormalStartCount,
									     pCtx->Bpd_Module[i].SecureStartCount, pCtx->Bpd_Module[i].UserStartCount);
	seq_printf(p, "\n");
	}
	...
	seq_printf(p, "Normal Need	:%-7d  | Secure Need	     :%d  |  User Need	    :%-7d\n", pCtx->Dsp_Module[i].NormalNeed,
									     pCtx->Dsp_Module[i].SecureNeed, pCtx->Dsp_Module[i].UserNeed);
	seq_printf(p, "\n");
	}
...
VOID VDH_User_WakeUpDistributor(VOID)
{
    ...
    if (pCtx->UserWakeUp != WAKE_UP_NOTHING)
    {
	OSAL_GiveEvent_l(&NormalCtx.ThreadEvent, (UINT32)pCtx->UserWakeUp);
	pCtx->UserWakeUp = WAKE_UP_NOTHING;
    }
