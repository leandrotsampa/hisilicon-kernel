...
static struct __hisi_iommu_domain *to_hisi_domain(struct iommu_domain *dom)
{
	if (!dom)
		return NULL;

	return container_of(dom, struct __hisi_iommu_domain, domain);
}
...
{
#ifdef DEBUG
	struct __hisi_iommu_domain *hisi_domain = to_hisi_domain(domain);
...
static struct iommu_domain *hisi_domain_alloc(unsigned type)
{
	...
	hisi_domain->domain.capablity.iova_start = HISI_IOMMU_IOVA_START;
	hisi_domain->domain.capablity.iova_end   = HISI_IOMMU_IOVA_END;
	hisi_domain->domain.capablity.iova_align = HISI_IOVA_POOL_ALIGN;
	hisi_domain->domain.capablity.pg_sz = PAGE_SIZE;
	hisi_domain->domain.capablity.pgtbl_base = hisi_iommu_ptable_addr;
	hisi_domain->domain.capablity.off_on = true;
...
{
	struct __hisi_iommu_domain *hisi_domain = NULL;
