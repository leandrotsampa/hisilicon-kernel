...
HI_VOID HAL_HDMI_Debug(HDMI_HAL_S *pstHdmiHal, HDMI_HAL_DEBUG_CMD_E enDebugCmd,HI_VOID *pData)
{
    ...
    volatile HI_U32	 u32RegVal = 0;
    volatile HI_U32	 *pu32BaseAddr = HI_NULL;
	...
	case HDMI_DEBUG_CMD_DITHER:
	    HAL_NULL_CHK_NO_RET(pData);
	    pu32BaseAddr = (uint32_t *)HDMI_IO_MAP(HDMI_CTRL_BASE_ADDR + 0x3710, 4);//Dither mode,reg 0x3dc4 bit[1:0]
	    u32RegVal = HdmiRegRead(pu32BaseAddr);
	    u32RegVal &= 0xfffffffc;
	    u32RegVal |= (*(HDMI_VIDEO_DITHER_E *)pData) & 0x00000003;
	    HdmiRegWrite(pu32BaseAddr,u32RegVal);
	    HDMI_IO_UNMAP(pu32BaseAddr);
	    break;
...
HI_S32 HAL_HDMI_AudioEnable(struct hiHDMI_HAL_S *pstHdmiHal, HDMI_AUDIO_INTERFACE_E enAoIntf,HI_BOOL bEnable)
{
