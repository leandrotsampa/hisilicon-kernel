...
static struct hiclk_hw hiclks_hw[] = {
	...
	CLK(PERI_CRG52_SF, 0x0003, 0x0003, BIT(3), 0, NULL),
	CLK_SHARED(HIGMAC_MAC0_CLK, PERI_CRG51_GSF, 0x2, 0x2, 0, 0, NULL),
	CLK_SHARED(HIGMAC_MAC_IF0_CLK, PERI_CRG51_GSF, 0x8, 0x8, 0, 0, NULL),
...
static unsigned long hiclk_recalc_rate_hi3798mv200(struct clk_hw *hw,
...
static int hiclk_set_rate_hi3798mv200(struct clk_hw *hw, unsigned long drate,
				unsigned long parent_rate)
{
	u32 val, ix;
	struct hiclk_hw *clk = to_hiclk_hw(hw);
...
static int hiclk_get_phase_hi3798mv200(struct clk_hw *hw)
{
	u32 val = 0;
	struct hiclk_hw *clk = to_hiclk_hw(hw);
...
static int hiclk_set_phase_hi3798mv200(struct clk_hw *hw, int degrees)
{
	u32 val;
	struct hiclk_hw *clk = to_hiclk_hw(hw);
...
static int hiclk_enable_hi3798mv200(struct clk_hw *hw)
{
	hiclk_enable(hw);
	hiclk_recalc_rate_hi3798mv200(hw, 0);
	return 0;
}
/******************************************************************************/

static struct clk_ops clk_ops_hi3798mv200 = {
	.enable = hiclk_enable_hi3798mv200,
	.recalc_rate = hiclk_recalc_rate_hi3798mv200,
	.set_rate = hiclk_set_rate_hi3798mv200,
	.get_phase = hiclk_get_phase_hi3798mv200,
	.set_phase = hiclk_set_phase_hi3798mv200,
};
/******************************************************************************/

static void __init hi3798mv200_clocks_init(struct device_node *np)
{
	int ix;
	...
	for (ix = 0; ix < ARRAY_SIZE(hiclks_hw); ix++) {
		...
		if (!ops->enable)
			ops->enable = hiclk_enable_hi3798mv200;
		if (!ops->recalc_rate)
			ops->recalc_rate = hiclk_recalc_rate_hi3798mv200;
	}

	hiclk_clocks_init(np, hiclks_hw, ARRAY_SIZE(hiclks_hw),
		CLK_MAX >> 2, &clk_ops_hi3798mv200);
}
CLK_OF_DECLARE(fixed_clock, "hi3798mv200.clock", hi3798mv200_clocks_init);
