...
#else
		printk("flush_inner_cache: mmz_flush_dcache_area not used!!\n");
#endif
}
...
#else
			printk("__dma_clear_buffer: mmz_flush_dcache_area(ptr=%p, length=%u) not used!!\n", ptr, length);
#endif
		}
	} else {
		on_each_cpu((smp_call_func_t)mmz_flush_dcache_all, NULL, 1);
...
#else
		printk("hil_mmb_unmap: __flush_dcache_area not used!!!\n");
#endif
		down(&mmz_lock);
	}
...
int mmb_put(u32 addr, int iommu)
{
	...
	if (mmb->phy_ref > 0) {
		...
		HI_PRINT("%s : phy_ref is wrong:ref:%d\n", __func__,
						mmb->phy_ref);
		up(&mmz_lock);
		return HI_FAILURE;
	}
...
static char setup_zones[MMZ_SETUP_CMDLINE_LEN] = "ddr,0,0,160M";
