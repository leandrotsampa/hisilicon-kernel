...
#else
		printk("TODO: mmz_flush_dcache_area: udata->uvirt=%p, mmb->length=%u\n", (void *)udata->uvirt, (size_t)mmb->length);
#endif
		flush_outer_cache_range(phyaddr, mmb->length, iommu);
	}
	...
	while(mmb->sec_smmu_ref) {
		if (mmb->sec_flag) {
			if (mmb->iommu){
				...
				if  (!ret) {
					/* just warning	  */
					HI_PRINT("%s, %d: secmem_free failed! \
						 \n", __func__, __LINE__);
				}
			} else {
		    ...
		    if (0 != ret) {
				HI_PRINT("%s: secmem_unmap_from_secsmmu failed! \n", __func__);
				return;
			}
		}
	}
	...
	if (!mmb->iommu) {
		while (mmb->cma_smmu_ref) {
			ret = hil_mmb_cma_unmapfrom_iommu(mmb->iommu_addr,
							  !mmb->iommu);
			if (ret) {
				HI_PRINT("%s  %d unmap iommu failed!\n",
							__func__, __LINE__);
				return;
			}
		}
