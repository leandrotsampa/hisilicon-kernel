...
HI_S32 AVPLAY_SetWinSource(AVPLAY_S *pAvplay, HI_BOOL bEnable)
{
    HI_S32                      Ret;
    HI_BOOL                     bWinRun;
    ...
    Ret = HI_MPI_WIN_GetEnable(pAvplay->MasterFrmChn.hWindow, &bWinRun);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("HI_MPI_WIN_GetEnable, Ret=0x%x\n", Ret);
        return HI_ERR_AVPLAY_INVALID_OPT;
    }

    if (HI_TRUE == bWinRun)
    {
        Ret = HI_MPI_WIN_SetEnable(pAvplay->MasterFrmChn.hWindow, HI_FALSE);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("HI_MPI_WIN_SetEnable, Ret=0x%x\n", Ret);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
    }
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        ...
        stSrcInfo.hSrc          = pAvplay->hBlPort;
        stSrcInfo.hSecondSrc    = bEnable ? pAvplay->hElPort : HI_INVALID_HANDLE;

        Ret = HI_MPI_WIN_SetSource(pAvplay->MasterFrmChn.hWindow, &stSrcInfo);
        if (HI_SUCCESS != Ret)
        {
    ...
    {
        stSrcInfo.enSrcMode     = HI_ID_VDEC;
        stSrcInfo.hSrc          = bEnable ? pAvplay->hBlPort : HI_INVALID_HANDLE;
        stSrcInfo.hSecondSrc    = HI_INVALID_HANDLE;

        Ret = HI_MPI_WIN_SetSource(pAvplay->MasterFrmChn.hWindow, &stSrcInfo);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("ERR: HI_MPI_WIN_SetSource, Ret=0x%x\n", Ret);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
    }
    ...
    if (bWinRun == HI_TRUE)
    {
        Ret = HI_MPI_WIN_SetEnable(pAvplay->MasterFrmChn.hWindow, HI_TRUE);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("ERR: HI_MPI_WIN_SetEnable, Ret=0x%x\n", Ret);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
    }
...
HI_S32 AVPLAY_StartHDRChn(const AVPLAY_S *pAvplay, AVPLAY_HDR_CHAN_E HDRChn)
{
    HI_S32                  Ret = HI_SUCCESS;
...
HI_S32 AVPLAY_StopHDRChn(AVPLAY_S *pAvplay, AVPLAY_HDR_CHAN_E HDRChn)
{
    HI_S32                  Ret = HI_SUCCESS;
...
HI_S32 AVPLAY_SetHDRPortAttr(AVPLAY_S *pAvplay, AVPLAY_HDR_CHAN_E HDRChn, VDEC_PORT_TYPE_E enType)
{
    HI_S32                      Ret;
...
HI_S32 AVPLAY_CreateHDRPort(AVPLAY_S *pAvplay, AVPLAY_HDR_CHAN_E HDRChn, VDEC_PORT_ABILITY_E enAbility)
{
    HI_S32             Ret = HI_SUCCESS;
...
HI_S32 AVPLAY_DestroyHDRPort(AVPLAY_S *pAvplay, AVPLAY_HDR_CHAN_E HDRChn)
{
    HI_S32                      Ret = HI_SUCCESS;
    ...
    if (HDRChn & ((HI_U32)AVPLAY_HDR_CHAN_BL))
    {
        Ret = HI_MPI_VDEC_DestroyPort(pAvplay->hVdec, pAvplay->hBlPort);
        if (HI_SUCCESS != Ret)
        {
...
HI_S32 AVPLAY_SetHDR(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_HDR_ATTR_S *pstAttr)
{
    ...
    if (pstAttr->bEnable == HI_TRUE)
    {
        ...
        {
            if (pstAttr->enElType >= HI_UNF_VCODEC_TYPE_BUTT)
            {
                HI_ERR_AVPLAY("vcodec type %d is invalid.\n", pstAttr->enElType);
                return HI_ERR_AVPLAY_INVALID_PARA;
            }

            if (pstAttr->stElCodecOpt.enDecType >= HI_UNF_VCODEC_DEC_TYPE_BUTT)
            {
                HI_ERR_AVPLAY("enDecType %d is invalid.\n", pstAttr->stElCodecOpt.enDecType);
                return HI_ERR_AVPLAY_INVALID_PARA;
            }

            if (pstAttr->stElCodecOpt.enCapLevel >= HI_UNF_VCODEC_CAP_LEVEL_BUTT)
            {
                HI_ERR_AVPLAY("enCapLevel %d is invalid.\n", pstAttr->stElCodecOpt.enCapLevel);
                return HI_ERR_AVPLAY_INVALID_PARA;
            }

            if (pstAttr->stElCodecOpt.enProtocolLevel >= HI_UNF_VCODEC_PRTCL_LEVEL_BUTT)
            {
            ...
            if (pAvplay->hElVdec == HI_INVALID_HANDLE)
            {
                Ret = AVPLAY_AllocHDRChn(pAvplay, AVPLAY_HDR_CHAN_BL | AVPLAY_HDR_CHAN_EL);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_AllocHDRChn fail.\n");
                    return Ret;
                }
                ...
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_SetHDRChnAttr fail.\n");
    ...
    else if (pstAttr->bEnable == HI_FALSE)
    {
       ...
       {
            Ret = AVPLAY_SetWinSource(pAvplay, HI_FALSE);
            if (Ret != HI_SUCCESS)
            {
                return Ret;
            }

            //HDRPlay channel disable, window resume normal mode
            ...
            pAvplay->bDualChnPlay       = HI_FALSE;
            pAvplay->MasterFrmChn.hPort = pAvplay->hBlPort;
            pAvplay->hBlPort            = HI_INVALID_HANDLE;
       }

       if (pAvplay->AvplayHDRAttr.enHDRStreamType == HI_UNF_AVPLAY_HDR_STREAM_TYPE_SL_VES)
       {
           VdecHDRAttr.AvplayHDRAttr.bEnable = HI_FALSE;
           VdecHDRAttr.enVdecHDRType = HI_VDEC_HDR_CHAN_TYPE_BUTT;
            Ret = HI_MPI_VDEC_SetHDRAttr(pAvplay->hVdec, &VdecHDRAttr);
...
HI_VOID AVPLAY_ProcVdecToVo(AVPLAY_S *pAvplay)
{
    ...
    if ( (pAvplay->FrcCurPlayCnt < pAvplay->FrcNeedPlayCnt) || (0 == pAvplay->FrcNeedPlayCnt) )
    {
...
HI_S32 AVPLAY_FilterIFrame(AVPLAY_S *pAvplay, HI_U8 *Buf, HI_U32 Size)
{
    ...
    list_for_each_safe(pos, temp, &KeepFrameList)
    {
        ...
        if (pos->next != &KeepFrameList)
        {
            ...
            pCtx->pNalAddr = pCurEntry->Pos + sizeof(g_StartCode);
            pCtx->NalLen   = (HI_SIZE_T)(pNextEntry->Pos - pCtx->pNalAddr);
    ...
    if (pAvplay->bHDRPlay == HI_TRUE)
    {
        if (pAvplay->bDualChnPlay == HI_TRUE)
        {
            if (HI_INVALID_HANDLE == pAvplay->hElPort)
            {
                ...
                Ret = AVPLAY_CreateHDRPort(pAvplay, AVPLAY_HDR_CHAN_EL, VDEC_PORT_HD);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_CreateHDRPort failed\n");
                    return Ret;
                }

                Ret = AVPLAY_SetHDRPortAttr(pAvplay, AVPLAY_HDR_CHAN_EL, VDEC_PORT_TYPE_MASTER);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_SetHDRPortAttr failed\n");
                    (HI_VOID)AVPLAY_DestroyHDRPort(pAvplay, AVPLAY_HDR_CHAN_EL);
                    return Ret;
                }

                Ret = HI_MPI_WIN_SetMode(pAvplay->MasterFrmChn.hWindow, HI_DRV_WINDOW_MODE_DOLBY);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("HI_MPI_WIN_SetMode failed\n");
                    (HI_VOID)AVPLAY_DestroyHDRPort(pAvplay, AVPLAY_HDR_CHAN_EL);
                    return Ret;
                }

                Ret = AVPLAY_SetWinSource(pAvplay, HI_TRUE);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_SetWinSource failed\n");
                    (HI_VOID)AVPLAY_DestroyHDRPort(pAvplay, AVPLAY_HDR_CHAN_EL);
                    return Ret;
                }

                Ret = AVPLAY_StartHDRChn(pAvplay, AVPLAY_HDR_CHAN_BL | AVPLAY_HDR_CHAN_EL);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_StartHDRChn failed\n");
                    (HI_VOID)AVPLAY_DestroyHDRPort(pAvplay, AVPLAY_HDR_CHAN_EL);
                    return Ret;
                }
            }
            else
            {
                ...
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_AVPLAY("AVPLAY_StartHDRChn fail.\n");
                    return Ret;
                }
            }
        }
        else
        {
            ...
            Ret = AVPLAY_StartHDRChn(pAvplay, AVPLAY_HDR_CHAN_BL);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("AVPLAY_StartHDRChn failed.\n");
                pAvplay->hBlPort = HI_INVALID_HANDLE;
...
static HI_S32 AVPLAY_StopVidChn(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_STOP_MODE_E enMode)
{
    ...
    if (HI_TRUE == pAvplay->bHDRPlay)
    {
        ...
        if (pAvplay->bDualChnPlay)
        {
            Ret = AVPLAY_StopHDRChn(pAvplay, AVPLAY_HDR_CHAN_BL | AVPLAY_HDR_CHAN_EL);
            if (Ret != HI_SUCCESS)
            {
                return Ret;
            }
        ...
        {
            Ret = AVPLAY_StopHDRChn(pAvplay, AVPLAY_HDR_CHAN_BL);
            if (Ret != HI_SUCCESS)
            {
                return Ret;
            }
...
HI_S32 AVPLAY_AttachWindow(AVPLAY_S *pAvplay, HI_HANDLE hWin)
{
    ...
    if (HI_DRV_WIN_ACTIVE_MAIN_AND_SLAVE == stWinInfo.eType)
    {
        ...
        if (HI_TRUE == pAvplay->bHDRPlay)
        {
            Ret = AVPLAY_CreateHDRPort(pAvplay, AVPLAY_HDR_CHAN_BL, VDEC_PORT_HD);
            if (HI_SUCCESS != Ret)
            {
                return Ret;
            }

            Ret = AVPLAY_SetHDRPortAttr(pAvplay, AVPLAY_HDR_CHAN_BL, VDEC_PORT_TYPE_MASTER);
            if (HI_SUCCESS != Ret)
            {
                (HI_VOID)AVPLAY_DestroyHDRPort(pAvplay, AVPLAY_HDR_CHAN_BL);
                return Ret;
            }
...
HI_S32 AVPLAY_DetachWindow(AVPLAY_S *pAvplay, HI_HANDLE hWin)
{
    ...
    /* homologous window*/ /* CNcomment: ͬԴ���� */
    ...
    /*  analogous master window*/ /* CNcomment: ��ͬԴ �����ڼ��Ӵ��� */
    ...
    /* analogous virtual window*/ /* CNcomment ��ͬԴ ���ⴰ��*/
    else
    {
        for (i = 0; i < AVPLAY_MAX_VIR_FRMCHAN; i++)
        {
...
static HI_S32 AVPLAY_ProcRead(HI_PROC_SHOW_BUFFER_S *ProcBuf, HI_VOID *Data)
{
...
#ifdef HI_HDR_SUPPORT
    switch (pAvplay->AvplayHDRAttr.enElType)
    {
        case HI_UNF_VCODEC_TYPE_MPEG2 :
            VidElType = "MPEG2";
            break;
    ...
    if (HI_TRUE == pAvplay->bHDRPlay)
    {
        HI_PROC_Printf(ProcBuf,
            ...
            pAvplay->AvplayHDRAttr.enHDRStreamType,
            VidType, VidElType,
    ...
    Ret = HI_MPI_ADEC_GetDecoderList((HI_HA_DECODE_LIB_S**)(&pHaLib));
    if (HI_SUCCESS == Ret)
    {
...
HI_S32 HI_MPI_AVPLAY_Create(const HI_UNF_AVPLAY_ATTR_S *pstAvAttr, HI_HANDLE *phAvplay)
{
    ...
    /*
    * FIX DTS2015092306923. These members should be 0 when create avplay called,
...
HI_S32 HI_MPI_AVPLAY_SetAttr(HI_HANDLE hAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, HI_VOID *pPara)
{
    ...
    if ((HI_U32)enAttrID >= HI_UNF_AVPLAY_ATTR_ID_BUTT)
    {
    ...
    switch (enAttrID)
    {
        ...
        case HI_UNF_AVPLAY_ATTR_ID_HDR:
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Set HDR Attr failed.\n");
            }
        #else
            Ret = HI_ERR_AVPLAY_NOT_SUPPORT;
        #endif
        ...
        case HI_UNF_AVPLAY_ATTR_ID_AD:
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Set AD Attr failed.\n");
            }
            break;
        ...
        case HI_UNF_AVPLAY_ATTR_ID_AUD_SECURITY:
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Set audio security failed.\n");
            }
            break;
...
HI_S32 HI_MPI_AVPLAY_GetAttr(HI_HANDLE hAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, HI_VOID *pPara)
{
    ...
    switch (enAttrID)
    {
        ...
        case HI_UNF_AVPLAY_ATTR_ID_HDR:
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Get HDR Attr failed.\n");
            }
        #else
            Ret = HI_ERR_AVPLAY_NOT_SUPPORT;
        #endif
        ...
        case HI_UNF_AVPLAY_ATTR_ID_AD:
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Get AD Attr failed.\n");
            }
            break;
                                              ...
                                              HI_UNF_VIDEO_FRAME_INFO_S *pstCapPicture)
{
    ...
    if (HI_TRUE == bCapture)
    {
        ...
        stNewCfg.eFormat = HI_DRV_PIX_FMT_NV21;
    }
    ...
    for (i = 0; i < 20; i++)
    {
        Ret = HI_MPI_VDEC_ReceiveFrame(pAvplay->hVdec, &stFrmPack, AVPLAY_RECVFRAME_MAX_TIME);
        if (Ret == HI_SUCCESS)
        {
            break;
        }
    ...
    else
    {
        ...
        {
            Ret = HI_MPI_VDEC_ReleaseFrame(stFrmPack.stFrame[i].hport, &stFrmPack.stFrame[i].stFrameVideo);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("avplay release frame failed 0x%x\n", Ret);
            }
...
HI_S32 HI_MPI_AVPLAY_Start(HI_HANDLE hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn)
{
    ...
    if (enChn & ((HI_U32)HI_UNF_AVPLAY_MEDIA_CHAN_VID))
    {
        ...
        if (!pAvplay->VidEnable)
        {
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("HI_MPI_SYNC_Play Vid failed 0x%x\n", Ret);
            }
    ...
    if (StopOpt.u32TimeoutMs)
    {
        ...
        while (1)
        {
            ...
            if (!Block)
            {
                ...
                if (DurationTime >= StopOpt.u32TimeoutMs)
                {
                            ...
                            HI_U32                u32TimeOutMs)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD == enBufId)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            ...
            {
                HI_ERR_AVPLAY("HI_MPI_ADEC_GetBuffer failed 0x%x.\n", Ret);
            }
                                       ...
                                       HI_U32 u32ValidDataLen, HI_U32 u32Pts, HI_UNF_AVPLAY_PUTBUFEX_OPT_S *pstExOpt)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID_HDR_BL == enBufId)
    {
        ...
        if (pstExOpt->bContinue)
        {
            pAvplay->AvplayBlEsBuf.bDiscontinuous = HI_FALSE;
        }
        else
        {
            pAvplay->AvplayBlEsBuf.bDiscontinuous = HI_TRUE;
        }
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID_HDR_EL == enBufId)
    {
        ...
        if (pstExOpt->bContinue)
        {
            pAvplay->AvplayElEsBuf.bDiscontinuous = HI_FALSE;
        }
        else
        {
            pAvplay->AvplayElEsBuf.bDiscontinuous = HI_TRUE;
        }
