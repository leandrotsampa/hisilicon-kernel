...
png_read_end(png_structp png_ptr, png_infop info_ptr)
{
   ...
   if (png_ptr == NULL)
    ...
    if (pstHwStruct && (pstHwStruct->chunk_name[0] != '\0'))
    {
    ...
    png_infop end_info_ptr)
{
   ...
   if (png_ptr->private_ptr)
   {
       ...
       if (!(pstHwStruct->u8DecMode & HIPNG_HW_DEC)
        ...
        && pstHwStruct->u32TmpPhyaddr)
        {
#ifdef HIPNG_COMMON_MMZ
			HI_GFX_Unmap(pstHwStruct->u32TmpPhyaddr, HI_TRUE);
            HI_GFX_FreeMem(pstHwStruct->u32TmpPhyaddr, HI_TRUE);
#else
        	HI_GFX_Unmap(pstHwStruct->s32MmzFd, pstHwStruct->u32TmpPhyaddr, pstHwStruct->pTmpMemData);
            HI_GFX_FreeMem(pstHwStruct->s32MmzFd, pstHwStruct->u32TmpPhyaddr, pstHwStruct->pTmpMemData, HI_TRUE);
#endif	
			pstHwStruct->u32TmpPhyaddr = 0;
        }
       hipng_destroy_read_struct_hw(pstHwStruct);
...
#if defined(HIPNG_ACCELERATE) && defined(CONFIG_GFX_MEM_MMZ)
{
	 ...
	 if (pstHwStruct)
	 {
		...
		HI_CHAR *pZoneName = NULL;
	     ...
	     u32Phyaddr = HI_GFX_AllocMem(u32RowBytes * info_ptr->height, 16, pZoneName, "PNG_DECODE", &bMmu);
