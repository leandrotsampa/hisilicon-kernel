...
static HI_VOID *Hdmi_CEC_EventThread(HI_VOID* pParam)
{
    ...
    while(HI_TRUE == s_stCECAttr.bCecEnable)
    {
...
HI_S32 HI_MPI_HDMI_ComGetVOAttr(HI_UNF_HDMI_ID_E enHdmi, DRV_HDMI_VO_ATTR_S *pstDrvVOAttr)
{
	...
	memset(pstDrvVOAttr, 0, sizeof(HDMI_VIDEO_ATTR_S));
...
HI_S32 HI_MPI_HDMI_ComSetCECCommand(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CEC_CMD_S *pCECCmd)
{
    ...
    memset(&stDrvCecCmd, 0, sizeof(HDMI_CEC_CMD_S));
...
HI_S32 HI_MPI_HDMI_ComCECStatus(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CEC_STATUS_S *pStatus)
{
    ...
    pStatus->bEnable        = CECStatus.stCecInfo.bNetValid;
...
HI_S32 HI_MPI_HDMI_ComCECEnable(HI_UNF_HDMI_ID_E enHdmi)
{
    ...
    s32Ret = pthread_create(&s_stCECAttr.CECTread, HI_NULL,
                         Hdmi_CEC_EventThread, HI_NULL);  
    if (s32Ret != HI_SUCCESS)
    {
        HDMI_UNLOCK();
...
HI_S32 HI_MPI_HDMI_ComSetDispAttr(HI_UNF_HDMI_ID_E enHdmi, HDMI_VIDEO_ATTR_S *pstVideoAttr)
{
    HI_S32 s32Ret = 0;
    DRV_HDMI_DISP_ATTR_S stDispAttr;

    HDMI_CHECK_ID(enHdmi);
    HDMI_CHECK_NULL_PTR(pstVideoAttr);
    
    HDMI_LOCK();
    stDispAttr.enHdmiID = enHdmi;
    memcpy(&stDispAttr.stVideoAttr, pstVideoAttr, sizeof(HDMI_VIDEO_ATTR_S));

    s32Ret = ioctl(s_s32HdmiDevFd, CMD_HDMI_SET_DISP_ATTR, &stDispAttr);
