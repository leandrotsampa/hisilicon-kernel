...
HI_VOID JPEG_HDEC_GetHuffmanDcTable(const struct jpeg_decompress_struct *cinfo)
{
      ...
      if ((NULL == cinfo->dc_huff_tbl_ptrs[0]) || (NULL != cinfo->dc_huff_tbl_ptrs[2])){
          pDcDefaultTable += 1;
          memcpy(&(DcHuffmanTablePtr[LU].bits[1]), pDcDefaultTable, 16);
          pDcDefaultTable += 16;
          memcpy(&(DcHuffmanTablePtr[LU].huffval[0]), pDcDefaultTable, 12);
          pDcDefaultTable += 12;

          pDcDefaultTable += 1;
          memcpy(&(DcHuffmanTablePtr[CH].bits[1]), pDcDefaultTable, 16);
          pDcDefaultTable += 16;
          memcpy(&(DcHuffmanTablePtr[CH].huffval[0]), pDcDefaultTable, 12);
          pDcDefaultTable += 12;
      }else {
          memcpy(&(DcHuffmanTablePtr[LU]), cinfo->dc_huff_tbl_ptrs[0], sizeof(DcHuffmanTablePtr[LU]));
          if (NULL == cinfo->dc_huff_tbl_ptrs[1]){
              memcpy(&(DcHuffmanTablePtr[CH]),cinfo->dc_huff_tbl_ptrs[0],sizeof(DcHuffmanTablePtr[LU]));
          }else {
              memcpy(&(DcHuffmanTablePtr[CH]),cinfo->dc_huff_tbl_ptrs[1],sizeof(DcHuffmanTablePtr[LU]));
          }
      }
...
static HI_VOID JPEG_HDEC_GetHuffmanAcTable(const struct jpeg_decompress_struct *cinfo)
{
     ...
     if ((NULL == cinfo->ac_huff_tbl_ptrs[0]) || (NULL != cinfo->ac_huff_tbl_ptrs[2])){
         pAcDefaultTable += 1;
         memset(&(AcHuffmanTablePtr[LU].bits[0]),0,17);
         memcpy(&(AcHuffmanTablePtr[LU].bits[1]), pAcDefaultTable, 16);
         pAcDefaultTable += 16;
         memset(&(AcHuffmanTablePtr[LU].huffval[0]),0,256);
         memcpy(&(AcHuffmanTablePtr[LU].huffval[0]), pAcDefaultTable, 162);
         pAcDefaultTable += 162;

         pAcDefaultTable += 1;
         memset(&(AcHuffmanTablePtr[CH].bits[0]),0,17);
         memcpy(&(AcHuffmanTablePtr[CH].bits[1]), pAcDefaultTable, 16);
         pAcDefaultTable += 16;
         memset(&(AcHuffmanTablePtr[CH].huffval[0]),0,256);
         memcpy(&(AcHuffmanTablePtr[CH].huffval[0]), pAcDefaultTable, 162);
     }else {
         memset(&(AcHuffmanTablePtr[LU].bits[0]),0,17);
         memset(&(AcHuffmanTablePtr[LU].huffval[0]),0,256);
         memset(&(AcHuffmanTablePtr[CH].bits[0]),0,17);
         memset(&(AcHuffmanTablePtr[CH].huffval[0]),0,256);
         memcpy(&(AcHuffmanTablePtr[LU]), cinfo->ac_huff_tbl_ptrs[0], sizeof(AcHuffmanTablePtr[LU]));
         
         if (NULL == cinfo->ac_huff_tbl_ptrs[1]){
             memcpy(&(AcHuffmanTablePtr[CH]),cinfo->ac_huff_tbl_ptrs[0],sizeof(AcHuffmanTablePtr[LU]));
         }else {
             memcpy(&(AcHuffmanTablePtr[CH]),cinfo->ac_huff_tbl_ptrs[1],sizeof(AcHuffmanTablePtr[LU]));
         }
     }
