...
HI_S32 JPEG_HDEC_SendStreamFromPhyMem(j_decompress_ptr cinfo)
{
     ...
     HARDDEC_TSTART();
      ...
      JPEG_HDEC_WriteReg(pJpegHandle->pJpegRegVirAddr,JPGD_REG_RESUME, 0x2);
#ifdef CONFIG_JPEG_4KDDR_DISABLE
      JPEG_HDEC_WriteReg(pJpegHandle->pJpegRegVirAddr,JPGD_REG_START, 0x5 | s32LowDelayValue);
#else
      JPEG_HDEC_WriteReg(pJpegHandle->pJpegRegVirAddr,JPGD_REG_START, 0x1 | s32LowDelayValue);
#endif
...
static inline HI_BOOL JPEG_HDEC_CheckCallBackFuncStreamIsNull(j_decompress_ptr cinfo)
{
...
HI_S32 JPEG_HDEC_SendStreamFromCallBack(j_decompress_ptr cinfo)
{
     ...
     do
     {
         ...
         bReachEOF = JPEG_HDEC_CheckCallBackFuncStreamIsNull(cinfo);
