...
static inline HI_BOOL JPEG_HDEC_ExistQuantTable     (const struct jpeg_decompress_struct *cinfo);
static inline HI_BOOL JPEG_HDEC_ExistHuffTable      (const struct jpeg_decompress_struct *cinfo);
...
HI_BOOL JPEG_HDEC_IfSupport(j_decompress_ptr cinfo)
{
    ...
    HI_BOOL ExistQuantTable = HI_FALSE;
    HI_BOOL ExistHuffTable  = HI_FALSE;
    ...
    ExistQuantTable = JPEG_HDEC_ExistQuantTable(cinfo);
    if (HI_TRUE != ExistQuantTable){
        return HI_FALSE;
    }

    ExistHuffTable = JPEG_HDEC_ExistHuffTable(cinfo);
    if (HI_TRUE != ExistHuffTable){
        return HI_FALSE;
    }
...
static inline HI_BOOL JPEG_HDEC_SupportCts(HI_VOID)
{
#if defined(CONFIG_JPEG_ADD_GOOGLEFUNCTION)
