...
#ifdef CONFIG_GFX_JPGE_ENC_ENABLE
  s32Ret  = JPGE_HENC_Init((j_common_ptr)cinfo);
  if(HI_SUCCESS != s32Ret)
  {
	 ERREXIT(cinfo, JERR_HENC_INIT_FAILURE);
  }
#else
 /**
...
jpeg_finish_compress (j_compress_ptr cinfo)
{
  JDIMENSION iMCU_row;
	...
	if(NULL != pJpgeHandle && HI_TRUE == pJpgeHandle->bEncodeSuccess)
	{
		JPEG_HENC_WriteData(cinfo);
		return;
	}
#endif
