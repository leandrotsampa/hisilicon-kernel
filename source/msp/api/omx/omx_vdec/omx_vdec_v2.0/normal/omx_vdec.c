...
#endif
#ifdef ANDROID
static OMX_ERRORTYPE omx_tunneled_mode_create_win(OMX_COMPONENT_PRIVATE* pcom_priv, HI_HANDLE* hWin)
{
    ...
    if (NULL == pcom_priv)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: Error! Bad Paramter in %s, %d\n", __func__, __LINE__);
    ...
    if (Ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: call HI_UNF_VO_Init failed!\n");
    ...
    if (Ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: call HI_UNF_VO_CreateWindow failed!\n");
        Ret = HI_UNF_VO_DeInit();

        if (Ret != HI_SUCCESS)
        {
            DEBUG_PRINT_ERROR("\nTunneled-playback Mode: call HI_UNF_VO_DeInit failed!\n");
            return OMX_ErrorInsufficientResources;
        }
...
static OMX_ERRORTYPE omx_tunneled_mode_attach_win(OMX_COMPONENT_PRIVATE* pcom_priv)
{
    HI_S32 Ret;

    if (NULL == pcom_priv)
    {
        DEBUG_PRINT_ERROR("Error! Bad Paramter in %s, %d\n", __func__, __LINE__);
        return OMX_ErrorBadParameter;
    }

    Ret = HI_UNF_VO_AttachWindow(pcom_priv->drv_ctx.hWin, pcom_priv->stParams.nAudioHwSync);
    ...
    Ret = HI_UNF_VO_SetWindowEnable(pcom_priv->drv_ctx.hWin, HI_TRUE);

    if (Ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("call HI_UNF_VO_SetWindowEnable failed.\n");
        HI_UNF_VO_DetachWindow(pcom_priv->drv_ctx.hWin, pcom_priv->stParams.nAudioHwSync);
        return OMX_ErrorInsufficientResources;
    }
...
static OMX_ERRORTYPE omx_tunneled_mode_detach_win(OMX_COMPONENT_PRIVATE* pcom_priv)
{
    if (NULL == pcom_priv)
    {
        DEBUG_PRINT_ERROR("Error! Bad Paramter in %s, %d\n", __func__, __LINE__);
    ...
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Disable Window! hWin: %p, avplay handle: %p, omx handle : %p\n", pcom_priv->drv_ctx.hWin, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
    HI_UNF_VO_SetWindowEnable(pcom_priv->drv_ctx.hWin, HI_FALSE);
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Detach Window! hWin: %p, avplay handle: %p, omx handle : %p\n", pcom_priv->drv_ctx.hWin, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
    HI_UNF_VO_DetachWindow(pcom_priv->drv_ctx.hWin, pcom_priv->stParams.nAudioHwSync);

    return OMX_ErrorNone;
}

static OMX_ERRORTYPE omx_tunneled_mode_destroy_win(OMX_COMPONENT_PRIVATE* pcom_priv)
{
    if (NULL == pcom_priv)
    {
        DEBUG_PRINT_ERROR("Error! Bad Paramter in %s, %d\n", __func__, __LINE__);
    ...
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Destroy Window! hWin: %p\n", pcom_priv->drv_ctx.hWin);
    HI_UNF_VO_DestroyWindow(pcom_priv->drv_ctx.hWin);
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Deinit VO!\n");
    HI_UNF_VO_DeInit();

    return OMX_ErrorNone;
}

static OMX_ERRORTYPE omx_tunneled_mode_enable_avplay(OMX_COMPONENT_PRIVATE* pcom_priv)
{
    ...
    Ret = HI_UNF_AVPLAY_SetAttr(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_ATTR_ID_TVP, &(pcom_priv->drv_ctx.drv_cfg.is_tvp));
    if (HI_SUCCESS != Ret)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay set TVP attr fail! TVP enable: %d, avplay handle: %p, omx handle : %p\n", pcom_priv->drv_ctx.drv_cfg.is_tvp, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        //HI_UNF_AVPLAY_ChnClose(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID);
        return OMX_ErrorInsufficientResources;
    }
#endif

    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay set Stream Mode! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
    memset(&stAvplayAttr, 0, sizeof(HI_UNF_AVPLAY_ATTR_S));
    Ret = HI_UNF_AVPLAY_GetAttr(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_ATTR_ID_STREAM_MODE, &stAvplayAttr);
    if (HI_SUCCESS != Ret)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay Get VID Stream Mode attr fail!, avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        //HI_UNF_AVPLAY_ChnClose(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID);
        return OMX_ErrorInsufficientResources;
    }
    ...
    Ret = HI_UNF_AVPLAY_SetAttr(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_ATTR_ID_STREAM_MODE, &stAvplayAttr);

    if (HI_SUCCESS != Ret)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay set VID Stream BUF fail!, StreamBufSize: %d, avplay handle: %p, omx handle : %p\n", stAvplayAttr.stStreamAttr.u32VidBufSize, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        //HI_UNF_AVPLAY_ChnClose(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID);
        return OMX_ErrorInsufficientResources;
    }

    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay set Stream Mode attr Success! StreamType: %d, VidBufSize: %d, avplay handle: %p, omx handle : %p\n", stAvplayAttr.stStreamAttr.enStreamType, stAvplayAttr.stStreamAttr.u32VidBufSize, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
    memset(&stOpenOpt, 0, sizeof(stOpenOpt));
    drv_cfg = &(pcom_priv->drv_ctx.drv_cfg);
    ...
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: call avplay Chanopen! avplay handle: %p, width: %d, height: %d, CapLevel: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, drv_cfg->cfg_width, drv_cfg->cfg_height, stOpenOpt.enCapLevel, pcom_priv->drv_ctx.chan_handle);
    Ret = HI_UNF_AVPLAY_ChnOpen(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID, &stOpenOpt);

    if (Ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("set_parameter: AVPLAY ChnOpen Fail! ret: %p, avplay handle: %p, omx handle : %p\n", Ret, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        return OMX_ErrorInsufficientResources;
    }

    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: call avplay Chanopen success, set tunnel mode to avplay! Tunnel mode enable: %p, omx handle : %p\n", pcom_priv->drv_ctx.drv_cfg.TunnelModeEnable, pcom_priv->drv_ctx.chan_handle);
    Ret = HI_MPI_AVPLAY_SetTunnelMode(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID, pcom_priv->drv_ctx.drv_cfg.TunnelModeEnable);

    if (Ret != HI_SUCCESS)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay Set tunnel Mode Fail, call avplay chanclose! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        HI_UNF_AVPLAY_ChnClose(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID);
        return OMX_ErrorBadParameter;
    }

    pchan_cfg = &pcom_priv->drv_ctx.drv_cfg.chan_cfg;
    ...
    stVdecAttr.pCodecContext  = &(pcom_priv->drv_ctx);

    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay Set Tunnel Mode success, call avplay set attr! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
    Ret = HI_UNF_AVPLAY_SetAttr(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_ATTR_ID_VDEC, &stVdecAttr);
    if (HI_SUCCESS != Ret)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay set attr fail! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        HI_UNF_AVPLAY_ChnClose(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID);
        return OMX_ErrorInsufficientResources;
    }

    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay set attr success! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);

    return OMX_ErrorNone;
}

static OMX_ERRORTYPE omx_tunneled_mode_disable_avplay(OMX_COMPONENT_PRIVATE* pcom_priv)
{
    if (NULL == pcom_priv)
    {
        DEBUG_PRINT_ERROR("Error! Bad Paramter in %s, %d\n", __func__, __LINE__);
    ...
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay Close Chn! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
    HI_UNF_AVPLAY_ChnClose(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID);

    return OMX_ErrorNone;
}

static OMX_ERRORTYPE omx_tunneled_mode_avplay_flush(OMX_COMPONENT_PRIVATE* pcom_priv)
{
    HI_S32 Ret = OMX_ErrorNone;
    ...
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay flush Chn! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);

    Ret = HI_UNF_AVPLAY_Stop(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID, HI_NULL);
    if(HI_SUCCESS != Ret)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay flush Chn Error, avplay stop fail! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        return Ret;
    }

    Ret = HI_UNF_AVPLAY_Start(pcom_priv->stParams.nAudioHwSync, HI_UNF_AVPLAY_MEDIA_CHAN_VID, HI_NULL);
    if(HI_SUCCESS != Ret)
    {
        DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay flush Chn Error, avplay start fail! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        return Ret;
    }
    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: avplay flush Chn Success! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
       ...
       OMX_IN OMX_U8* p_handle)
{
    ...
    OMX_CHECK_ARG_RETURN(p_private_handle->ion_phy_addr == 0 || p_private_handle->ion_phy_addr == -1);
    ...
    {
        DEBUG_PRINT_ERROR("[UNB] stride invalid, p_private_handle->stride = %ld, OMX stride = %ld\n",\
    ...
    //VPSS��4K�����Ҫ�������buf������Ϣ
    if (metadata_phy_addr != -1 && metadata_phy_addr != NULL)
    {
    #if 0
...
#ifdef HI_OMX_TEE_SUPPORT
    Phyaddr = (HI_U32)((HI_ULONG)buffer);
    buffer_type = OMX_USE_SECURE;
#else
    buffer_type = OMX_USE_OTHER;
...
static OMX_ERRORTYPE get_supported_profile_level(OMX_COMPONENT_PRIVATE *pcomp_priv, OMX_VIDEO_PARAM_PROFILELEVELTYPE *profileLevelType)
{
    ...
    else if (!strncmp((OMX_STRING)pcomp_priv->m_role, OMX_ROLE_VIDEO_DECODER_VP9, OMX_MAX_STRINGNAME_SIZE))
    {
...
static OMX_ERRORTYPE  fill_this_buffer_proxy(OMX_COMPONENT_PRIVATE *pcom_priv, OMX_IN OMX_BUFFERHEADERTYPE* pomx_buf)
{
    ...
    if ((HI_TRUE == pcom_priv->drv_ctx.drv_cfg.bVpssBypass) && (puser_buf->need_release_frame == 1))
    {
        /*if vpss bypass, should try to release the frame now*/
        /*�˴����������ӿڵ��ں�̬��֡�����������channel_submit_frame �л�֡��Ҫ����Ϊ�����ʱ�߽��������flush״̬��֧����֡��
          ��ֱ�ӷ��أ�û�б��ͷţ�������Ԥ��*/
...
static OMX_S32 message_process (OMX_COMPONENT_PRIVATE  *pcom_priv, void* message)
{
    ...
    switch (vdec_msg->msgcode)
    {
        ...
        case VDEC_MSG_RESP_INPUT_DONE:
            ...
            port_priv = &pcom_priv->m_port[INPUT_PORT_INDEX];
            if (0 == puser_buf->client_data) //for 64bit
            {
            ...
            //DEBUG_PRINT_TRACE("[EBD] post ||| phyaddr = %x, useraddr = %p\n", puser_buf->phyaddr, pomx_buf->pBuffer);
        ...
        case VDEC_MSG_RESP_OUTPUT_DONE:
            ...
            puser_buf->need_release_frame = OMX_TRUE;
            ...
            //DEBUG_PRINT_TRACE("[FBD] post ||| phyaddr = %x, useraddr = %p, data_len = %ld\n", puser_buf->phyaddr, pomx_buf->pBuffer, pomx_buf->nFilledLen);
       ...
       OMX_U32 *sem_posted)
{
    ...
    else if (pcom_priv->m_state == OMX_StateIdle)
    {
        ...
        {
        #ifdef VFMW_VPSS_BYPASS_EN
            if (HI_TRUE == pcom_priv->drv_ctx.drv_cfg.bVpssBypass)
            {
        ...
        if (state == OMX_StateIdle)
        {
            ...
            if (!*sem_posted)
            {
                *sem_posted = 1;
                ...
                {
                    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Flush InputPort\n");
       ...
       OMX_U32 port)
{
    ...
    if(1 == pcom_priv->drv_ctx.drv_cfg.TunnelModeEnable)
    {
        ret = omx_tunneled_mode_avplay_flush(pcom_priv);
        if(OMX_ErrorNone != ret)
        {
            DEBUG_PRINT_ERROR("\nTunneled-playback Mode: avplay flush Chn Fail! avplay handle: %p, omx handle : %p\n", pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        }
    }
    #endif
    ...
    switch (id)
       {
        ...
        case OMX_GENERATE_START_DONE:
            ...
            if(bit_present(&pcom_priv->m_flags, OMX_STATE_EXECUTE_PENDING))
            {
                ...
                {
                    DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: OMX Start Success! omx handle: %d, avplay handle: %u\n", pcom_priv->drv_ctx.chan_handle, pcom_priv->drv_ctx.AvplayHandle);
                }
       ...
       OMX_INOUT OMX_PTR param_data)
{
    ...
    HI_UNF_AVPLAY_STREAM_INFO_S stAvplayStreamInfo;
    ...
    memset(&stAvplayStreamInfo, 0, sizeof(HI_UNF_AVPLAY_STREAM_INFO_S));
    if(pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable)
    {
        ret = HI_UNF_AVPLAY_GetStreamInfo(pcomp_priv->drv_ctx.AvplayHandle, &stAvplayStreamInfo);
    ...
    switch (param)
    {
        case OMX_IndexParamPortDefinition:
        {
            ...
            else if (OUTPUT_PORT_INDEX == portDefn->nPortIndex)
            {
               ...
               portDefn->format.video.eColorFormat = \
                             convert_omx_to_hal_pixel_fmt(pcomp_priv, pcomp_priv->drv_ctx.drv_cfg.cfg_color_format);
        ...
        case OMX_GoogleIndexGetAndroidNativeBufferUsage:
        {
            ...
            else
            {
                ...
                if(1 != pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable)
                {
        ...
        case OMX_HISIIndexVideoPostProcessBypasInfo:
        {
...
#endif
        case OMX_GoogleAndroidIndexConfigureVideoTunnelMode:
        {
            memcpy(param_data, &(pcomp_priv->stParams), sizeof(ConfigureVideoTunnelModeParams));
            break;
        }
       ...
       OMX_IN OMX_PTR param_data)
{
    ...
    switch (param)
    {
        case OMX_IndexParamPortDefinition:
        {
            ...
            port_priv = &pcomp_priv->m_port[portDefn->nPortIndex];
            if((1 != pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable) || \
              ((1 == pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable) && \
               (INPUT_PORT_INDEX == portDefn->nPortIndex)))
            {
                if (portDefn->nBufferCountActual < port_priv->port_pro.min_count)
            {
                DEBUG_PRINT_ERROR("set_parameter: setted buf cnt too small! min: %ld, nBufferCountActual: %ld\n", port_priv->port_pro.min_count, portDefn->nBufferCountActual);
                ...
                if (port_priv->port_pro.max_count < portDefn->nBufferCountActual)
            {
                ...
                DEBUG_PRINT_ALWS("set_parameter: setted buf cnt too big! max: %ld\n", port_priv->port_pro.max_count);
                return OMX_ErrorUndefined;
            }
            }

            if((1 == pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable) && \
               (OUTPUT_PORT_INDEX == portDefn->nPortIndex))
            {
                port_priv->port_pro.max_count = 0;//portDefn->nBufferCountActual;
                port_priv->port_pro.min_count = 0;//portDefn->nBufferCountMin;
            }
            else
            {
            ...
            //mediacodec Ϊ�˼�¼��С����֡�ʣ���16λ�洢��������16λ�洢С����
            ...
            if((1 == pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable) && \
               (OUTPUT_PORT_INDEX == portDefn->nPortIndex) && \
               (0 == portDefn->nBufferCountActual))
            {
                OMX_ERRORTYPE Ret;

                DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Enable avplay!, avplay handle: %p, omx handle: %p\n", pcomp_priv->stParams.nAudioHwSync, pcomp_priv->drv_ctx.chan_handle);
                Ret = omx_tunneled_mode_enable_avplay(pcomp_priv);

                if (OMX_ErrorNone != Ret)
                {
                    DEBUG_PRINT_ERROR("\nTunneled-playback Mode: Enable avplay fail, Destroy win! avplay handle: %p, win handle: %p, omx handle: %p\n", pcomp_priv->stParams.nAudioHwSync, pcomp_priv->drv_ctx.hWin, pcomp_priv->drv_ctx.chan_handle);
                    omx_tunneled_mode_destroy_win(pcomp_priv);
                    return OMX_ErrorInsufficientResources;
                }

                DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Enable avplay success, attach_win!, avplay handle: %p, win handle: %p, omx handle: %p\n", pcomp_priv->stParams.nAudioHwSync, pcomp_priv->drv_ctx.hWin, pcomp_priv->drv_ctx.chan_handle);
                Ret = omx_tunneled_mode_attach_win(pcomp_priv);

                if (OMX_ErrorBadParameter == Ret)
                {
                    DEBUG_PRINT_ERROR("\nTunneled-playback Mode: Enable avplay fail1, disable avplay, destory win!, avplay handle: %p, win handle: %p, omx handle: %p\n", pcomp_priv->stParams.nAudioHwSync, pcomp_priv->drv_ctx.hWin, pcomp_priv->drv_ctx.chan_handle);
                    omx_tunneled_mode_disable_avplay(pcomp_priv);
                    omx_tunneled_mode_destroy_win(pcomp_priv);
                    return OMX_ErrorBadParameter;
                }
                else if (OMX_ErrorInsufficientResources == Ret)
                {
                    DEBUG_PRINT_ERROR("\nTunneled-playback Mode: Enable avplay fail2, detach win, disable avplay, destory win!, avplay handle: %p, win handle: %p, omx handle: %p\n", pcomp_priv->stParams.nAudioHwSync, pcomp_priv->drv_ctx.hWin, pcomp_priv->drv_ctx.chan_handle);
                    omx_tunneled_mode_detach_win(pcomp_priv);
                    omx_tunneled_mode_disable_avplay(pcomp_priv);
                    omx_tunneled_mode_destroy_win(pcomp_priv);
                    return OMX_ErrorBadParameter;
                }

                DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Attach_win success!, avplay handle: %p, win handle: %p, omx handle: %p\n", pcomp_priv->stParams.nAudioHwSync, pcomp_priv->drv_ctx.hWin, pcomp_priv->drv_ctx.chan_handle);
        ...
        case OMX_GoogleAndroidIndexConfigureVideoTunnelMode:
        {
            ConfigureVideoTunnelModeParams* pParams = (ConfigureVideoTunnelModeParams*) param_data;
            HI_HANDLE hWin;
            OMX_ERRORTYPE Ret;

            DEBUG_PRINT_TUNNELED("\nEnter Tunneled-playback mode\n");

            if (pParams->nPortIndex != OUTPUT_PORT_INDEX)
            {
                DEBUG_PRINT_ERROR("\nset_parameter: Bad port index %ud\n", (int)pParams->nPortIndex);
                return OMX_ErrorBadPortIndex;
            }

            if (pParams->nAudioHwSync == HI_INVALID_HANDLE)
            {
                DEBUG_PRINT_ERROR("\nset_parameter: Bad Handle %ud\n", pParams->nAudioHwSync);
                return OMX_ErrorBadParameter;
            }

            DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Create win!, avplay handle: %p\n", pParams->nAudioHwSync);

            Ret = omx_tunneled_mode_create_win(pcomp_priv, &hWin);
            ...
            DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Create win success!, win handle: %p\n", hWin);

            pcomp_priv->stNativeHandle.version = sizeof(native_handle_t);
            pcomp_priv->stNativeHandle.numFds  = 0;
            pcomp_priv->stNativeHandle.numInts = 1;
            pcomp_priv->stNativeHandle.data[0] = hWin;

            pcomp_priv->drv_ctx.hWin = hWin;
            //End

            pParams->pSidebandWindow = &(pcomp_priv->stNativeHandle);
            pcomp_priv->drv_ctx.AvplayHandle = pParams->nAudioHwSync;

            pcomp_priv->drv_ctx.drv_cfg.TunnelModeEnable = 1;

            memcpy(&(pcomp_priv->stParams), pParams, sizeof(ConfigureVideoTunnelModeParams));
...
//#if 1  // native buffer
    ...
    {
        *pindex_type = (OMX_INDEXTYPE)OMX_HISIIndexVideoPostProcessBypasInfo;
    }
#endif
...
OMX_ERRORTYPE  component_deinit(OMX_IN OMX_HANDLETYPE phandle)
{
    ...
    if(pcom_priv->drv_ctx.drv_cfg.TunnelModeEnable)
    {
        DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Exit Tunneled playback mode! hWin: %p, avplay handle: %p, omx handle : %p\n", pcom_priv->drv_ctx.hWin, pcom_priv->stParams.nAudioHwSync, pcom_priv->drv_ctx.chan_handle);
        omx_tunneled_mode_detach_win(pcom_priv);
        omx_tunneled_mode_disable_avplay(pcom_priv);
        omx_tunneled_mode_destroy_win(pcom_priv);
    }
    #endif
...
OMX_ERRORTYPE component_init(OMX_HANDLETYPE phandle, OMX_STRING comp_name)
{
    ...
    pcom_priv->drv_ctx.AvplayHandle     = HI_INVALID_HANDLE;
    pcom_priv->drv_ctx.hWin             = HI_INVALID_HANDLE;
