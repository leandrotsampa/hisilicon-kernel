...
OMX_S32 channel_start(OMXVDEC_DRV_CONTEXT *drv_ctx)
{
    ...
    if(drv_ctx->drv_cfg.TunnelModeEnable)
    {
        DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: OMX is ready! Now call avplay start! avplay handle: %u, omx handle: %d\n", drv_ctx->AvplayHandle, drv_ctx->chan_handle);
        return HI_UNF_AVPLAY_Start(drv_ctx->AvplayHandle, HI_UNF_AVPLAY_MEDIA_CHAN_VID, NULL);
    }
    else
    {
    vdec_fd      = drv_ctx->video_driver_fd;
...
OMX_S32 channel_stop(OMXVDEC_DRV_CONTEXT *drv_ctx)
{
    ...
    if(drv_ctx->drv_cfg.TunnelModeEnable)
    {
        DEBUG_PRINT_TUNNELED("\nTunneled-playback Mode: Call AVPLAY_STOP! avplay handle: %u, omx handle: %d\n", drv_ctx->AvplayHandle, drv_ctx->chan_handle);
        return HI_UNF_AVPLAY_Stop(drv_ctx->AvplayHandle, HI_UNF_AVPLAY_MEDIA_CHAN_VID, NULL);
    }
    else
    {
    vdec_fd      = drv_ctx->video_driver_fd;
...
OMX_S32 channel_flush_port(OMXVDEC_DRV_CONTEXT *drv_ctx, OMX_U32 flush_dir)
{
    ...
    msg.chan_num = drv_ctx->chan_handle;
    dir          = flush_dir;
    msg.in       = (HI_SIZE_T)(&dir); //@f00241306 for 64 bits
    msg.out      = 0;
...
OMX_S32 channel_alloc_buffer(OMXVDEC_DRV_CONTEXT *drv_ctx, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    DEBUG_PRINT_TRACE("Alloc buffer vir:0x%p phy:0x%x, size:%d dir:%d sec:%d success!\n", \
         (void *)(long)puser_buf->bufferaddr, puser_buf->phyaddr, puser_buf->buffer_len, puser_buf->dir, puser_buf->is_sec);
...
void channel_release_buffer(OMXVDEC_DRV_CONTEXT *drv_ctx, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    DEBUG_PRINT_TRACE("Free buffer phy:0x%x dir:%d  sec:%d success!\n", \
...
OMX_S32 channel_getVpssBypassInfo(OMXVDEC_DRV_CONTEXT *drv_ctx)
{
    ...
    drv_ctx->drv_cfg.bVpssBypass = (VpssBypass == 0)? 0 : 1;
