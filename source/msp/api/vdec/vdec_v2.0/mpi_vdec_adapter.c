...
static HI_S32 VFMW_AcqUserData(HI_HANDLE hInst, VFMW_USERDATA_S* pstParam)
{
	...
	// l00284814 for 64bit 
	pstParam->pstData->enBroadcastProfile = stParam.stUserData.enBroadcastProfile;
	pstParam->pstData->enPositionInStream = stParam.stUserData.enPositionInStream;
	pstParam->pstData->u32Pts             = stParam.stUserData.u32Pts;
	pstParam->pstData->u32SeqCnt          = stParam.stUserData.u32SeqCnt;
	pstParam->pstData->u32SeqFrameCnt     = stParam.stUserData.u32SeqFrameCnt;
	pstParam->pstData->pu8Buffer          = (HI_U8 *)(HI_SIZE_T)stParam.stUserData.pu8Buffer;
	pstParam->pstData->u32Length          = stParam.stUserData.u32Length;
	pstParam->pstData->bBufferOverflow    = stParam.stUserData.bBufferOverflow;
	pstParam->pstData->bTopFieldFirst     = stParam.stUserData.bTopFieldFirst;
    //end
...
static HI_S32 VFMW_RlsUserData(HI_HANDLE hInst, const HI_UNF_VIDEO_USERDATA_S* pstParam)
{
	...
	stParam.stUserData.enBroadcastProfile = pstParam->enBroadcastProfile;
	stParam.stUserData.enPositionInStream = pstParam->enPositionInStream;
	stParam.stUserData.u32Pts             = pstParam->u32Pts;
	stParam.stUserData.u32SeqCnt          = pstParam->u32SeqCnt;
	stParam.stUserData.u32SeqFrameCnt     = pstParam->u32SeqFrameCnt;
	stParam.stUserData.pu8Buffer          = (HI_U64)(HI_SIZE_T)pstParam->pu8Buffer;
	stParam.stUserData.u32Length          = pstParam->u32Length;
	stParam.stUserData.bBufferOverflow    = pstParam->bBufferOverflow;
	stParam.stUserData.bTopFieldFirst     = pstParam->bTopFieldFirst;
	//end
...
HI_S32 VDEC_SetHDRAttr(HI_HANDLE hInst, VDEC_HDR_ATTR_S *vdec_hdr_attr)
{
    ...
    if (vdec_hdr_attr == NULL)
    {
        HI_ERR_VDEC("Bad param.\n");
        return HI_ERR_CODEC_INVALIDPARAM;
    }
    vdec_cmd_hdr_attr.hHandle = VFMW_INST_HANDLE(hInst);
    vdec_cmd_hdr_attr.vdec_hdr_attr = *vdec_hdr_attr;
...
HI_S32 VDEC_GetLeftStreamFrm(HI_HANDLE hInst, HI_U32* pLeftFrm)
{
    HI_S32 s32Ret;;
...
#ifdef HI_HDR_SUPPORT
    case VFMW_CMD_SET_HDR_ATTR:
        return VDEC_SetHDRAttr(hInst, (VDEC_HDR_ATTR_S *)pParam);
#endif
    case VFMW_CMD_SET_COLORSPACE:
        return VDEC_SetColorSpace(hInst,(HI_UNF_COLOR_SPACE_E*)pParam);
    case VFMW_CMD_GET_LEFT_STREAM_FRAME:
        return VDEC_GetLeftStreamFrm(hInst, (HI_U32*)pParam);
    default:
        return HI_ERR_CODEC_UNSUPPORT;
    }
}
//add by l00225186
HI_S32 VPSS_RecvFrm(HI_HANDLE hVpss, VDEC_RECVFRM_OPT_S* pstVdecRecvOpt)
{
	HI_S32 s32Ret;
...
HI_S32 VPSS_Control(HI_HANDLE handle, HI_U32 u32CMD, HI_VOID * pParam)
{
	switch (u32CMD)
    {
    	...
    	case VPSS_CMD_RECEIVEFRAME:
    		return VPSS_RecvFrm(handle, (VDEC_RECVFRM_OPT_S*)pParam);
...
#endif
{
    ...
    {
        stUserAddr.u64UserAddr = stBuf.u32PhyAddr; /*前提是安全内存物理地址和虚拟地址一致，否则有极大风险,用户在安全侧用这个地址的时候也要先映射一下才使用的*/
    }
    else
#endif
    {
