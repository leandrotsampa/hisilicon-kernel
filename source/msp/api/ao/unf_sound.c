...
HI_S32  HI_UNF_SND_GetAdacEnable(HI_UNF_SND_E enSound,  HI_BOOL* pbEnable)
{
    HI_S32 s32Ret;

    API_SND_CheckId(enSound);

    s32Ret = HI_MPI_AO_SND_GetAdacEnable(enSound, pbEnable);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("HI_MPI_AO_SND_GetAdacEnable failed(0x%x)\n", s32Ret);
    }
