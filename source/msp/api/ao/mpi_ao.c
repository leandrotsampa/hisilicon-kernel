...
{
    AO_SND_SoundSetDelay_Param_S stSoundDelay;

    stSoundDelay.enSound = enSound;
    stSoundDelay.enOutPort = enOutPort;
    stSoundDelay.u32DelayMs = u32DelayMs;

    return ioctl(g_s32AOFd, CMD_AO_SND_SETSOUNDDELAY, &stSoundDelay);
}
...
HI_S32   HI_MPI_AO_SND_GetDelayCompensationMs(HI_UNF_SND_E enSound, HI_UNF_SND_OUTPUTPORT_E enOutPort, HI_U32* pu32DelayMs)
{
    ...
    s32Ret = ioctl(g_s32AOFd, CMD_AO_SND_GETSOUNDDELAY, &stSoundDelay);
    if (HI_SUCCESS == s32Ret)
    {
        *pu32DelayMs = stSoundDelay.u32DelayMs;
    }
