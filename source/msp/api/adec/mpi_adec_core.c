...
#if 0 //Spitzbube
    pstDfOpenParam->u32InADBufSize = ADEC_DEFAULT_INPUT_BUFFER_SIZE;
#endif
    return HI_SUCCESS;
}

static HI_S32 ADECSwInbufInit(ADEC_CHAN_S* pstAdecChan, HI_U32 u32PaddingSize)
{
...
#endif
    {
        if (HI_SUCCESS != ADECSwInbufInit(pstAdecChan, u32PaddingSize))
        {
            HI_ERR_ADEC("ADECSwInbufInit fail\n");
            return HI_FAILURE;
        }
    }
...
HI_S32 ADEC_GetDelayMs(HI_HANDLE hAdec, HI_U32* pDelayMs)
{
    ...
    // input buffer more than 1.2s but out buffer num less than 1 make flow control donot work
    //ADEC_GetDelayMs:u32OutBufDurationMs 1,*pDelayMs 1344
...
#endif
    {
        ...
        {
            HI_INFO_ADEC("u32OutBufDurationMs %d,*pDelayMs %d\n", u32OutBufDurationMs, *pDelayMs);
            *pDelayMs = 0;
        }
    }
...
static HI_VOID ADECResetProcInfo2(ADEC_CHAN_S* pstAdecChan)
{
    ...
    pstAdecInfo->u32ADBufSize  = pstAdecChan->InADStreamBuf.u32BufSize;
    pstAdecInfo->s32ADBufRead  = 0;
    pstAdecInfo->u32ADBufWrite = 0;
...
#if 0
    {
        HA_CODEC_COMPRESSMODE_SET_PARAM_S stCompressMode;
        memset(&stCompressMode, 0, sizeof(HA_CODEC_COMPRESSMODE_SET_PARAM_S));
        stCompressMode.bCompressMode = HI_TRUE;
        stCompressMode.enCmd = HA_CODEC_COMPRESSMODE_SET_CMD;
        HI_HA_DECODE_S* pHaDecoder = (HI_HA_DECODE_S*)pstAecAttr->pHaDecoderDev;
        pHaDecoder->DecSetConfig(pstAecAttr->hDecoder, &stCompressMode);
    }
#endif
    return Ret;
}
...
static HI_VOID ADECInitParams(HI_HANDLE hAdec)
{
    ADEC_CHAN_S* pstAdecChan = HI_NULL_PTR;
    pstAdecChan = g_pstAdecChan[hAdec];
    HI_S32 s32Ret;

    if (pstAdecChan->bDualDecoder == HI_TRUE)
    {
        s32Ret = ADEC_SetADMixAttr(hAdec, pstAdecChan->bADEnable, pstAdecChan->bADToSomePort);
        if (s32Ret != HI_SUCCESS)
        {
            HI_ERR_ADEC("Set AD Enable Failed\n");
        }

        s32Ret = ADEC_SetADBalance(hAdec, pstAdecChan->s16ADBalance);
        if (s32Ret != HI_SUCCESS)
        {
            HI_ERR_ADEC("ADEC_SetADBalance Failed\n");
        }

        HI_WARN_ADEC("ADECInitParams AD Enable %d, AD Balance %d\n", pstAdecChan->bADEnable, pstAdecChan->s16ADBalance);
    }
}


static HI_S32 ADECResetDecoder(HI_HANDLE hAdec, ADEC_INFO_S* pstAecAttr)
{
    HI_S32 nRet;
                                       ...
                                       HI_U32 u32StreamReadPos)
{
    ...
    HI_U32 u32FixStreamReadPos;
    HI_U32 u32FixADStreamReadPos = 0;
#ifdef HA_HW_CODEC_SUPPORT
    ADEC_STREAM_OUTBUF_S*  pstOutBuf = HI_NULL_PTR;
#endif
    HI_S32* ps32PcmBuf;
    //ADEC_PROC_ITEM_S* pstAdecInfo = HI_NULL;
    ADEC_PTS_INFO_S* pstPTSInfo;
    //ADEC_PTS_QUE_S*       pstPTSADQue;
    //pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstPTSInfo = &(pstAdecChan->stPTSInfo);
    //pstPTSADQue = &(pstAdecChan->PTSADQue);
    ...
    if (HI_TRUE == pstAdecChan->bHwDecoder)
    {
        ...
        if (pstAdecChan->bDualDecoder)
        {
            u32FixADStreamReadPos = pstAout->stPtsInfo.unAdPts.u32HwAdDecoderPtsReadPos;
        }
    }
    else
#endif
    ...
    pstPTSInfo->u32PTSDif = ptRetBuf->u32PTS - pstPTSInfo->u32LastPTS;
    pstPTSInfo->u32LastPTS  = ptRetBuf->u32PTS;

    if ((pstAdecChan->bDualDecoder == HI_TRUE) && (pstAdecChan->bADEnable == HI_TRUE))
    {
        ptRetBuf->u32ADPTS = ADECFindPTS(&(pstAdecChan->PTSADQue), (HI_U32)u32FixADStreamReadPos,
                                         u32PcmSamplesPerFrame,
                                         pstAout->u32OutSampleRate, &u32OrgADPTS);
        ptRetBuf->u32OrgADPTS = u32OrgADPTS;
        pstPTSInfo->u32ADPTSDif = ptRetBuf->u32ADPTS - pstPTSInfo->u32ADLastPTS;
        pstPTSInfo->u32ADLastPTS  = ptRetBuf->u32ADPTS;
#if 0
        //abnormal PTS process
        ADECDualDecoderPTS(pstAdecChan, ptRetBuf, pstAout, u32PcmSamplesPerFrame);
#endif
    }
    ...
    if (ADEC_MAX_VOLUME != pstAdecChan->u32Volume)
    {
        ...
        {
            pstOutBuf = &(pstAdecChan->outStreamBuf);
            ps32PcmBuf = (HI_S32*)((HI_VOID*)pstOutBuf->pu32BaseAddr + (HI_U32)((HI_U8*)pstAout->ps32PcmOutBuf - (HI_U8*)HI_NULL));
        }
        else
#endif
        {
...
static HI_S32 ADECGetADStreamPTS(ADEC_CHAN_S* pstAdecChan, HI_S32* s32BufferBps, HI_U32* pInStreamPts)
{
    ADEC_PTS_QUE_S*   pstPTSADQue = HI_NULL_PTR;
    ADEC_STREAM_BUFFER_S*  pstADInBuf = HI_NULL_PTR;
    HI_U32 u32totalbytes = 0;
    HI_U32 u32PtsMs;
    pstPTSADQue = &(pstAdecChan->PTSADQue);
    pstADInBuf = &(pstAdecChan->InADStreamBuf);
    if (pstPTSADQue->u32LastStorePtsMs == 0xffffffff)
    {
        u32PtsMs = 0;
        *pInStreamPts = u32PtsMs;
        return HI_SUCCESS;
    }
    if (pstPTSADQue->u32LastPtsMs == 0xffffffff)
    {
        pstPTSADQue->u32PtsBeforeRevise = pstPTSADQue->u32LastStorePtsMs;
    }
    if (pstPTSADQue->u32LastPtsMs < pstPTSADQue->u32LastStorePtsMs)
    {
        u32PtsMs = (pstPTSADQue->u32LastStorePtsMs - pstPTSADQue->u32LastPtsMs);
        pstPTSADQue->u32RecyleStorePtsMs    = 0;
        pstPTSADQue->u32RecycleFirstPtsMs = 0;
        pstPTSADQue->u32PtsBeforeRevise = pstPTSADQue->u32LastPtsMs;
    }
    else if (pstPTSADQue->u32LastStorePtsMs == pstPTSADQue->u32LastPtsMs)
    {
        u32PtsMs = 0;
    }
    else
    {
        if ((pstPTSADQue->u32RecyleStorePtsMs) && (pstPTSADQue->u32LastPtsMs <= pstPTSADQue->u32RecyleStorePtsMs))
        {
            u32PtsMs = (pstPTSADQue->u32RecyleStorePtsMs
                        - pstPTSADQue->u32LastPtsMs) + (pstPTSADQue->u32LastStorePtsMs - pstPTSADQue->u32RecycleFirstPtsMs); //pts re-cycle
            pstPTSADQue->u32RecycleFirstPtsMs = pstPTSADQue->u32LastStorePtsMs;
        }
        else if ((pstPTSADQue->u32RecyleStorePtsMs) && (pstPTSADQue->u32LastPtsMs > pstPTSADQue->u32RecyleStorePtsMs))
        {
            u32PtsMs = (pstPTSADQue->u32LastStorePtsMs - pstPTSADQue->u32RecycleFirstPtsMs); //pts re-cycle and modify
            pstPTSADQue->u32RecycleFirstPtsMs = pstPTSADQue->u32LastStorePtsMs;
        }
        else
        {
            u32PtsMs = pstPTSADQue->u32LastStorePtsMs - pstPTSADQue->u32PtsBeforeRevise; //pts modify,currentpts = lastpts + delta
            pstPTSADQue->u32PtsBeforeRevise = pstPTSADQue->u32LastStorePtsMs;
        }
    }
    ADECGetBsByteLefts(pstADInBuf, &u32totalbytes);
    if (u32PtsMs)
    {
        *s32BufferBps = (u32totalbytes * 1000) / (u32PtsMs);
    }
...
static HI_S32 ADECSwADInbufInit(ADEC_CHAN_S* pstAdecChan, HI_U32 u32PaddingSize)
{
    ADEC_STREAM_BUFFER_S*  pstInBuf = &pstAdecChan->InADStreamBuf;
...
static HI_S32 ADECInitADInBuf(ADEC_CHAN_S* pstAdecChan, ADEC_STREAM_BUFFER_S*  pstInBuf, HI_U32 u32PaddingSize,
                              HI_U32 u32AdecChID)
{
    CHECK_ADEC_NULL_PTR(pstInBuf);

    if (HI_SUCCESS != ADECSwADInbufInit(pstAdecChan, u32PaddingSize))
    {
        HI_ERR_ADEC("ADECSwADInbufInit fail\n");
        return HI_FAILURE;
    }
...
static HI_VOID ADECDeInitADInBuf(ADEC_CHAN_S* pstAdecChan, ADEC_STREAM_BUFFER_S*  pstInBuf)
{
    CHECK_NULL_PTR_NORET(pstInBuf);
...
static HI_U32 ADECFixMainStreamReadPos(ADEC_CHAN_S* pstAdecChan, HI_U32 u32StreamReadPos, HI_U32 u32BackwardBytes)
{
    HI_U32 u32FixStreamReadPos;
    ADEC_STREAM_BUFFER_S*  pstInBuf = &(pstAdecChan->InStreamBuf);

    if (u32StreamReadPos >= u32BackwardBytes)
    {
        u32FixStreamReadPos = u32StreamReadPos - u32BackwardBytes;
    }
    else
    {
        u32FixStreamReadPos = u32StreamReadPos;
    }
...
static HI_U32 ADECFixADStreamReadPos(ADEC_CHAN_S* pstAdecChan, HI_U32 u32StreamReadPos, HI_U32 u32BackwardBytes)
{
    HI_U32 u32FixStreamReadPos;
    ADEC_STREAM_BUFFER_S*  pstInBuf = &(pstAdecChan->InADStreamBuf);
...
static HI_U32 ADECFindInPTS(ADEC_PTS_QUE_S* pstPTSQue, HI_U32 u32Pos, HI_U32* pu32OrgPTS)
{
    HI_U32 u32PtsMs;
    HI_U32 Pos, FoundPtsPos = (HI_U32) - 1;
    HI_U32 rdPos;
    ...
    if (((HI_U32) - 1) == FoundPtsPos)
    {
        ...
        if (((HI_U32) - 1) != pstPTSQue->u32LastPtsMs)
        {
            u32PtsMs = pstPTSQue->u32LastPtsMs;
            if (((HI_U32) - 1) == u32PtsMs)
            {
                u32PtsMs = 0;
            }
        ...
        {
            /* if the PTS of first Frame isnot valid, dont insert the PTS */
            u32PtsMs = (HI_U32) - 1;
        }
    }
    else
...
static HI_VOID ADECGetADInputBuf(ADEC_CHAN_S* pstAdecChan, HI_HADECODE_INPACKET_S* pAvpkt)
{
    ADEC_STREAM_BUFFER_S*  pstInBuf   = HI_NULL_PTR;
    HI_S32 s32BsLeftBytes;

    pstInBuf = &(pstAdecChan->InADStreamBuf);
    memset(pAvpkt, 0, sizeof(HI_HADECODE_INPACKET_S));
...
static HI_VOID ADECUpdateADInputBuf(ADEC_CHAN_S* pstAdecChan, HI_U32 u32ConsumeBytes)
{
    ADEC_STREAM_BUFFER_S*  pstInBuf   = HI_NULL_PTR;
    ADEC_PROC_ITEM_S* pstAdecInfo = HI_NULL_PTR;
    ...
    //update intpu buf status
    pstAdecInfo->s32ADBufRead = pstInBuf->s32BufReadPos;
    return;
}

static HI_VOID ADECDualConvertHaOut2MpiOut(ADEC_CHAN_S* pstAdecChan, ADEC_OUTPUTBUF_S* ptRetBuf,
        HI_HADECODE_OUTPUT_S* pstAout)
{
    HI_S32* ps32PcmBuf;
    //ADEC_PTS_INFO_S* pstPTSInfo;
...
static HI_S32 ADECSwDualDecode(ADEC_CHAN_S* pstAdecChan)
{
    ...
    ADEC_PTS_QUE_S* pstPTSQue       = HI_NULL_PTR;
    ADEC_PTS_QUE_S* pstPTSADQue     = HI_NULL_PTR;
    HI_HA_DECODE_S* pHaDev          = HI_NULL_PTR;
    HI_HADECODE_INPACKET_S avpkt;
    HI_HADECODE_INPACKET_S avADpkt;
    HI_HADECODE_OUTPUT_S sOut;
    HI_U32 u32StreamReadPos       = 0;
    HI_U32 u32FrameConsumeBytes   = 0;
    HI_U32 u32ByteLeftBfDecode    = 0;
    HI_U32 u32ADStreamReadPos     = 0;
    HI_U32 u32ADFrameConsumeBytes = 0;
    HI_U32 u32ADByteLeftBfDecode  = 0;
    HI_U32 u32FixStreamReadPos    = 0;
    HI_U32 u32FixADStreamReadPos  = 0;
    HI_U32 u32OrgPTS              = 0;
    HI_U32 u32OrgADPTS            = 0;
    HI_U32 u32PcmSamplesPerFrame  = 0;
    ADEC_PTS_INFO_S* pstPTSInfo = &(pstAdecChan->stPTSInfo);
    ...
    while (1)
    {
        ...
        if (HI_TRUE == pstAdecChan->bADEnable)//Tocheck jiaxi
        {
            ADECGetADInputBuf(pstAdecChan, &avADpkt);
        }
        ...
        /* Get InPTS for MainStream before decode one frame */
        u32StreamReadPos    = ADECGetStreamReadPos(&pstAdecChan->InStreamBuf);
        u32FixStreamReadPos = ADECFixMainStreamReadPos(pstAdecChan, u32StreamReadPos, sOut.stPtsInfo.unPts.u32SwDecoderBytesLeft);
        avpkt.u32PtsMs = ADECFindInPTS(&(pstAdecChan->PTSQue), (HI_U32)u32FixStreamReadPos, &u32OrgPTS);
        pstPTSInfo->u32PTSDif  = avpkt.u32PtsMs - pstPTSInfo->u32LastPTS;
        pstPTSInfo->u32LastPTS = avpkt.u32PtsMs;
        u32ByteLeftBfDecode    = avpkt.s32Size;

        /* Get InPTS for ADStream before decode one frame */
        if (HI_TRUE == pstAdecChan->bADEnable)
        {
            u32ADStreamReadPos    = ADECGetStreamReadPos(&pstAdecChan->InADStreamBuf);
            u32FixADStreamReadPos = ADECFixADStreamReadPos(pstAdecChan, u32ADStreamReadPos, sOut.stPtsInfo.unAdPts.u32SwAdDecoderBytesLeft);
            avADpkt.u32PtsMs = ADECFindInPTS(&(pstAdecChan->PTSADQue), (HI_U32)u32FixADStreamReadPos, &u32OrgADPTS);
            pstPTSInfo->u32ADPTSDif  = avADpkt.u32PtsMs - pstPTSInfo->u32ADLastPTS;
            pstPTSInfo->u32ADLastPTS = avADpkt.u32PtsMs;
            u32ADByteLeftBfDecode    = avADpkt.s32Size;
            pstAdecInfo->u32ADPTSDif = pstPTSInfo->u32ADPTSDif;
            pstAdecInfo->u32PTSDif   = pstPTSInfo->u32PTSDif;
            if (avpkt.u32PtsMs > avADpkt.u32PtsMs)
            {
                pstAdecInfo->bPriPTSAhead = HI_TRUE;
                pstAdecInfo->u32PTSDelay = avpkt.u32PtsMs - avADpkt.u32PtsMs;
            }
            else
            {
                pstAdecInfo->bPriPTSAhead = HI_FALSE;
                pstAdecInfo->u32PTSDelay = avADpkt.u32PtsMs - avpkt.u32PtsMs;
            }
        }

        /* Decode one audio frame */
        ADEC_UNLOCK(&g_AdecIOMutex[pstAdecChan->u32AdecChID]);
        sRet = pHaDev->DecDualDecodeFrame(hDecoder, &avpkt, &avADpkt, &sOut);
        ADEC_LOCK(&g_AdecIOMutex[pstAdecChan->u32AdecChID]);
        ...
        if (HI_TRUE == pstAdecChan->bADEnable)
        {
            if ((HI_U32)avADpkt.s32Size > u32ADByteLeftBfDecode)
            {
                HI_ERR_ADEC("ADBytesBfDecode=0x%.8x,ADBytesAfDecode=0x%.8x\n", u32ADByteLeftBfDecode, avADpkt.s32Size);
                avADpkt.s32Size = u32ADByteLeftBfDecode;
            }
            u32ADFrameConsumeBytes = u32ADByteLeftBfDecode - avADpkt.s32Size;
        }
        ...
        if (HA_ErrorNone == sRet)    /* Success */
        {
            ...
            ADECDualConvertHaOut2MpiOut(pstAdecChan, ptRetBuf, &sOut);
            ...
            {
                pstAdecChan->CalPTS = avpkt.u32PtsMs;
                ptRetBuf->u32PTS    = avpkt.u32PtsMs;
            }
            else
            {
                ptRetBuf->u32PTS = pstAdecChan->CalPTS;
            }
            ptRetBuf->u32OrgPTS   = u32OrgPTS;
            ptRetBuf->u32OrgADPTS = u32OrgADPTS;
            /* look for PTS */
            u32PcmSamplesPerFrame = sOut.u32PcmOutSamplesPerFrame & 0xffff; //for dolby_truehd, frame_samples stored in 0~15 bit
            if (!sOut.u32PcmOutSamplesPerFrame)
            {
                if ((sOut.u32OutChannels) && (sOut.u32BitsOutBytesPerFrame & 0xffff))
                {
                    u32PcmSamplesPerFrame = (sOut.u32BitsOutBytesPerFrame & 0xffff) / (sOut.u32OutChannels * sizeof(HI_U16)); //only passthrough exist
                }
                else
                {
                    u32PcmSamplesPerFrame = 0;
            ...
            pstAdecChan->CalPTS += (u32PcmSamplesPerFrame * 1000) / pstAdecInfo->enSampleRate;

            ADECUpdateInputBuf(pstAdecChan, u32FrameConsumeBytes);
            if (HI_TRUE == pstAdecChan->bADEnable)
            {
                ADECUpdateADInputBuf(pstAdecChan, u32ADFrameConsumeBytes);
            }
        }
        else                         /* Not enough data or failed */
        {
            if (((pstPTSQue->ulPTSwrite + 1) % ADEC_MAX_STORED_PTS_NUM) == pstPTSQue->ulPTSread)
            {
                ADECDiscardPTS(pstPTSQue, u32FixStreamReadPos);
            }

            if (HI_TRUE == pstAdecChan->bADEnable)
            {
                if (((pstPTSADQue->ulPTSwrite + 1) % ADEC_MAX_STORED_PTS_NUM) == pstPTSADQue->ulPTSread)
                {
                    ADECDiscardPTS(pstPTSADQue, u32FixADStreamReadPos);
                }
            }

            if ((HA_ErrorNotEnoughData == sRet) || (HA_ErrorNotEnoughADData == sRet))
            {
                /* Use for check whether end of decode the input buffer(decide by main data) */
            ...
            else
            {
                ...
                if (HI_TRUE == pstAdecChan->bADEnable)
                {
                    if (!u32ADFrameConsumeBytes)
                    {
                        /* Decode holding, discard bytes */
                        u32ADFrameConsumeBytes = avADpkt.s32Size;
                        if (u32ADFrameConsumeBytes > ADEC_BYTES_DISCARD_THD)
                        {
                            u32ADFrameConsumeBytes = ADEC_BYTES_DISCARD_THD;
                        }
                    }
                }
            }
...
HI_S32 ADEC_GetADDelayMs(HI_HANDLE hAdec, HI_U32* pDelayMs)
{
    ...
    {
        ADECHwGetStatusInfo(pstAdecChan, &stStatus);
        pstAdecChan->InADStreamBuf.s32BufReadPos = stStatus.u32InBufReadPos;
        pstAdecChan->InADStreamBuf.u32BufWritePos = stStatus.u32InBufWritePos;
    }
#endif
    ADECGetADStreamPTS(pstAdecChan, &s32BufferBps, &u32InStreamPts);
    HI_SYS_GetTimeStampMs(&pstAdecChan->u32EndTime);
    u32TotalAdecTime = (pstAdecChan->u32EndTime - pstAdecChan->u32StartTime);
    if (u32TotalAdecTime > 0)
    {
...
HI_S32 ADEC_SendADStream(HI_HANDLE hAdec, const HI_UNF_STREAM_BUF_S* pstStream, HI_U32 u32PtsMs)
{
    ...
    pstPTSQue = &(pstAdecChan->PTSADQue);
    pstInBuf = &(pstAdecChan->InADStreamBuf);
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    ...
    if (HI_TRUE == ADECIsPTSFull(pstPTSQue))
    {
        ...
        ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
        HI_WARN_ADEC("ADECIsPTS Full fail \n" );
    ...
    if (HI_TRUE == pstAdecChan->bPacketDecoder)
    {
        if (HI_TRUE == ADECIsPacketFull(pstAdecChan->pstPacketQue))
        {
            ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
            ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
            HI_WARN_ADEC("ADECIsPacketFull fail \n" );
            return HI_ERR_ADEC_IN_PTSBUF_FULL;
        }
    }
    ...
    pstAdecInfo->u32ADBufWrite = pstInBuf->u32BufWritePos;
    ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
    ...
    if (ADEC_CMD_CTRL_START == pstAdecInfo->enEsCtrlState)
    {
        if (!fAdEstream)
        {
            fAdEstream = fopen(pstAdecInfo->adfilePath, "wb");
            if (!fAdEstream)
            {
                HI_ERR_ADEC("can not open file (%s)\n", pstAdecInfo->adfilePath);
            }
        }
        if (fAdEstream)
        {
            (HI_VOID)fwrite(pstStream->pu8Data, 1, pstStream->u32Size, fAdEstream);
        }
    }
                        ...
                        HI_UNF_STREAM_BUF_S* pstStream2)
{
    ...
    if (u32RequestSize == 0)
    {
        HI_WARN_ADEC("AD u32RequestSize == 0!\n");
        return HI_SUCCESS;
    }
    if (u32RequestSize > ADEC_MAX_INPUT_BLOCK_SIZE)
    {
        HI_WARN_ADEC("want AD stream u32Bytes big than %d !\n", ADEC_MAX_INPUT_BLOCK_SIZE );
        return HI_FAILURE;
    }
    ...
    CHECK_ADEC_STATEARG2(pstAdecChan->beWork, &g_AdecIOMutex[hAdec], &g_AdecApiMutex[hAdec]);
    pstInBuf = &(pstAdecChan->InADStreamBuf);
    pstMidState = &(pstAdecChan->midADState);
    pstPTSQue = &(pstAdecChan->PTSADQue);
    if (HI_TRUE == ADECIsPTSFull(pstPTSQue))
    {
        ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
...
HI_S32 ADEC_PutADBuffer(HI_HANDLE hAdec,
                        ...
                        HI_U32 u32PtsMs)
{
    ...
    {
        HI_ERR_ADEC("AD stream1 u32Size(%d)+ AD pstStream2 u32Size(%d) exceed maxsize(%d) \n",
                    ADEC_MAX_INPUT_BLOCK_SIZE );
        return HI_FAILURE;
    }
    ...
    {
        HI_ERR_ADEC("invalid AD pstStream1->pu8Data(0x%x) or AD pstStream2->pu8Data(0x%x) \n",
                    pstStream1->pu8Data,
                    pstStream2->pu8Data);
        return HI_FAILURE;
    }
    ...
    {
        HI_WARN_ADEC(" (AD pstStream1->u32Size + AD pstStream2->u32Size) == 0 \n");
        return HI_SUCCESS;
    }
    pstAdecChan = g_pstAdecChan[hAdec];
    ADEC_LOCK(&g_AdecApiMutex[hAdec]);
    ...
    if (ADEC_CMD_CTRL_START == pstAdecInfo->enEsCtrlState)
    {
        ...
        if (fAdEstream)
        {
            (HI_VOID)fwrite(pstStream1->pu8Data, 1, pstStream1->u32Size, fAdEstream);
            (HI_VOID)fwrite(pstStream2->pu8Data, 1, pstStream2->u32Size, fAdEstream);
        }
    }
    if (ADEC_CMD_CTRL_STOP == pstAdecInfo->enEsCtrlState)
    {
    ...
    pstAdecInfo->u32ADBufWrite = (HI_U32)pstInBuf->u32BufWritePos;
    ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
...
HI_VOID ADEC_DbgCountTryGetADBuffer(HI_HANDLE hAdec)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstAdecInfo->u32DbgGetADBufCount_Try++;
}
...
HI_VOID ADEC_DbgCountGetADBuffer(HI_HANDLE hAdec)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstAdecInfo->u32DbgGetADBufCount++;
}
...
HI_VOID ADEC_DbgCountTryPutADBuffer(HI_HANDLE hAdec)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstAdecInfo->u32DbgPutADBufCount_Try++;
}
...
HI_VOID ADEC_DbgCountPutADBuffer(HI_HANDLE hAdec)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstAdecInfo->u32DbgPutADBufCount++;
}
...
HI_VOID ADEC_DbgCountTrySendADStream(HI_HANDLE hAdec)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstAdecInfo->u32DbgSendADStraemCount_Try++;
}
...
HI_VOID ADEC_DbgCountSendADStream(HI_HANDLE hAdec)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    pstAdecInfo = pstAdecChan->pstAdecInfo;
    pstAdecInfo->u32DbgSendADStraemCount++;
}
...
static HI_S32 ADECInitChannel(ADEC_CHAN_S* pstAdecChan)
{
...
#ifdef HA_SW_DUALDEC_SUPPORT
    nRet = ADECInitADInBuf(pstAdecChan, &pstAdecChan->InADStreamBuf, u32PaddingSize, pstAdecChan->u32AdecChID);
    if (nRet != HI_SUCCESS)
    {
        ADECDeInitInBuf(pstAdecChan, &pstAdecChan->InStreamBuf);
        ADECDeInitDecoder(&pstAdecChan->decAttr);
        HI_ERR_ADEC("ADECInitADInBuf err=0x%x\n", nRet);
        return HI_FAILURE;
    }
#endif
    ...
    {
#ifdef HA_SW_DUALDEC_SUPPORT
        ADECDeInitADInBuf(pstAdecChan, &pstAdecChan->InADStreamBuf);
#endif
        ADECDeInitInBuf(pstAdecChan, &pstAdecChan->InStreamBuf);
        ADECDeInitDecoder(&pstAdecChan->decAttr);
...
#ifdef HA_SW_DUALDEC_SUPPORT
    ADECResetMidState(&pstAdecChan->midADState);
    ADECRestPTSQuet(&pstAdecChan->PTSADQue);
#endif
...
static HI_S32 ADECGetInputAttr(ADEC_CHAN_S* pstAdecChan, const ADEC_ATTR_S*  pstAttr)
{
    ...
    if (0xffffffff != pstAttr->u32CodecID)
    {
        ...
        HA_CODEC_DUALDECODE_QUERY_PARAM_S    stDualDecoder;
        ...
        stDualDecoder.enCmd = HA_CODEC_DUAL_DECODE_QUERY_CMD;
        stDualDecoder.bDualDecSupport = HI_FALSE;
        if (HI_SUCCESS == ADEC_SetConfigDeoder(pstAttr->u32CodecID, &stDualDecoder))
        {
            pstAdecChan->bDualDecoder = stDualDecoder.bDualDecSupport;
        }
        else
        {
            pstAdecChan->bDualDecoder = HI_FALSE;
        }
    }

    pstInBuf->u32BufSize = pstAttr->u32InBufSize;
    pstADInBuf->u32BufSize = pstAttr->u32InBufSize;
    ...
    HI_INFO_ADEC("u32CodecID=0x%x to 0x%x bDualDecoder=%d,beWork %d\n", pstAdecChan->u32CodecID, pstAdecChan->u32CodecID,
                 pstAdecChan->bDualDecoder, pstAdecChan->beWork);
    return HI_SUCCESS;
}
    ...
    while (pstAdecChan->AdecThreadRun)
    {
        ...
        if ((pstAdecChan->beAssigned == HI_TRUE) && (pstAdecChan->beWork == HI_TRUE))
        {
...
#endif
            {
                ...
                if ( pstAdecChan->bDualDecoder == HI_TRUE )
                {
                    (RET_VOID) ADECSwDualDecode( pstAdecChan );
                }
                else
#endif
                {
...
HI_S32 ADEC_Open(HI_HANDLE* phAdec)
{
...
#ifdef HA_SW_DUALDEC_SUPPORT
    pstAdecChan->CalPTS = 0;
#endif
    pstAdecChan->u32ADTotalAdecInByte = 0;
    ...
    *phAdec = chan_id;
    ...
    nRet = pthread_create(&pstAdecChan->AdecThreadInst, HI_NULL, ADEC_DecThread, (HI_VOID*)((HI_U8*)HI_NULL + *phAdec));
    if (HI_SUCCESS != nRet)
    {
...
HI_S32 ADEC_GetAttr(HI_HANDLE hAdec, HI_U32 u32Command, void*  pstAttr)
{
    ...
    switch (u32Command)
    {
        ...
        case ADEC_ATTR_ALLATTR:
...
#if 0 //Spitzbube
            ((ADEC_ATTR_S*)pstAttr)->u32InADBufSize  = pstInBuf->u32BufSize;
#endif
            ...
            ((ADEC_ATTR_S*)pstAttr)->bDualDec  = pstAdecChan->bDualDecoder;
            break;
...
HI_S32 ADEC_DropStream(HI_HANDLE hAdec, HI_U32 u32SeekPts)
{
    ...
    for (u32SeekPos = pstPTSQue->ulPTSread; u32SeekPos != pstPTSQue->ulPTSwrite; u32SeekPos = (u32SeekPos + 1) % (ADEC_MAX_STORED_PTS_NUM))
    {
        ...
        if (pstCurrentPTS->u32PtsMs != (HI_U32) - 1)
        {
            ...
            if ((u32SeekPts <= pstCurrentPTS->u32PtsMs) && (u32SeekPts > u32LastPts))
            {
                ...
                (RET_VOID)ADECResetDecoder(hAdec, pstAecAttr);       //reset the decoder in case there may be some data left in the codec
    ...
    if (u32SeekPts < pstCurrentPTS->u32PtsMs + ADEC_MAX_PTS_SCOPE)
    {
        ...
        (RET_VOID)ADECResetDecoder(hAdec, pstAecAttr);
        ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
        ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
        ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
        return HI_SUCCESS;
...
static HI_S32 ADECHwOutbufInit( ADEC_CHAN_S* pstAdecChan, HI_U32 uMaxBufSizePerFrame)
{
    ...
    stOutbufAddr.pu32OutBufAddr = pstOutBuf->pu32BaseAddr;
                                  ...
                                  ADEC_EXTFRAMEINFO_S* pstExtInfo)
{
    ...
    if (HA_ErrorNone == sRet)    /* Success */
    {
        ...
        pstAOFrame->ps32PcmBuffer  = (HI_S32*)((HI_VOID*)pstOutBuf->pu32BaseAddr + (HI_U32)((HI_U8*)(stOutPutBuff.ps32PcmOutBuf) - (HI_U8*)HI_NULL));
        pstAOFrame->ps32BitsBuffer = (HI_S32*)((HI_VOID*)pstOutBuf->pu32BaseAddr + (HI_U32)((HI_U8*)(stOutPutBuff.ps32BitsOutBuf) - (HI_U8*)HI_NULL));
...
HI_S32 ADEC_SetADMixAttr(HI_HANDLE hAdec, HI_BOOL bAdEnable, HI_BOOL bAdToSomePort)
{
    ADEC_CHAN_S* pstAdecChan = HI_NULL_PTR;
    HI_HA_DECODE_S* pHaDev;
    HA_CODEC_OUTPUT_AD_MODE_E enAdMode;
    HA_CODEC_ADMODE_SET_PARAM_S stAdMode;
    pstAdecChan = g_pstAdecChan[hAdec];
    ...
    if ( pstAdecChan->bDualDecoder == HI_TRUE )
    {
        if (bAdEnable == HI_TRUE && bAdToSomePort == HI_TRUE)
        {
            enAdMode = HA_OUTPUT_AD_OFF_WITHASSOC;
        }
        else if (bAdEnable == HI_TRUE && bAdToSomePort == HI_FALSE)
        {
            enAdMode = HA_OUTPUT_AD_ON;
        }
        else
        {
            enAdMode = HA_OUTPUT_AD_OFF;
        }

        stAdMode.enCmd = HA_CODEC_AD_OUTPUTMODE_SET_CMD;
        stAdMode.enAdMode = enAdMode;
        //HI_ERR_ADEC("Set AD Cmd %d\n", eAdMode);
        if (HA_ErrorNone != pHaDev->DecSetConfig(pstAdecChan->decAttr.hDecoder, (HI_VOID*)(&stAdMode)))
        {
            HI_ERR_ADEC("DecSetConfig fail\n");
            ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
            ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
    ...
    pstAdecChan->bADEnable = bAdEnable;
    pstAdecChan->bADToSomePort = bAdToSomePort;
    //HI_ERR_ADEC("AD Enable %d, ADToSomePort %d\n", bAdEnable, bAdToSomePort);
    pstAdecChan->pstAdecInfo->bADEnable = bAdEnable;
    //pstAdecChan->pstAdecInfo->bADToSomePort = bAdToSomePort;
    ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
...
HI_S32 ADEC_GetADMixAttr(HI_HANDLE hAdec, HI_BOOL* pbAdEnable, HI_BOOL* pbAdToSomePort)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    ADEC_LOCK(&g_AdecApiMutex[hAdec]);
    *pbAdEnable =  pstAdecChan->bADEnable;
    *pbAdToSomePort =  pstAdecChan->bADToSomePort;
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
...
HI_S32 ADEC_SetADBalance(HI_HANDLE hAdec, HI_S32 s16ADBalance)
{
    ADEC_CHAN_S* pstAdecChan = HI_NULL_PTR;
    HI_HA_DECODE_S* pHaDev;
    HA_CODEC_ADBALANCE_SET_PARAM_S    stADBalance;
    pstAdecChan = g_pstAdecChan[hAdec];
    CHECK_ADEC_STATE(pstAdecChan->beAssigned, &g_AdecApiMutex[hAdec]);
    CHECK_ADEC_STATE(pstAdecChan->beWork, &g_AdecApiMutex[hAdec]);
    ADEC_LOCK(&g_AdecApiMutex[hAdec]);
    ...
    if ( pstAdecChan->bDualDecoder == HI_TRUE )
    {
        stADBalance.enCmd = HA_CODEC_AD_BALANCE_SET_CMD;
        stADBalance.s16ADBalance = s16ADBalance;
        if (HA_ErrorNone != pHaDev->DecSetConfig(pstAdecChan->decAttr.hDecoder, (HI_VOID*)(&stADBalance)))
        {
            HI_ERR_ADEC(" AD Balance DecSetConfig fail\n");
            ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
            ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
            return HI_FAILURE;
        }
        pstAdecChan->s16ADBalance = s16ADBalance;
    }
    pstAdecChan->pstAdecInfo->s16ADBalance = s16ADBalance;
    ADEC_UNLOCK(&g_AdecDataMutex[hAdec]);
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
    return HI_SUCCESS;
}
...
HI_S32 ADEC_GetADBalance(HI_HANDLE hAdec, HI_S16* ps16ADBalance)
{
    ...
    pstAdecChan = g_pstAdecChan[hAdec];
    ADEC_LOCK(&g_AdecApiMutex[hAdec]);
    *ps16ADBalance =  pstAdecChan->s16ADBalance;
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
    return HI_SUCCESS;
}
